...
class CsvExportUtilTest {
    ...
    void withBom_prependsUtf8BomBytes() {
        ...
        byte[] result = CsvExportUtil.withBom(input);
    ...
    void withBom_contentAfterBomIsUnchanged() {
        ...
        byte[] result = CsvExportUtil.withBom(content);
    ...
    void withBom_emptyInput_returnsBomOnly() {
        byte[] result = CsvExportUtil.withBom(new byte[0]);
    ...
    void escape_equalsPrefix_isEscaped(String value) {
        assertThat(CsvExportUtil.escape(value)).startsWith("'=");
    }
    ...
    void escape_atPrefix_isEscaped() {
        assertThat(CsvExportUtil.escape("@SUM")).isEqualTo("'@SUM");
    }

    @Test
    void escape_tabPrefix_isEscaped() {
        assertThat(CsvExportUtil.escape("\tcell")).isEqualTo("'\tcell");
    }

    @Test
    void escape_crPrefix_isEscaped() {
        assertThat(CsvExportUtil.escape("\rcell")).isEqualTo("'\rcell");
    }
    ...
    void escape_safeValues_passThrough(String value) {
        assertThat(CsvExportUtil.escape(value)).isEqualTo(value);
    }

    @Test
    void escape_null_returnsNull() {
        assertThat(CsvExportUtil.escape(null)).isNull();
    }
    ...
    void escape_formulaCell_prefixedWithSingleApostrophe() {
        String result = CsvExportUtil.escape("=DANGEROUS()");
