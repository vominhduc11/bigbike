...
public class WordPressCouponMapper {
    ...
    public record MappedCoupon(
            ...
            String name,
            String description,
            String discountType,    // FIXED | PERCENT
            BigDecimal amount,
            ...
            int usageCount,
            Instant expiresAt,
            String status,          // ACTIVE | INACTIVE | ARCHIVED | EXPIRED
    ...
    public MappedCoupon map(WpPost post, List<WpPostMeta> metas) {
        ...
        String code = post.postTitle() != null ? post.postTitle().trim().toUpperCase() : "";
        if (code.isEmpty()) warnings.add("Empty coupon code for id=" + post.id());
        ...
        // Percent > 100 is invalid
        if ("PERCENT".equals(discountType) && amount != null
                && amount.compareTo(new BigDecimal("100")) > 0) {
            warnings.add("Percent coupon amount > 100: " + amount + " for id=" + post.id());
        }
        ...
        String status = deriveStatus(post.postStatus(), expiresAt);

        return new MappedCoupon(
                post.id(),
                code,
                post.postTitle() != null ? post.postTitle().trim() : "",
                post.postExcerpt(),
                discountType,
    ...
    private String deriveStatus(String postStatus, Instant expiresAt) {
        return switch (postStatus) {
            case "trash"   -> "ARCHIVED";
            case "draft", "pending" -> "INACTIVE";
            default -> {
    ...
    private String mapDiscountType(String type, List<String> warnings) {
        if (type == null || type.isBlank()) {
            warnings.add("Missing discount_type");
            return "FIXED";
        }
        return switch (type) {
            ...
            default -> {
                warnings.add("Unknown discount_type: " + type);
                yield "FIXED";
            }
        };
    }
    ...
    private Integer parseInt(String v, List<String> w) {
        ...
        try { return Integer.parseInt(v.trim()); }
            ...
            w.add("Cannot parse usage_limit: " + v);
            return null;
        }
    }
    ...
    private int parseInt2(String v, int def, List<String> w) {
        ...
        try { return Integer.parseInt(v.trim()); }
            ...
            w.add("Cannot parse usage_count: " + v);
            return def;
        }
    }

    private Instant parseExpiry(String v, List<String> w) {
        if (v == null || v.isBlank() || "0".equals(v.trim())) return null;
        try { return Instant.ofEpochSecond(Long.parseLong(v.trim())); }
        catch (NumberFormatException e) {
            w.add("Cannot parse date_expires: " + v);
            return null;
        }
    }
}
