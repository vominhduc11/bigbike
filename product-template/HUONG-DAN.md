...
>
> **Từ 2026-07-22:** khóa chuẩn cho danh mục là `categorySlugs: string[]`, theo đúng thứ tự; phần tử đầu là danh mục chính. Mỗi sản phẩm mới phải dùng ít nhất một slug danh mục đang hiển thị do shop cung cấp. Không dùng `uncategorized`: đây là danh mục hệ thống bị ẩn, chỉ là fallback sau khi xóa vĩnh viễn và không thể gán mới. Có thể dùng `categoryIds: string[]` thay cho slug list khi biết ID nội bộ; `categoryId` cũ chỉ là alias một phần tử trong giai đoạn chuyển đổi, không được gửi cùng mảng mới. Danh mục trùng bị gộp theo lần xuất hiện đầu; danh mục không tồn tại, đã ẩn hoặc nằm trong Thùng rác sẽ làm lỗi đúng dòng nhập. Sau khi nạp, admin có thể gán lại danh mục thật và đổi thứ tự.

---

## ⛔ 5 quy tắc BẮT BUỘC (sai là hỏng)

1. **`categorySlugs` là bắt buộc khi tạo mới, kể cả khi chỉ có một danh mục.** Đây là mảng slug danh mục theo thứ tự (ví dụ `["mu-bao-hiem", "phu-kien"]`); `brandId` là slug thương hiệu (ví dụ `ls2`). Hệ thống chỉ đối chiếu, không tự tạo; slug không tồn tại, bị ẩn hoặc nằm trong Thùng rác làm lỗi dòng đó. `categoryIds` là lựa chọn thay thế khi file có ID nội bộ; `categoryId` cũ là alias một slug trong giai đoạn chuyển đổi, tuyệt đối không gửi cùng `categorySlugs` hoặc `categoryIds`. Nếu dùng AI soạn file, shop phải cung cấp slug danh mục đang hiển thị cho từng sản phẩm; thiếu thông tin thì AI phải hỏi lại hoặc tách sản phẩm đó ra, không tự đoán và không dùng `uncategorized`.
   **Nếu tự tay soạn file** (không qua AI) và biết rõ đúng slug thật thì ghi thẳng danh sách slug theo thứ tự — **không tự đặt/đoán slug** cho danh mục hoặc thương hiệu chưa chắc đã có trong hệ thống (kể cả khi đó là thương hiệu/nhóm sản phẩm có thật ngoài đời — ví dụ bịa `"kovix"` khi shop chưa từng bán hàng Kovix, hoặc bịa `"khoa-chong-trom"` khi shop chưa có danh mục khoá) — phải hỏi lại người bán để lấy đúng slug đang dùng trong Admin. Không thể tạm dùng `"uncategorized"` cho danh mục vì nó bị ẩn và không gán mới được. Hai khoá xử lý khác nhau khi bỏ trống: `categorySlugs` **luôn bắt buộc phải có ít nhất một giá trị khi tạo mới** — bỏ trống báo lỗi "Thiếu danh mục", **không** tự hiểu là "Chưa phân loại"; `brandId` **được phép bỏ hẳn khoá** nếu chưa rõ thương hiệu — sản phẩm sẽ không gắn thương hiệu nào (hiện dấu "—"), khác với gán tường minh `"uncategorized-brand"` (hiện đúng chữ "Chưa phân loại").
   ...
   - **Ma trận bắt buộc theo có/không biến thể (PRODUCT_RULE_005, áp dụng ngay cả khi import, vì import gọi thẳng luồng lưu sản phẩm thường):** Luôn bắt buộc: `name` (khi tạo mới)/`categorySlugs` (hoặc `categoryIds`)/`brandId`/`gender`. Sản phẩm **KHÔNG có `variants`**: `sku` và `retailPrice` **cấp sản phẩm** cũng bắt buộc — thiếu 1 trong 2 sẽ báo lỗi dòng đó. Sản phẩm **CÓ `variants`**: `sku` cấp sản phẩm vẫn bắt buộc; mỗi phần tử trong `variants[]` bắt buộc phải có `sku` riêng — còn `retailPrice` riêng của từng biến thể **chỉ bắt buộc nếu sản phẩm không có `retailPrice` cấp sản phẩm hợp lệ** làm giá chung thay thế (xem mục Biến thể và ghi chú "giá chung" ở đầu file, 2026-07-07). Ảnh (`image`, `variants[].imageUrl`) **không bao giờ bắt buộc lúc import** vì import luôn tạo Nháp — ảnh chỉ bắt buộc khi admin bấm "Đăng" thủ công sau đó trong trang quản trị.
...
| `categorySlugs` | mảng chuỗi | **BẮT BUỘC khi tạo mới.** Các slug danh mục đang hiển thị theo thứ tự; phần tử đầu là danh mục chính (ví dụ `["mu-bao-hiem", "phu-kien"]`). AI chỉ dùng slug shop đã cung cấp, không đoán và không dùng `uncategorized`. `categoryIds` là lựa chọn thay thế theo ID nội bộ; `categoryId` cũ chỉ là alias một phần tử và không được kết hợp với mảng mới. |
...
- ⚠️ **Màu nhập qua file KHÔNG gắn vào danh sách màu chuẩn của shop** (khác với tự bấm chọn màu trên trang quản trị) — `optionValue` chỉ là chữ tự do, không đối chiếu với danh sách màu có sẵn. Web vẫn hiển thị/lọc màu bình thường, nhưng: (1) gõ lệch chính tả giữa các sản phẩm (vd "Đen" / "đen" / "Đen bóng") sẽ tạo ra các màu tưởng như khác nhau, không gộp lại được — nên **dùng đúng tên màu đã có sẵn trong danh sách màu của shop** khi soạn `optionValue`. Lưu ý: trang quản trị **không có màn hình riêng liệt kê toàn bộ màu đang dùng** — màu chỉ hiện khi mở từng sản phẩm → sửa biến thể → ô chọn màu của sản phẩm đó, và AI soạn file không tự nhìn thấy danh sách này. Nếu muốn AI dùng đúng tên màu có sẵn, phải **tự liệt kê tay các màu shop hay dùng rồi dán vào prompt cho AI**; nếu không, AI sẽ tự đặt tên màu theo mô tả sản phẩm — vẫn nạp được bình thường (không phải khoá bắt buộc khớp có sẵn như `categorySlugs`/`brandId`), chỉ là dễ lệch chính tả như trên, nên nạp xong nên tự rà lại. (2) Nạp file để **cập nhật lại** một sản phẩm đã có biến thể màu gắn sẵn với danh sách màu chuẩn (do trước đó chọn tay trên quản trị) sẽ **âm thầm gỡ mối gắn đó** — màu vẫn hiện đúng chữ như cũ, không vỡ gì trên web, nhưng mất khả năng đổi tên màu hàng loạt qua danh sách màu chuẩn, dù file nạp không hề chủ ý đổi gì về màu.
...
- Khoá **có đưa vào** trong số `commitments`, `highlights.positiveNotes`, `highlights.negativeNotes`, `faqs`, `variants` → **thay thế toàn bộ danh sách cũ bằng danh sách mới**, không cộng dồn. Muốn giữ mục cũ + thêm mục mới → phải liệt kê lại **đầy đủ cả cũ lẫn mới** trong cùng mảng đó. (`positiveNotes`/`negativeNotes` là 2 mảng con độc lập trong `highlights` — chỉ đưa `highlights.positiveNotes` thì `negativeNotes` cũ vẫn giữ nguyên, và ngược lại.) ⚠️ Với `variants` có màu: nạp lại sẽ gỡ mối gắn màu với danh sách màu chuẩn của các biến thể đó dù không đổi gì về màu — xem cảnh báo ở mục Biến thể phía trên.
...
BẮT BUỘC:
- Mỗi object tạo mới PHẢI có ít nhất một `categorySlugs` đang hiển thị trong danh sách tôi cung
  cấp, cùng `brandId` (nếu biết) và `gender`. Không tự khớp, không tự đoán, không tự đặt slug
  danh mục/thương hiệu. Nếu thiếu danh mục hợp lệ cho một sản phẩm, hãy liệt kê sản phẩm đó để
  tôi quyết định trước; không dùng `uncategorized` vì danh mục hệ thống này bị ẩn và bị chặn khi
  gán mới.
...
Kiểm tra sau khi AI trả về: mở JSON bằng công cụ kiểm tra (jsonlint) xem có hợp lệ không; rà mọi sản phẩm tạo mới đều có `categorySlugs` không rỗng, không trùng và chỉ gồm slug danh mục đang hiển thị shop đã cung cấp; rà mỗi cột song ngữ đều là object lồng (không còn khoá `translations` rời). (Không cần rà nguồn ảnh trong descriptionBlocks/suitabilitySection/sizeGuideSection nữa — ảnh ở đây luôn bị xoá khi nạp bất kể nguồn.)
