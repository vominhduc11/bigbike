...
1. **`categoryId` là bắt buộc ở mọi object — kể cả khi chỉ cập nhật sản phẩm đã có.** `categoryId` là **slug danh mục** (ví dụ `mu-bao-hiem`), `brandId` là **slug thương hiệu** (ví dụ `ls2`) — **KHÔNG** phải mã nội bộ dạng `cat_...`/`brand_...`. Khi **tạo mới** phải có `name.nameVI` và `name.nameEN` — thiếu tên tiếng Anh sẽ báo lỗi dòng đó. Khi **cập nhật** sản phẩm cũ: nếu file **không** đổi tên, có thể bỏ hẳn khoá `name` (tên VI/EN cũ được giữ nguyên); nhưng nếu file **có** đổi `nameVI`, bắt buộc phải kèm `nameEN` mới trong cùng khoá `name` đó.
   - **Ma trận bắt buộc theo có/không biến thể (PRODUCT_RULE_005, áp dụng ngay cả khi import, vì import gọi thẳng luồng lưu sản phẩm thường):** Luôn bắt buộc: `name` (khi tạo mới)/`categoryId`/`brandId`/`gender`. Sản phẩm **KHÔNG có `variants`**: `sku` và `retailPrice` **cấp sản phẩm** cũng bắt buộc — thiếu 1 trong 2 sẽ báo lỗi dòng đó. Sản phẩm **CÓ `variants`**: `sku` cấp sản phẩm không bắt buộc (có thể bỏ hẳn); mỗi phần tử trong `variants[]` bắt buộc phải có `sku` riêng — còn `retailPrice` riêng của từng biến thể **chỉ bắt buộc nếu sản phẩm không có `retailPrice` cấp sản phẩm hợp lệ** làm giá chung thay thế (xem mục Biến thể và ghi chú "giá chung" ở đầu file, 2026-07-07). Ảnh (`image`, `variants[].imageUrl`) **không bao giờ bắt buộc lúc import** vì import luôn tạo Nháp — ảnh chỉ bắt buộc khi admin bấm "Đăng" thủ công sau đó trong trang quản trị.
...
| `specificationsHtml` | obj | `{ "specificationsHtmlVI": "...", "specificationsHtmlEN": "..." }` — bảng thông số kỹ thuật (HTML thô), mỗi vế tuỳ chọn độc lập. |
...
### Ô số liệu nổi bật — `specStatsHtml`
`{ "specStatsHtmlVI": "...", "specStatsHtmlEN": "..." }` — dải **con số bán hàng nổi bật** (pin, tầm xa, chống nước…) dạng HTML thô, không phải bảng thông số kỹ thuật. Mỗi vế tuỳ chọn độc lập.

### Dải tin cậy — `trustBadgesHtml`
`{ "trustBadgesHtmlVI": "...", "trustBadgesHtmlEN": "..." }` — dải nhãn tin cậy (bảo hành, chính hãng, freeship…) dạng HTML thô. Mỗi vế tuỳ chọn độc lập.
...
- Với các khoá song ngữ lồng nhau (`name`, `slug`, `shortDescription`, `specificationsHtml`, `specStatsHtml`, `trustBadgesHtml`, `quickAnswerSummary`, `originBrandCountry`, `seo`): chỉ cần gửi **vế nào muốn đổi** trong object đó — vd `"trustBadgesHtml": { "trustBadgesHtmlEN": "..." }` chỉ đổi bản tiếng Anh, bản tiếng Việt giữ nguyên. Bỏ hẳn cả khoá cha (vd bỏ hẳn khoá `trustBadgesHtml`) mới là "không đổi gì cả ở cột đó".
...
- Các cột song ngữ — MỖI CỘT LÀ 1 OBJECT LỒNG chứa cả 2 ngôn ngữ, không tách rời:
  ...
  { shortDescriptionVI, shortDescriptionEN }, specificationsHtml:
  { specificationsHtmlVI, specificationsHtmlEN }, specStatsHtml:
  { specStatsHtmlVI, specStatsHtmlEN }, trustBadgesHtml:
  { trustBadgesHtmlVI, trustBadgesHtmlEN }, originBrandCountry (xuất xứ
...
- Sản phẩm CÓ variants: bỏ hẳn sku cấp sản phẩm; MỖI biến thể trong variants phải có
