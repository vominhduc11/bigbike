...
| `genders` | mảng chuỗi | **Tuỳ chọn.** `[]`, `["Nam"]`, `["Nữ"]` hoặc `["Nam", "Nữ"]`; thứ tự chuẩn là `Nam` rồi `Nữ`. File cũ có thể dùng `gender: "Nam"`/`"Nữ"`; `gender: "Unisex"` được hiểu là `genders: []`. |
...
- Hàng hoá: sku, categorySlugs (dùng slug danh mục đang hiển thị mà tôi cung cấp), brandId,
  genders, retailPrice, salePrice. `genders` là tuỳ chọn: có thể là `[]`, `["Nam"]`,
  `["Nữ"]` hoặc `["Nam", "Nữ"]`. TUYỆT ĐỐI KHÔNG thêm khoá variants — biến thể
...
BẮT BUỘC:
- `genders` không bắt buộc. Nếu có gắn giới tính, chỉ dùng `Nam`, `Nữ` hoặc cả hai; không dùng
  `Unisex` và không suy đoán giới tính từ tên/danh mục. Với `categorySlugs`/`brandId`: dùng đúng slug tôi đã cung cấp
