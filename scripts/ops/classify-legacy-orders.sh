...
if [[ "$mode" == "--dry-run" ]]; then
  ...
  psql -P pager=off -c "
    ...
    where batch_key = 'LEGACY_WEB_IMPORT_2026_06_11';"
...
if [[ "$mode" == "--execute" ]]; then
  psql -c "
    ...
    begin
      ...
      if actual_total <> 1660 or actual_pending <> 388 or actual_processing <> 508 then
        raise exception 'Dừng an toàn: đo được total=%, pending=%, processing=%; cần đúng 1660/388/508.',
    ...
    insert into order_history_batches (
      ...
      '{\"selector\":\"legacy_id IS NOT NULL\",\"measuredOn\":\"2026-09-02\",\"expectedTotal\":1660,\"expectedPending\":388,\"expectedProcessing\":508}'::jsonb,
      1660, 388, 508, true
    )
    ...
    begin
      ...
      where b.batch_key = 'LEGACY_WEB_IMPORT_2026_06_11';
      if marked_count <> 1660 then
        raise exception 'Dừng an toàn: lô % có % đơn đã đánh dấu, cần đúng 1660.',
          'LEGACY_WEB_IMPORT_2026_06_11', marked_count;
      end if;
    end \$\$;
    commit;"
  echo "Đã đánh dấu 1.660 đơn lịch sử; không sửa bảng orders. Thời gian: $(( $(date +%s) - started_epoch )) giây."
  exit 0
fi
...
if [[ "$mode" == "--rollback" ]]; then
  psql -c "
    ...
    where batch_key = 'LEGACY_WEB_IMPORT_2026_06_11' and active = true;"
...
psql -c "
  ...
  begin
    ...
    if candidate_count <> 1660 or marked_count <> 1660 or mismatch_count <> 0 then
      raise exception 'Dừng an toàn: candidate=%, marked=%, mismatch=%; cần 1660/1660/0.',
  ...
  where batch_key = 'LEGACY_WEB_IMPORT_2026_06_11';
  commit;"
echo "Đã bật lại đúng lô 1.660 đơn lịch sử. Thời gian: $(( $(date +%s) - started_epoch )) giây."
