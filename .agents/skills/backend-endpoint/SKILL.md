...
Đọc trước (chỉ section liên quan; bảng tra đầy đủ ở `AGENTS.md` / `AGENTS.md` §3–§4): `API_CONTRACT.md` (endpoint shape), `DATA_CONTRACT.md` (entity/field), `PERMISSION_MATRIX.md` (permission key), `STATE_MACHINES.md` (nếu có transition), `BUSINESS_RULES.md` (rule liên quan). Đổi shape/rule → update docs trước.
