...
- Docs-first: mở đầu bằng bảng tra "sửa cái này thì đọc docs nào" trong `AGENTS.md` (hoặc `AGENTS.md` §3–§4) để chốt danh sách docs phải đọc — chỉ đọc đúng section liên quan. Nếu docs ghi `NEEDS_VERIFICATION` / `NOT_FOUND_IN_REPO` / `CONFLICTING_EVIDENCE` cho điều cần biết → **không bịa** permission/field/lifecycle/validation/side effect; gác món phụ thuộc rule đó vào "Cần owner quyết" và **làm tiếp phần còn lại**.
