---
name: preflight
description: "Chạy trước khi commit, push hoặc mở PR trong repo BigBike. Tự phát hiện sub-project nào đã đổi gồm web, admin hoặc backend; chạy đúng bộ check mirror CI của riêng nó; kiểm tra docs đã update nếu thay đổi chạm contract, data, permission hoặc state; và xuất PR summary đúng format bắt buộc."
---
...
| `docs/` | docs (có trong git) | Không có build gate — xem Bước 3 |
...
# ADMIN — có lint + test (vitest) + build
(cd bigbike-admin && npm run lint && npm run test && npm run build)
...
Nếu code đổi mà docs **chưa** phản ánh → update docs trước, rồi mới coi là xong. `docs/` nằm trong git và là source of truth — docs phải đi **cùng** thay đổi code trong một PR, đúng như Docs-First Contract (AGENTS.md §2).
...
- Nếu sửa text UI / CSS → cân nhắc chạy skill `hygiene` (mojibake, dead CSS).
...
- ❌ Không chạy `npm run test:e2e` ở admin trong preflight — Playwright cần container `:4000` chạy thật và login bị giới hạn 5 lần/phút. Chỉ chạy khi được yêu cầu rõ.
