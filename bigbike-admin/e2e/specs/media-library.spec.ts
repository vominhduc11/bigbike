...
async function captureLibrary(page: Page, testInfo: TestInfo) {
  for (const viewport of CAPTURE_VIEWPORTS) {
    ...
    const screenshotPath = testInfo.outputPath(
      `media-library-${CAPTURE_LABEL}-${viewport.name}.png`,
    )
...
test.describe('media-library', () => {
  test('bố cục thư viện sạch ở desktop, tablet và mobile', async ({
    adminPage,
    collect,
  }, testInfo) => {
    await navigateSpa(adminPage, '/admin/media')
    await expect(
      adminPage.getByRole('heading', { name: 'Thư viện ảnh', exact: true }),
    ).toBeVisible()
  ...
  test('tải ảnh/video, tải bản gốc, sửa, xoá mềm, khôi phục và xoá vĩnh viễn tệp thử nghiệm', async ({
    adminPage,
    collect,
  }) => {
    test.skip(
      Boolean(process.env.MEDIA_CAPTURE_ONLY),
      'Chỉ chụp ảnh trước/sau, không thay đổi dữ liệu.',
    )
    ...
    try {
      ...
      await test.step('tải, hiển thị và xoá sạch MP4 hợp lệ', async () => {
        ...
        const downloadedVideo = await downloadFromButton(
          adminPage,
          cardDownload,
          `${videoMarker}.mp4`,
        )
        ...
        const softDeleted = await adminPage.request.delete(
          `${apiOrigin}/api/v1/admin/media/${videoId}`,
          { headers },
        )
        ...
        expect(
          downloadedTrashVideo.equals(videoBytes),
          'Video trong thùng rác vẫn phải tải đúng bản gốc',
        ).toBeTruthy()
        const hardDeleted = await adminPage.request.delete(
          `${apiOrigin}/api/v1/admin/media/${videoId}?permanent=true`,
          { headers },
        )
      ...
      await test.step('tải SVG hợp lệ lên thư viện', async () => {
        ...
        const replaceResponse = await adminPage.request.post(
          `${apiOrigin}/api/v1/admin/media/${mediaId}/replace`,
        ...
        expect(downloadedImage.length, 'Ảnh tải về phải là object gốc, không phải biến thể').toBe(
          imageFileSize,
        )
      })
      ...
      await test.step('mở chi tiết và cập nhật metadata', async () => {
        ...
        const panel = adminPage.getByRole('complementary', {
          name: 'Chỉnh sửa thông tin ảnh/video',
        })
        ...
        await card.getByRole('button', { name: 'Sửa', exact: true }).click()
        const detailPanel = adminPage.getByRole('complementary', {
          name: 'Chỉnh sửa thông tin ảnh/video',
        })
        ...
        const downloadedFromLightbox = await downloadFromButton(
          adminPage,
          lightboxDownload,
          filename,
        )
      ...
      if (mediaId && apiOrigin && authorization) {
        ...
        await adminPage.request
          .delete(`${apiOrigin}/api/v1/admin/media/${mediaId}`, { headers })
          .catch(() => {})
        await adminPage.request
          .delete(`${apiOrigin}/api/v1/admin/media/${mediaId}?permanent=true`, { headers })
          .catch(() => {})
      }
      if (videoId && apiOrigin && authorization) {
        const headers = { Authorization: authorization }
        await adminPage.request
          .delete(`${apiOrigin}/api/v1/admin/media/${videoId}`, { headers })
          .catch(() => {})
        await adminPage.request
          .delete(`${apiOrigin}/api/v1/admin/media/${videoId}?permanent=true`, { headers })
          .catch(() => {})
      }
    }
  })
