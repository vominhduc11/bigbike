 ...
 */
test('inventory · the morning digest opens both complete product sections', async ({ adminPage, collect }) => {
  ...
  const digestItem = adminPage
    ...
    .first()
  await expect(digestItem, 'No persisted out-of-stock digest is available in the bell').toBeVisible()
  ...
  await expect(dialog.getByRole('heading', { name: /Thiếu cỡ hoặc màu|Missing size or colour/i })).toBeVisible()
  ...
  expect(await productLinks.count(), 'Digest should contain direct product-edit links').toBeGreaterThan(0)
