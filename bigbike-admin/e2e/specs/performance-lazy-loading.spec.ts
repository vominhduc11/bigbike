import { testAnon as anonTest, expect as anonExpect, expectRuntimeClean as expectAnonRuntimeClean } from '../fixtures/admin-test'
...
anonTest('hiển thị đăng nhập không lấy chunk trình soạn thảo', async ({ adminPage: page, collect }) => {
...
anonTest('màn đăng nhập giữ bố cục ở khổ điện thoại', async ({ adminPage: page, collect }) => {
  ...
  anonExpect(horizontalOverflow, 'Màn đăng nhập không được tràn ngang trên điện thoại').toBeLessThanOrEqual(1)
  expectAnonRuntimeClean(collect, { allowApi: true })
})

test('chỉ nạp biểu đồ khi cuộn tới, và rê menu chỉ nạp màn được trỏ', async ({ adminPage, collect }) => {
  await adminPage.setViewportSize({ width: 1440, height: 480 })
  await adminPage.route('**/api/v1/admin/dashboard?*', async (route) => {
    await route.fulfill({ contentType: 'application/json', body: JSON.stringify(DASHBOARD_WITH_CHARTS) })
  })
  ...
  expect(Math.abs(heightAfter - heightBefore), 'Vùng chờ biểu đồ doanh thu phải giữ chiều cao ổn định').toBeLessThanOrEqual(2)
  ...
  }, { message: 'Rê đúng menu Đơn hàng phải nạp trước riêng màn Đơn hàng' }).toBe(true)
  const afterHover = await loadedResourceNames(adminPage)
  expect(afterHover.some((url) => /ProductListScreen-[\w-]+\.js/.test(url)), 'Không được nạp trước màn khác').toBe(false)
  expectRuntimeClean(collect)
})

test('mở màn biên tập và cấu hình sẽ nạp trình soạn thảo mà không ghi dữ liệu', async ({ adminPage, collect }) => {
