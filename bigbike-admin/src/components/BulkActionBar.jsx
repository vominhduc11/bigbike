...
export function BulkActionBar({
  ...
  const countLabel =
    typeof selectedCount === 'number'
      ? t('common.selectedCount', {
          count: selectedCount,
          defaultValue: `${selectedCount} đã chọn`,
        })
      : selectedCount
  const showSelectAll =
  ...
  return (
    <div
      className={cn(
        'flex flex-wrap items-center gap-3 rounded-md border border-border bg-surface px-4 py-3 shadow-sm',
        className,
      )}
      role="region"
      aria-label={t('common.bulkActions', { defaultValue: 'Hành động hàng loạt' })}
    >
      ...
      {showSelectAll && (
        ...
        >
          {t('common.selectAllMatching', {
            count: totalMatching,
            defaultValue: `Chọn tất cả ${totalMatching} kết quả khớp`,
          })}
        </Button>
      )}
      {allMatchingSelected && typeof totalMatching === 'number' && (
        <span className="text-xs text-muted-foreground">
          {t('common.allMatchingSelected', {
            count: totalMatching,
            defaultValue: `Đã chọn tất cả ${totalMatching} kết quả`,
          })}
        </span>
      )}
