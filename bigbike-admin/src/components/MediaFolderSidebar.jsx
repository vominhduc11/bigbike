...
import { ChevronDown, ChevronRight, Folder, FolderOpen, Plus, Pencil, Trash2, Inbox, Hash } from 'lucide-react'
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-4 self-start rounded-[var(--admin-radius-card)] border border-border bg-surface p-3 lg:sticky lg:top-4 lg:max-h-[calc(100vh-var(--admin-space-8))] lg:w-56 lg:overflow-y-auto">
      ...
      <section className="flex flex-col gap-2">
        ...
        <ul className="m-0 flex list-none flex-col gap-1 p-0" role="tree" aria-label={t('media.folders')}>
          <li>
      ...
      <section className="flex flex-col gap-2">
        ...
        <ul className="m-0 flex list-none flex-col gap-1 p-0">
          ...
          <FolderTree
            ...
            onSelectFolder={(id) => { onSelectFolder(id); setMobileOpen(false) }}
...
function FolderTree({
  ...
  return folders.map((folder) => {
    ...
    return (
      <li key={folder.id} role="treeitem" aria-level={folder.depth + 1} aria-expanded={hasChildren ? expanded : undefined}>
        {editingId === folder.id ? (
          <FolderInput defaultValue={folder.name}
            onSubmit={(name) => onRename(folder.id, name, folder)}
            onCancel={() => onStartEditing(null)} />
        ) : (
          <div className={cn('relative flex items-center rounded-[var(--admin-radius-control)]', folderFilter === folder.id && 'bg-surface-selected text-primary')}>
            <Button
              type="button"
              variant="ghost"
              size="icon"
              ...
              aria-label={hasChildren ? (expanded ? t('media.collapseFolder') : t('media.expandFolder')) : t('media.folderNoChildren')}
              aria-expanded={hasChildren ? expanded : undefined}
            >
              {hasChildren ? (expanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />) : <span className="size-3" aria-hidden="true" />}
            </Button>
            <Button variant="ghost" onClick={() => onSelectFolder(folder.id)}
              ...
              <span className="min-w-0 flex-1 truncate text-left">{getMediaFolderLabel(folder, t)}</span>
              <span className={cn('rounded-full bg-surface-muted px-2 text-xs text-muted-foreground', folderFilter === folder.id && 'bg-primary text-primary-foreground')}>
                {folder.mediaCount}
              </span>
            </Button>
            {canUpdate && manageMode && !protectedFolder && (
              <div className="absolute right-1 top-1/2 flex -translate-y-1/2 gap-1 bg-surface p-1">
                <Button variant="ghost" size="sm" onClick={() => onStartEditing(folder.id)}
