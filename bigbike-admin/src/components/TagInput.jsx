...
export function TagInput({ value, onChange, placeholder, disabled }) {
  ...
  function onKeyDown(e) {
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      if (visibleSuggestions.length) { setShowSugg(true); setActiveIndex((i) => (i + 1) % visibleSuggestions.length) }
    } else if (e.key === 'ArrowUp') {
      e.preventDefault()
      if (visibleSuggestions.length) setActiveIndex((i) => (i <= 0 ? visibleSuggestions.length - 1 : i - 1))
    } else if (e.key === 'Enter') {
      e.preventDefault()
      if (activeIndex >= 0 && visibleSuggestions[activeIndex]) addTag(visibleSuggestions[activeIndex])
      ...
      <ul id={listboxId} role="listbox" className="list-none p-1">
        {suggError ? (
          <li role="alert" className="px-2 py-1 text-xs text-danger">
            {t('common.tagFetchError', { defaultValue: 'Không tải được gợi ý thẻ. Bạn vẫn có thể tự nhập thẻ.' })}
          </li>
