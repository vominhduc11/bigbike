...
export function CollapsibleSection({
  ...
  if (keepMounted) {
    body = (
      <div id={panelId} className={cn('flex flex-col gap-5', bodyClassName)} hidden={!open}>
        {children}
      </div>
    )
  } else if (open) {
    body = (
      <div id={panelId} className={cn('flex flex-col gap-5', bodyClassName)}>
        {children}
      </div>
    )
  }
  return (
    <section className={cn('flex flex-col gap-4', helpContent && 'relative', className)}>
      ...
      >
        ...
        {hint && !longHint ? (
          <span className="text-xs font-medium leading-snug text-muted-foreground">· {hint}</span>
        ) : null}
        {badge}
      </Button>
      {helpContent ? (
        <HelpTooltip content={helpContent} className="absolute right-3 top-2" />
      ) : null}
      {body}
    </section>
  )
