...
function ProductEditLink({ item, language, navigate, onClose, children }) {
  ...
  return (
    <a href={href} onClick={handleClick} className="block focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2">
      ...
      <DialogContent className="max-w-4xl overflow-hidden p-0">
        <DialogHeader className="border-b border-border pr-14">
          ...
          <div className="flex flex-wrap gap-2 pt-2 text-xs font-semibold">
            <span className="rounded-[var(--admin-radius-control)] bg-surface-muted px-3 py-2 text-foreground">
              {t('notifications.fullyOutTitle')}: {Number(counts.fullyOutOfStockProducts) || full.length}
            </span>
            <span className="rounded-[var(--admin-radius-control)] bg-surface-muted px-3 py-2 text-foreground">
              {t('notifications.partiallyOutTitle')}: {Number(counts.partiallyOutOfStockProducts) || partial.length}
            </span>
            <span className="rounded-[var(--admin-radius-control)] bg-surface-muted px-3 py-2 text-foreground">
              {t('notifications.unavailableVariants', { count: Number(counts.unavailableVariants) || 0 })}
            </span>
          </div>
        </DialogHeader>
        ...
        <div className="max-h-[70vh] space-y-6 overflow-y-auto p-6">
          <section aria-labelledby="fully-out-heading">
            <div className="mb-3">
              <h3 id="fully-out-heading" className="m-0 text-sm font-bold uppercase text-foreground">
                {t('notifications.fullyOutTitle')}
              </h3>
              <p className="mb-0 mt-1 text-xs text-muted-foreground">{t('notifications.fullyOutDescription')}</p>
            </div>
                ...
                >
                  <span className="flex flex-col gap-2 rounded-[var(--admin-radius-control)] border border-border bg-card px-4 py-3 transition-colors hover:bg-surface-hover sm:flex-row sm:items-center sm:justify-between">
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-semibold text-foreground">{localizedName(product, language)}</span>
                      <span className="mt-1 block text-xs text-muted-foreground">{t('notifications.sku')}: {product.sku || '—'}</span>
                    </span>
                    <span className="shrink-0 text-xs font-semibold text-primary">{ageLabel(product, t)}</span>
                  </span>
                </ProductEditLink>
              ))}
            </div>
          ...
          <section aria-labelledby="partially-out-heading">
            <div className="mb-3">
              <h3 id="partially-out-heading" className="m-0 text-sm font-bold uppercase text-foreground">
                {t('notifications.partiallyOutTitle')}
              </h3>
              <p className="mb-0 mt-1 text-xs text-muted-foreground">{t('notifications.partiallyOutDescription')}</p>
            </div>
            <div className="space-y-3">
              {partial.map((product) => {
                const variants = Array.isArray(product.unavailableVariants) ? product.unavailableVariants : []
                return (
                  <div key={product.productId} className="overflow-hidden rounded-[var(--admin-radius-card)] border border-border bg-card">
                    <ProductEditLink item={product} language={language} navigate={navigate} onClose={close}>
                        ...
                        >
                          <span className="flex flex-col gap-1 px-4 py-3 pl-7 hover:bg-surface-hover sm:flex-row sm:items-center sm:justify-between">
                            <span className="text-sm text-foreground">
                              ...
                              <span className="ml-2 text-xs text-muted-foreground">{t('notifications.sku')}: {variant.sku || '—'}</span>
                            </span>
                            <span className="shrink-0 text-xs font-medium text-muted-foreground">{ageLabel(variant, t)}</span>
                          </span>
                        </ProductEditLink>
                      ))}
                    </div>
...
export function NotificationBell({ navigate }) {
  ...
  const scopes = [canViewOrders ? 'orders' : null, canViewInventory ? 'inventory' : null]
  ...
  useEffect(() => {
    ...
    if (canViewInventory) {
      unsubscribers.push(subscribeAdminWs('/topic/admin/inventory', (event) => {
  ...
  return (
    <>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block text-xs text-muted-foreground">
                      {!isOverdueDigest && !isInventoryDigest && item.total ? `${formatCurrencyVnd(item.total)} · ` : ''}{formatWhen(item.at, i18n.language)}
                    </span>
                  </span>
                </DropdownMenuItem>
              )
      ...
      <InventoryDigestDialog
        ...
        navigate={navigate}
        t={t}
        onOpenChange={(next) => { if (!next) setSelectedDigest(null) }}
      />
    </>
  )
}
