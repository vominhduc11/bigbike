...
function PickerImageThumbnail({ src, alt }) {
  ...
  if (!src || failed) {
    return (
      <div
        className="mpicker-thumb mpicker-thumb-placeholder"
        role="img"
        aria-label={failed ? t('media.mediaLoadError') : t('media.missingPublicUrl')}
      >
...
export function MediaPickerModal({ onSelect, onClose, recommend, kind = 'image' }) {
  ...
  function brandLogoFailureMessage(failure, fallback) {
    ...
    if (
      failure?.message === 'MEDIA_UNAVAILABLE' ||
      failure?.message === 'BRAND_LOGO_MEDIA_UNAVAILABLE'
    ) {
  ...
  async function uploadFiles(files) {
    ...
    for (const file of files) {
      ...
      if (!pickerMimeTypes.includes(fileType)) {
        validationErrors.push(
          t(kind === 'image' ? 'media.unsupportedImageType' : 'media.unsupportedType', {
      ...
      if (isBrandLogo) {
        try {
          ...
          if (blockingIssues.length) {
            validationErrors.push(
              blockingIssues.map((issue) => brandLogoErrorMessage(issue, details)).join(' '),
            )
            continue
          }
          ...
          if (warningIssues.length) {
            validationWarnings.push(
              warningIssues.map((issue) => brandLogoErrorMessage(issue, details)).join(' '),
            )
          }
        ...
        if (reasons?.includes('wrongRatio')) {
          validationErrors.push(
            t('mediaReco.categoryImageWrongRatio', {
              w: dimensions.width,
              h: dimensions.height,
            }),
          )
    ...
    for (const entry of valid) {
      ...
      setUploadQueue((q) => [
        ...q,
        { id: itemId, name: file.name, progress: 0, error: null, status: 'pending' },
      ])
      try {
        const result = await uploadMedia(
          ...
          (pct) => {
            setUploadQueue((q) =>
              q.map((item) => (item.id === itemId ? { ...item, progress: pct } : item)),
            )
          },
          targetFolderId,
          targetClearFolder,
        ...
        if (url) {
          ...
          setUploadQueue((q) =>
            q.map((item) =>
              item.id === itemId ? { ...item, progress: 100, status: 'done' } : item,
            ),
          )
        } else {
          setUploadQueue((q) =>
        ...
        setUploadQueue((q) =>
          q.map((item) =>
            item.id === itemId
              ? {
                  ...item,
                  error: uploadFailure?.message || t('media.picker.uploadFailed'),
                  status: 'error',
                }
              : item,
          ),
    ...
    if (canRead) {
      ...
      try {
        ...
        setState({
          status: 'success',
          items,
          totalPages: Math.max(1, Number(r.pagination?.totalPages) || 1),
          error: '',
          refreshError: '',
        })
      } catch {
        /* effect bên dưới (refreshKey bump) sẽ tự thử lại */
      }
    }
    setRefreshKey((k) => k + 1)
    ...
    setTimeout(
      () => setUploadQueue((queue) => queue.filter((item) => item.status !== 'done')),
      3000,
    )
  ...
  async function toggleMedia(media) {
    ...
    try {
      ...
      setUploadError(
        brandLogoFailureMessage(selectionError, t('brands.logo.errors.mediaUnavailable')),
      )
    } finally {
  ...
  function handleConfirm() {
    ...
    if (selectedUrl && exactRatioReady && !validation.blocked) {
      ...
      const selectedMedia =
        recommend?.exactRatio && validation.status === 'loaded'
          ? {
              ...(cachedMedia || {}),
  ...
  const exactRatioReady = !recommend?.exactRatio || validation.status === 'loaded'
  const canConfirm =
    hasSelection && !isRefreshing && !validatingSelection && exactRatioReady && !validation.blocked
  ...
  async function attemptClose() {
    ...
    if (hasSelection) {
      const ok = await showConfirm(
        t('media.picker.closeConfirm', {
          defaultValue: 'Bạn đang có tệp đã chọn. Đóng sẽ bỏ lựa chọn này. Tiếp tục?',
        }),
    ...
    return createPortal(
      <>
        ...
        <div
          ref={modalRef}
          tabIndex={-1}
          className="mpicker-modal"
          role="dialog"
          aria-modal="true"
          aria-label={t('media.permissionDeniedTitle')}
        >
          ...
          <div className="p-4 text-sm text-muted-foreground">{t('media.permissionDeniedDesc')}</div>
        </div>
      </>,
      document.body,
    )
      ...
      {createPortal(
        <>
          ...
          >
            ...
            {uploadQueue.length > 0 && (
              <div className="mpicker-upload-queue">
                {uploadQueue.map((item) => (
                  <div
                    key={item.id}
                    className={`mpicker-upload-item${item.error ? ' mpicker-upload-item--error' : ''}`}
                  >
                    ...
                    {item.error ? (
                      <>
                        ...
                        <Button
                          ...
                          className="h-7 w-7"
                          onClick={() =>
                            setUploadQueue((queue) => queue.filter((entry) => entry.id !== item.id))
                          }
                      ...
                      <progress
                        ...
                        value={item.progress}
                        max="100"
                        aria-label={t('media.uploadProgress', { name: item.name })}
                      />
                    )}
                  </div>
                ))}
            ...
            <div className="mpicker-search">
              ...
              {filtersError && (
                <p className="mt-1 text-xs text-danger">
                  {t('media.picker.filtersError', {
                    defaultValue: 'Không tải được danh sách thư mục hoặc thẻ để lọc.',
                  })}
                </p>
              )}
            </div>
            ...
            {uploadError && (
              <div className="mpicker-upload-error">
                ...
                <Button
                  ...
                  aria-label={t('media.picker.dismissError')}
                >
                  <X size={18} strokeWidth={2.2} aria-hidden="true" />
                </Button>
              </div>
            ...
            {uploadWarning && (
              <div
                className="mx-4 mb-2 flex items-center justify-between gap-2 rounded-sm border border-warning bg-warning-bg px-3 py-2 text-sm text-warning"
                role="status"
              >
                <span>{uploadWarning}</span>
                <Button
                  variant="unstyled"
                  onClick={() => setUploadWarning('')}
                  aria-label={t('media.picker.dismissError')}
                >
                  <X size={18} strokeWidth={2.2} aria-hidden="true" />
                </Button>
            ...
            <div className="mpicker-body">
              {isLoading && <div className="mpicker-state">{t('common.loading')}</div>}
              {state.status === 'error' && (
              ...
              {state.refreshError && (
                <div
                  className="mx-4 mb-2 flex items-center justify-between gap-2 rounded-sm bg-danger-bg px-3 py-2 text-sm text-danger"
                  role="alert"
                >
                          ...
                          <div className="mpicker-item-info">
                            <span className="mpicker-item-name">
                              {(
                                ...
                                t('media.picker.defaultItemName')
                              ).replace(/\.[^.]+$/, '')}
                            </span>
                            {media.fileSize ? (
                              <span className="mpicker-item-size">
            ...
            {state.totalPages > 1 && (
              <div className="mpicker-pagination">
                ...
                <span className="mpicker-page-info">
                  {t('media.picker.pageInfo', { page, totalPages: state.totalPages })}
                </span>
                <Button
                  variant="secondary"
                  size="sm"
