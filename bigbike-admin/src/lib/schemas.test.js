...
describe('MEDIA_RULE_004 — writable video sources', () => {
  ...
  it('article video blocks reject TikTok/Facebook but accept YouTube/upload', () => {
    ...
    for (const block of [
      ...
      expect(pathsOf(schema.safeParse({ ...base, bodyBlocks: [block] }))).toContain(
        'bodyBlocks.0.url',
      )
    }
  })
  ...
  it('accepts a blank SEO block and validates media separately by role', () => {
    ...
    const valid = schema.safeParse(
      categoryForm({
        ...
        mobileBannerImageUrl: 'https://cdn.example.test/banner-mobile.jpg',
        mobileBannerImageAlt: 'Banner mobile',
