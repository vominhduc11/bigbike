...
describe('admin chat read contract', () => {
  ...
  it('keeps the detail read-only and safely parses product cards from stored JSON', async () => {
    ...
    expect(result.item.messages[1]).toMatchObject({
      source: 'AI',
      answerFormat: 'MARKDOWN',
      resultKind: 'PRODUCT_RESULTS',
    })
    expect(result.item.orderAttributions).toEqual([
      expect.objectContaining({ orderId: 'order-1', attributedAmount: 1590000 }),
    ])
