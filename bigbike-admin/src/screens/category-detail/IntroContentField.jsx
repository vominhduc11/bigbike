...
export function IntroContentField({ value, onChange, disabled, lang = 'vi', getAiPrompt }) {
  ...
  const updateFaq = (i, field, v) =>
    commitField(
      'faqs',
      model.faqs.map((f, idx) => (idx === i ? { ...f, [field]: v } : f)),
    )
  // Xoá câu hỏi là mất nội dung đã nhập — xác nhận trước (không có Hoàn tác).
  const removeFaq = async (i) => {
    const ok = await showConfirm(
      t('categories.detail.introFaqRemoveConfirm', {
        defaultValue:
          'Xoá câu hỏi này? Nội dung câu hỏi và câu trả lời sẽ bị xoá khỏi phần giới thiệu.',
      }),
  ...
  const removeBrand = (i) =>
    commitField(
      'brands',
      model.brands.filter((_, idx) => idx !== i),
    )

  const sectionClass =
    'rounded-[var(--admin-radius-sm)] border border-input p-3 flex flex-col gap-3'
        ...
        <TabsContent value="structured">
          <div className="flex flex-col gap-4">
            ...
            <CollapsibleSection
              ...
              hint={
                model.faqs.length > 0
                  ? t('categories.detail.introFaqCount', {
                      count: model.faqs.length,
                      defaultValue: '{{count}} câu hỏi',
                    })
                  : undefined
              }
              open={faqOpen}
              ...
              <fieldset className="flex flex-col gap-3 pt-1" disabled={disabled}>
                {model.faqs.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    {t('categories.detail.introFaqEmpty')}
                  </p>
                )}
                {model.faqs.map((faq, i) => (
                  <div
                    key={faq._key}
                    className="flex flex-col gap-2 rounded-[var(--admin-radius-xs)] border border-input p-2"
                  >
          ...
          <div className="flex flex-col gap-1">
            ...
            {html.trim() ? (
              ...
              <p className="text-sm text-muted-foreground">
                {t('categories.detail.introAdvancedPreviewEmpty')}
              </p>
            )}
          </div>
        </TabsContent>
