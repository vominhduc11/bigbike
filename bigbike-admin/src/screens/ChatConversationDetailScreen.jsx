                        ...
                        {!isUser ? (
                          <div className="mt-3 border-t border-border pt-2 text-xs text-muted-foreground">
                            <p>
                              ...
                              {message.aiCalled ? ` · ${t('chatAdmin.detail.usedAi')}` : ''}
                            </p>
        ...
        <aside className="grid content-start gap-4">
          <SectionCard title={t('chatAdmin.detail.summary')} headingLevel={2}>
            <dl className="grid gap-4">
              ...
              <DetailValue label={t('chatAdmin.columns.aiCalls')}>
                {conversation.aiCallCount}
              </DetailValue>
              <DetailValue label={t('chatAdmin.detail.tokens')}>
                {conversation.hasTelemetry
                  ? formatNumber(
                      (conversation.inputTokens ?? 0) +
                        (conversation.outputTokens ?? 0) +
                        (conversation.thinkingTokens ?? 0),
                    )
                  : '—'}
              </DetailValue>
              <DetailValue label={t('chatAdmin.detail.providerRequests')}>
                {conversation.hasTelemetry ? formatNumber(conversation.providerRequests) : '—'}
              </DetailValue>
              <DetailValue label={t('chatAdmin.columns.latency')}>
                {conversation.hasTelemetry ? formatLatency(conversation.averageLatencyMs) : '—'}
              </DetailValue>
              <DetailValue label={t('chatAdmin.columns.cost')}>
                {conversation.hasTelemetry ? formatUsd(conversation.estimatedCostUsd) : '—'}
              </DetailValue>
              <DetailValue label={t('chatAdmin.stats.contentRefusals')}>
                {formatNumber(conversation.contentRefusals)}
              </DetailValue>
              <DetailValue label={t('chatAdmin.stats.assistedOrders')}>
                {formatNumber(conversation.assistedOrders)}
              </DetailValue>
              <DetailValue label={t('chatAdmin.columns.assistedRevenue')}>
                {formatVnd(conversation.assistedRevenue)}
              </DetailValue>
            ...
            {conversation.orderAttributions.length > 0 ? (
              <ul className="grid gap-3">
                {conversation.orderAttributions.map((attribution) => (
                  <li
                    key={
                      attribution.orderLineItemId ||
                      `${attribution.orderId}-${attribution.createdAt}`
                    }
                    className="rounded-[var(--admin-radius-card)] border border-border bg-surface-muted p-3"
                  >
                    <Button
                      type="button"
                      variant="link"
                      className="h-auto p-0 font-mono text-sm"
                      onClick={() => navigate(`/admin/orders/${attribution.orderId}`)}
                    >
                      {t('chatAdmin.detail.openOrder', { id: attribution.orderId.slice(0, 8) })}
                    </Button>
                    <p className="mt-1 text-sm font-semibold text-foreground">
                      {formatVnd(attribution.attributedAmount)}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      {formatDateTime(attribution.createdAt)}
                    </p>
                  </li>
                ))}
              </ul>
              ...
              <p className="text-sm text-muted-foreground">
                {t('chatAdmin.detail.noAssistedOrders')}
              </p>
            )}
          </SectionCard>
          ...
          <SectionCard title={t('chatAdmin.detail.lead')} headingLevel={2}>
            {conversation.lead ? (
              <dl className="grid gap-4">
                <DetailValue label={t('chatAdmin.detail.leadName')}>
                  {conversation.lead.name}
                </DetailValue>
                <DetailValue label={t('chatAdmin.detail.phone')}>
                  {conversation.lead.phone}
                </DetailValue>
                <DetailValue label={t('chatAdmin.detail.note')}>
                  {conversation.lead.note || t('chatAdmin.detail.none')}
                </DetailValue>
                <DetailValue label={t('chatAdmin.detail.contactSource')}>
                  {leadSourceLabel(conversation.lead.source, t)}
                </DetailValue>
                <DetailValue label={t('chatAdmin.detail.consentedAt')}>
                  {formatDateTime(conversation.lead.consentedAt)}
                </DetailValue>
              </dl>
            ) : (
