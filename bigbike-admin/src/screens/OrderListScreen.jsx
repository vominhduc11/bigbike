...
export function OrderListScreen({ navigate, canUpdate }) {
  ...
  const filterChips = [
    ...
    query.orderScope !== INITIAL_QUERY.orderScope
      ? {
          ...
          onRemove: () =>
            updateQuery({ orderScope: INITIAL_QUERY.orderScope }, { resetPage: true }),
        }
      : null,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (order) => {
        const inlineTargets =
          canUpdate && order.orderScope !== 'HISTORICAL'
  ...
  return (
    <Screen>
      ...
      <Alert
        tone={query.orderScope === 'HISTORICAL' ? 'warning' : 'info'}
        size="sm"
        className="mb-4"
        role="status"
      >
