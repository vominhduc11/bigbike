...
function authorKey(review) {
  return String(review.authorName || review.id || 'unknown')
    .trim()
    .toLowerCase()
}
...
function AuthorIdentity({ review, t }) {
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      <span
        className={`inline-flex size-9 shrink-0 items-center justify-center rounded-full text-xs font-bold ${color}`}
        aria-hidden="true"
      >
        {initials(name)}
      </span>
...
function PhotoThumb({ review, t }) {
  ...
  if (!url || failed) {
    return (
      <span
        className="inline-flex size-10 shrink-0 items-center justify-center rounded-xs bg-surface-muted text-muted-foreground"
        aria-label={t('reviews.photoUnavailable')}
      >
        <ImageIcon size={16} />
      </span>
    )
  ...
  return (
    <img
      ...
      alt={t('reviews.photoAlt', {
        count: 1,
        author: formatText(review.authorName, t('reviews.unknownAuthor')),
      })}
      loading="lazy"
...
function ProductLink({ review, contentLang, navigate, t }) {
  const name = formatText(
    contentLang === 'en' ? review.productNameEn || review.productName : review.productName,
    t('reviews.unknownProduct'),
  )
...
export function ReviewListScreen({ navigate, canUpdate, isSuperAdmin = false }) {
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem('reviews:listQuery', window.location.search)
    } catch {
  ...
  useEffect(() => {
    if (state.status !== 'success' || state.isFetching || Object.keys(staleVersions).length === 0)
      return
    const timer = window.setTimeout(() => {
      setStaleVersions((current) => {
        const next = Object.fromEntries(
          Object.entries(current).filter(([id, staleVersion]) => {
  ...
  const confirmStatusChange = useCallback(
    async (nextStatus, count) => {
      ...
      return showConfirm(confirmation.message, confirmation.title, {
  ...
  const confirmDelete = useCallback(
    async (count) =>
      showConfirm(
        t('reviews.deleteConfirmPermanent', {
          count,
          defaultValue: `Xoá vĩnh viễn ${count} đánh giá. Không thể hoàn tác; đánh giá và ảnh đính kèm sẽ bị xoá.`,
        }),
  ...
  const handleDelete = useCallback(
    async (review) => {
      if (
        pendingId ||
        bulkBusy ||
        staleVersions[review.id] !== undefined ||
        !canPermanentlyDeleteReview(review, isSuperAdmin) ||
  ...
  const runBulk = useCallback(
    async (kind) => {
      ...
      if (kind === 'DELETE') {
        if (
          !isSuperAdmin ||
          !selectedReviews.some((review) => review.status === 'TRASH') ||
          !(await confirmDelete(versionedItems.length))
        )
          return
      } else {
        if (
          !hasReviewStatusTarget(selectedReviews, kind) ||
          !(await confirmStatusChange(kind, versionedItems.length))
        )
          return
      }
      setActionError('')
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        skeletonWidth: '22%',
        render: (review) => (
          <ProductLink review={review} contentLang={contentLang} navigate={navigate} t={t} />
        ),
      },
      {
        key: 'rating',
      ...
      },
      {
        ...
        skeletonWidth: '28%',
        render: (review) => (
          <p className="m-0 max-w-sm truncate text-sm text-muted-foreground">
            {formatText(review.body, t('reviews.contentMissing'))}
          </p>
        ),
      },
      {
        ...
        render: (review) =>
          review.photos?.length ? (
            <div className="flex items-center gap-2">
              <PhotoThumb review={review} t={t} />
              <span className="text-xs text-muted-foreground">{review.photos.length}</span>
            </div>
          ) : (
            <span className="text-muted-foreground">—</span>
          ),
            ...
            <div className="flex flex-wrap items-center gap-1">
              ...
              {canUpdate && targets.includes('SPAM') ? (
                <Button
                  ...
                  disabled={busy}
                  loading={pendingId === review.id}
                  onClick={() => handleStatusChange(review, 'SPAM')}
                >
              ...
              {canUpdate && targets.includes('TRASH') ? (
                <Button
                  ...
                  disabled={busy}
                  loading={pendingId === review.id}
                  onClick={() => handleStatusChange(review, 'TRASH')}
                >
                  <Trash2 size={16} aria-hidden="true" />
                  {t('reviews.moveToTrash')}
                </Button>
              ) : null}
              {canUpdate && targets.includes('PENDING') ? (
                ...
                >
                  ...
                  {review.status === 'TRASH' ? t('reviews.restore') : t('reviews.returnPending')}
                </Button>
              ) : null}
              {canUpdate && canPermanentlyDeleteReview(review, isSuperAdmin) ? (
                <Button
                  ...
                  aria-label={t('reviews.deletePermanent')}
                >
                  <Trash2 size={16} aria-hidden="true" />
                </Button>
              ) : null}
    ...
    (review) => {
      ...
      return {
        ...
        meta: [
          ...
          {
            label: t('reviews.colPhotos'),
            value: review.photos?.length ? `${review.photos.length}` : '—',
          },
          {
            label: t('reviews.colContent'),
            value: formatText(review.body, t('reviews.contentMissing')),
          },
        ],
        actions: (
          <div className="flex flex-wrap gap-2">
            ...
            {canUpdate && targets.includes('SPAM') ? (
              <Button
                type="button"
                variant="ghost"
                className="min-h-11 flex-1 text-danger"
                disabled={busy}
                loading={pendingId === review.id}
                onClick={() => handleStatusChange(review, 'SPAM')}
              >
  ...
  return (
    <Screen>
      ...
      {state.status === 'error' && items.length > 0 ? (
        <Alert tone="danger">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span>{state.error || t('reviews.error')}</span>
            <Button type="button" variant="ghost" className="min-h-11" onClick={state.refetch}>
              {t('common.retry')}
            </Button>
          </div>
        </Alert>
      ...
      <RecentItemsChips
        ...
        onSelect={(item) => navigate(`/admin/reviews/${item.id}`)}
      />

      <div className="mb-6 grid gap-4 lg:grid-cols-3">
        <section
          className="rounded-md border border-border bg-surface p-5 lg:col-span-2"
          aria-labelledby="review-public-score"
        >
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 id="review-public-score" className="font-display text-lg font-semibold">
                {t('reviews.publicScoreTitle')}
              </h2>
              <p className="m-0 text-sm text-muted-foreground">{t('reviews.publicScoreHint')}</p>
            </div>
          ...
          {summaryLoading ? (
            <div
              className="h-24 animate-pulse rounded-sm bg-surface-muted"
              role="status"
              aria-label={t('reviews.loading')}
            />
          ) : summaryQuery.isError ? (
            <p className="m-0 text-sm text-danger">
              {summaryQuery.error?.message || t('reviews.summaryError')}
            </p>
          ) : (
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="min-w-32 text-center">
                <div className="font-display text-4xl font-semibold text-primary">
                  {Number(summary?.approved?.averageRating || 0).toFixed(1)}
                </div>
                <ReviewStars
                  rating={summary?.approved?.averageRating}
                  label={t('reviews.publicScore')}
                />
                <p className="mt-2 mb-0 text-sm text-muted-foreground">
                  {t('reviews.publicReviewCount', { count: summary?.approved?.totalReviews || 0 })}
                </p>
              </div>
              <div className="grid grid-cols-3 gap-x-6 gap-y-2 sm:grid-cols-5">
                {RATING_BREAKDOWN_LEVELS.map((star) => (
                  <div key={star} className="flex flex-col gap-1 text-sm">
                    <span className="font-semibold">{star} ★</span>
                    <span className="text-muted-foreground">
                      {Number(ratingBreakdown[String(star)] || 0).toLocaleString()}
                    </span>
                  </div>
                ))}
              </div>
        ...
        >
          <h2 id="review-queue" className="mb-1 font-display text-lg font-semibold">
            {t('reviews.needsAction')}
          </h2>
          <p className="mb-4 text-sm text-muted-foreground">{t('reviews.queueHint')}</p>
          {summaryLoading ? (
            <div
              className="h-20 animate-pulse rounded-sm bg-surface-muted"
              role="status"
              aria-label={t('reviews.loading')}
            />
          ) : summaryQuery.isError ? (
            ...
            <div className="grid gap-2">
              <Button
                type="button"
                variant="outline"
                className="flex min-h-11 items-center justify-between gap-3 px-3 text-left"
                onClick={() => updateQuery({ status: 'PENDING', rating: '' })}
              >
                <span className="flex items-center gap-2">
                  <MessageSquare size={16} />
                  {t('reviews.pendingCount', { count: summary?.pending?.totalReviews || 0 })}
                </span>
                <span className="font-semibold text-warning">
                  {summary?.pending?.totalReviews || 0}
                </span>
              </Button>
              <Button
                type="button"
                variant="outline"
                className="flex min-h-11 items-center justify-between gap-3 border-danger-border px-3 text-left"
                onClick={() => updateQuery({ status: 'PENDING', rating: '1' })}
              >
                <span className="flex items-center gap-2">
                  <MessageSquare size={16} />
                  {t('reviews.lowRatingPending', { count: summary?.pending?.oneStarReviews || 0 })}
                </span>
                <span className="font-semibold text-danger">
                  {summary?.pending?.oneStarReviews || 0}
                </span>
              </Button>
            </div>
          )}
      ...
      <FilterBar ariaLabel={t('reviews.filterStatus')} className="mb-4">
        <div className="min-w-56 flex-1">
          ...
          >
            {t('reviews.searchLabel')}
          </label>
          <FilterSearchInput
            value={searchInput}
            onChange={setSearchInput}
            placeholder={t('reviews.searchPlaceholder')}
            ariaLabel={t('reviews.searchLabel')}
          />
        </div>
        <label className="grid gap-1 text-xs font-semibold text-muted-foreground">
          ...
          <FilterSelect
            ...
            options={STATUS_OPTIONS.map((status) => ({
              value: status,
              label: status === 'ALL' ? t('common.all') : statusLabel(status, t),
            }))}
          />
        </label>
        <label className="grid gap-1 text-xs font-semibold text-muted-foreground">
          ...
          <FilterSelect
            value={query.rating}
            onValueChange={(rating) => updateQuery({ rating })}
            ariaLabel={t('reviews.filterRating')}
            options={RATING_OPTIONS.map((rating) => ({
              value: rating,
              label: rating ? t('reviews.ratingOption', { rating }) : t('reviews.allRatings'),
            }))}
          />
        </label>
        <PageSizeSelect
        ...
        />
        {isFiltered ? (
          <Button type="button" variant="ghost" className="min-h-11" onClick={resetFilters}>
          ...
          actions={[
            {
              ...
              disabled: bulkBusy || !hasReviewStatusTarget(selectedReviews, 'APPROVED'),
              loading: bulkBusy,
            },
            {
              label: t('reviews.spam'),
              onClick: () => runBulk('SPAM'),
              disabled: bulkBusy || !hasReviewStatusTarget(selectedReviews, 'SPAM'),
              loading: bulkBusy,
            },
            {
              label: t('reviews.moveToTrash'),
              ...
              disabled: bulkBusy || !hasReviewStatusTarget(selectedReviews, 'TRASH'),
              loading: bulkBusy,
            },
            {
              label: t('reviews.returnPendingBulk'),
              onClick: () => runBulk('PENDING'),
              disabled: bulkBusy || !hasReviewStatusTarget(selectedReviews, 'PENDING'),
              loading: bulkBusy,
            },
            isSuperAdmin
              ? {
                  ...
                  disabled:
                    bulkBusy || !selectedReviews.some((review) => review.status === 'TRASH'),
                  loading: bulkBusy,
                }
              : null,
          ].filter(Boolean)}
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <p className="m-0 text-sm text-muted-foreground" aria-live="polite">
          {t('reviews.filteredResults', { count: state.pagination?.totalItems || 0 })}
        </p>
      ...
      {state.status !== 'error' && state.status === 'success' && items.length === 0 ? (
        <StatePanel
          tone="neutral"
          title={isFiltered ? t('reviews.empty') : t('reviews.emptyAll')}
          description={isFiltered ? t('reviews.emptyDesc') : t('reviews.emptyAllDesc')}
          actionLabel={isFiltered ? t('common.resetFilters') : undefined}
          onAction={isFiltered ? resetFilters : undefined}
        />
      ) : null}
      {state.status !== 'error' && (items.length > 0 || listLoading) ? (
        <AdminTable
          columns={columns}
          rows={items}
          loading={listLoading}
