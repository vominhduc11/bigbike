...
function RoleBadge({ role, label }) {
  return (
    <span className={`bb-badge ${ROLE_BADGE[role] || 'bb-badge-neutral'}`}>{label || '—'}</span>
  )
}
...
export function AdminUsersScreen({
  ...
  function openEdit(user) {
    ...
    if (savedForm)
      toast.info(
        t('adminUsers.draftRestored', {
          defaultValue: 'Đã khôi phục bản nháp tài khoản quản trị.',
        }),
      )
  }
    ...
    if (sensitiveChangeCount > 1) {
      ...
      const ok = await showConfirm(
        t('adminUsers.confirmPasswordChange', {
          defaultValue: 'Bạn có chắc muốn đổi mật khẩu tài khoản này không?',
        }),
        t('adminUsers.confirmSensitiveTitle'),
        {
          variant: 'danger',
  ...
  if (statusFilter) {
    ...
    activeFilterChips.push({
      key: 'status',
      label: t('adminUsers.chipStatus', {
        value: statusLabel,
        defaultValue: `Trạng thái: ${statusLabel}`,
      }),
              ...
              <div className="inline-flex items-center justify-end gap-1">
                {u.status === 'INVITED' && (
                  <Button
                    ...
                    type="button"
                    className="bb-icon-btn"
                    title={t('adminUsers.resendInvite')}
    ...
    <div>
        ...
        {canUpdate && (
          <div className="bb-screen-actions">
            <Button
              ...
              title={
                !canChangeRoles
                  ? 'Cần quyền roles.read và danh sách vai trò để chọn vai trò cho tài khoản mới.'
                  : undefined
              }
            >
              <UserPlus size={14} />
      ...
      {canUpdate && !canReadRoles ? (
        <Alert tone="warning" icon={Lock}>
          Cần quyền roles.read để lọc theo vai trò, mời tài khoản mới hoặc thay đổi vai trò. Các
          thao tác tên, mật khẩu và trạng thái vẫn khả dụng.
        </Alert>
      ) : null}
