...
export function ChatConversationListScreen({ navigate }) {
  ...
  const state = useAdminList(
    ['chat-conversations', query, range],
  ...
  const columns = useMemo(() => [
    ...
    { key: 'turnCount', label: t('chatAdmin.columns.turns'), align: 'right' },
    {
      ...
      render: (item) => item.lastResultKind
        ? t(`chatAdmin.resultKind.${item.lastResultKind}`, { defaultValue: item.lastResultKind })
        : '—',
    },
    {
      key: 'lastMessageAt',
  ...
  const { visibleColumns, hiddenKeys, toggle: toggleColumn, allColumns } = useColumnVisibility(
    columns,
    'columns:chat',
  )
  ...
  return (
    <Screen>
      <ScreenHeader
        ...
        actions={<Button variant="secondary" onClick={() => navigate('/admin/settings?group=AI_ASSISTANT')}>
