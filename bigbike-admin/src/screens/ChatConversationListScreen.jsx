...
function SummaryCard({ icon, label, value, detail }) {
  return (
    <article className="flex min-h-28 items-start gap-4 rounded-md border border-border bg-surface p-4">
      <span
        className="inline-flex size-11 shrink-0 items-center justify-center rounded-sm bg-primary/10 text-primary"
        aria-hidden="true"
      >
        {icon}
      </span>
    ...
    () => [
      {
        ...
        render: (item) =>
          t(`chatAdmin.actions.types.${item.actionType}`, { defaultValue: item.actionType }),
      },
      ...
      {
        key: 'revenue',
        label: t('chatAdmin.actions.revenue'),
        align: 'right',
        render: (item) => formatVnd(item.revenue),
      },
      {
        key: 'conversionRate',
        label: t('chatAdmin.actions.conversion'),
        align: 'right',
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (item) => (
          <span className="whitespace-nowrap">
            {item.hasTelemetry ? formatLatency(item.averageLatencyMs) : '—'}
          </span>
        ),
        align: 'right',
      },
      {
        ...
        render: (item) => (
          <span className="whitespace-nowrap">
            {item.hasTelemetry ? formatUsd(item.estimatedCostUsd) : '—'}
          </span>
        ),
        align: 'right',
      },
      {
        ...
        render: (item) => (
          <span className="whitespace-nowrap">{formatVnd(item.assistedRevenue)}</span>
        ),
        align: 'right',
      },
      {
  ...
  return (
    <Screen>
      ...
      {stats?.monthlyCostWarningExceeded && stats.monthlyCostWarningUsd > 0 ? (
        <div
          className="mb-4 flex items-start gap-3 rounded-md border border-warning bg-warning-bg p-4 text-warning"
          role="alert"
        >
          ...
          <div>
            ...
            <p className="mb-0 mt-1 text-sm">
              {t('chatAdmin.monthlyWarning.description', {
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <SummaryCard
          ...
          label={t('chatAdmin.stats.aiCalls')}
          value={stats?.aiCalls ?? '—'}
          detail={stats ? t('chatAdmin.stats.limit', { count: stats.dailyLimit }) : ''}
        />
        <SummaryCard
          icon={<Bot size={20} />}
          label={t('chatAdmin.stats.remaining')}
          value={stats?.remainingAiCalls ?? '—'}
        />
        <SummaryCard
          icon={<MessageCircle size={20} />}
          label={t('chatAdmin.stats.conversations')}
          value={stats?.conversations ?? '—'}
        />
        <SummaryCard
          icon={<PhoneCall size={20} />}
          label={t('chatAdmin.stats.leads')}
          value={stats?.leads ?? '—'}
        />
        <SummaryCard
          icon={<CircleAlert size={20} />}
          label={t('chatAdmin.stats.unanswered')}
          value={stats?.unanswered ?? '—'}
        />
        <SummaryCard
          icon={<Clock3 size={20} />}
          label={t('chatAdmin.stats.averageLatency')}
          value={stats?.hasTelemetry ? formatLatency(stats.averageLatencyMs) : '—'}
        />
        <SummaryCard
          icon={<Bot size={20} />}
          ...
          detail={
            stats?.hasTelemetry
              ? t('chatAdmin.stats.providerRequests', { count: stats.providerRequests ?? 0 })
              : t('chatAdmin.stats.noTelemetry')
          }
        />
        <SummaryCard
          icon={<BadgeDollarSign size={20} />}
          label={t('chatAdmin.stats.estimatedCost')}
          value={stats?.hasTelemetry ? formatUsd(stats.estimatedCostUsd) : '—'}
          detail={
            stats?.hasTelemetry
              ? t('chatAdmin.stats.estimateNotice')
              : t('chatAdmin.stats.noTelemetry')
          }
        />
        <SummaryCard
          icon={<ShieldAlert size={20} />}
          label={t('chatAdmin.stats.contentRefusals')}
          value={formatNumber(stats?.contentRefusals)}
        />
        <SummaryCard
          icon={<ReceiptText size={20} />}
          label={t('chatAdmin.stats.assistedOrders')}
          value={formatNumber(stats?.assistedOrders)}
          detail={stats ? formatVnd(stats.assistedRevenue) : ''}
        />
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        ...
        >
          <SummaryCard
            ...
            value={
              stats
                ? formatNumber((stats.quality?.answers ?? 0) + (stats.quality?.productResults ?? 0))
                : '—'
            }
            detail={
              stats
                ? t('chatAdmin.quality.productResults', {
                    count: stats.quality?.productResults ?? 0,
                  })
                : ''
            }
          />
          <SummaryCard
            ...
            value={stats ? formatNumber(stats.quality?.clarifications) : '—'}
          />
          <SummaryCard
            icon={<ShieldAlert size={20} />}
            label={t('chatAdmin.quality.outOfScope')}
            value={stats ? formatNumber(stats.quality?.outOfScope) : '—'}
          />
          <SummaryCard
            icon={<ShieldAlert size={20} />}
            label={t('chatAdmin.quality.refusals')}
            value={stats ? formatNumber(stats.quality?.contentRefusals) : '—'}
          />
        </DetailSection>
        ...
        >
          <SummaryCard
            icon={<PhoneCall size={20} />}
            label={t('chatAdmin.leadFunnel.sequence1')}
            value={stats ? formatNumber(stats.leadFunnel?.sequence1Viewed) : '—'}
          />
          <SummaryCard
            icon={<PhoneCall size={20} />}
            label={t('chatAdmin.leadFunnel.sequence2')}
            value={stats ? formatNumber(stats.leadFunnel?.sequence2Viewed) : '—'}
          />
          <SummaryCard
            icon={<PhoneCall size={20} />}
            label={t('chatAdmin.leadFunnel.accepted')}
            value={stats ? formatNumber(stats.leadFunnel?.accepted) : '—'}
          />
          <SummaryCard
            icon={<CircleAlert size={20} />}
            label={t('chatAdmin.leadFunnel.declined')}
            value={stats ? formatNumber(stats.leadFunnel?.declined) : '—'}
          />
        </DetailSection>
      </div>
      ...
      >
        <AdminTable
          ...
          pageSize={5}
          mobileCard={(item) => ({
            title: t(`chatAdmin.actions.types.${item.actionType}`, {
        ...
        {!statsQuery.isLoading && !statsQuery.isError && (stats?.actionStats?.length ?? 0) === 0 ? (
          <StatePanel
            tone="neutral"
            title={t('chatAdmin.actions.empty')}
            description={t('chatAdmin.actions.emptyDescription')}
          />
        ) : null}
      </DetailSection>
          ...
          <AdminTable
            ...
            mobileCard={(item) => ({
              ...
              meta: [
                ...
                {
                  label: t('chatAdmin.columns.cost'),
                  value: item.hasTelemetry ? formatUsd(item.estimatedCostUsd) : '—',
                },
                {
                  label: t('chatAdmin.columns.assistedRevenue'),
                  value: formatVnd(item.assistedRevenue),
                },
              ],
