...
function AbbreviationEditor({ value, onChange, readOnly }) {
  ...
  const remove = (index) =>
    onChange(writeItems(items.filter((_, itemIndex) => itemIndex !== index)))
  const add = () =>
    onChange(writeItems([...items, { locale: 'vi', phrase: '', expansion: '', enabled: true }]))
  ...
  return (
    <div className="space-y-3">
      ...
      {items.length === 0 ? (
        <EmptyEditor>{t('settings.assistantConfig.abbreviationsEmpty')}</EmptyEditor>
      ) : null}

      {items.map((item, index) => (
        <div
          key={index}
          className="grid gap-3 rounded-md border border-border bg-surface-muted p-3 lg:grid-cols-[8rem_1fr_1fr_auto]"
        >
          <label className="grid gap-1 text-xs font-semibold text-foreground">
            {t('settings.assistantConfig.language')}
            <Select
              value={item.locale === 'en' ? 'en' : 'vi'}
              onValueChange={(locale) => update(index, { locale })}
              disabled={readOnly}
            >
          ...
          <label className="grid gap-1 text-xs font-semibold text-foreground">
            ...
            <Input
              ...
              maxLength={160}
              disabled={readOnly}
              onChange={(event) => update(index, { expansion: event.target.value })}
            />
          </label>
          <div className="flex min-h-11 items-end justify-between gap-3 lg:justify-end">
            ...
            {!readOnly ? (
              <Button
                ...
                aria-label={t('settings.assistantConfig.removeAbbreviation')}
              >
...
function TemplateEditor({ value, onChange, readOnly }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {items.length === 0 ? (
        <EmptyEditor>{t('settings.assistantConfig.templatesEmpty')}</EmptyEditor>
      ) : null}
        ...
        <section key={index} className="rounded-md border border-border bg-surface-muted p-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {!readOnly ? (
                <Button
                  ...
                  size="icon"
                  onClick={() => remove(index)}
                  aria-label={t('settings.assistantConfig.removeTemplate')}
                >
                  <Trash2 size={16} aria-hidden="true" />
                </Button>
              ) : null}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="grid gap-1 text-xs font-semibold text-foreground">
              ...
              <Textarea
                className="min-h-24"
                value={listToLines(item.triggersVi)}
                disabled={readOnly}
                placeholder={t('settings.assistantConfig.onePerLine')}
                onChange={(event) => update(index, { triggersVi: linesToList(event.target.value) })}
              />
            </label>
            <label className="grid gap-1 text-xs font-semibold text-foreground">
              ...
              <Textarea
                className="min-h-24"
                value={listToLines(item.triggersEn)}
                disabled={readOnly}
                placeholder={t('settings.assistantConfig.onePerLine')}
                onChange={(event) => update(index, { triggersEn: linesToList(event.target.value) })}
              />
            </label>
            <label className="grid gap-1 text-xs font-semibold text-foreground">
              ...
              <Textarea
                ...
                maxLength={2000}
                disabled={readOnly}
                onChange={(event) => update(index, { answerVi: event.target.value })}
              />
            </label>
            <label className="grid gap-1 text-xs font-semibold text-foreground">
