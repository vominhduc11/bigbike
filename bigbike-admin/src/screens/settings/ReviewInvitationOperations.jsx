...
function operationMessage(row, t) {
  ...
  if (row.status === 'SENT')
    return t('settings.reviewInvitation.acceptedAt', {
      date: formatDateTime(row.providerAcceptedAt),
    })
  return '—'
}
  ...
  const columns = useMemo(
    () => [
        ...
        render: (row) => (
          <div className="text-sm">
            ...
            <div className="text-xs text-muted-foreground">
              {t('settings.reviewInvitation.completedAt', {
                date: formatDateTime(row.completedAt),
              })}
            </div>
          </div>
        ),
      },
      {
        ...
        render: (row) =>
          t('settings.reviewInvitation.productProgress', {
  ...
  if (loading) {
    return (
      <DetailSection title={t('settings.reviewInvitation.operationsTitle')}>
        <StatePanel
          title={t('settings.reviewInvitation.loading')}
          description={t('settings.reviewInvitation.loadingDescription')}
        />
      </DetailSection>
    )
  }
  ...
  if (error) {
    return (
      <DetailSection title={t('settings.reviewInvitation.operationsTitle')}>
        <StatePanel
          tone="danger"
          title={t('settings.reviewInvitation.loadErrorTitle')}
          description={error}
  ...
  return (
    <div className="mb-6 space-y-6">
      <div
        ...
        aria-label={t('settings.reviewInvitation.summaryAria')}
      >
        <KpiCard
          label={t('settings.reviewInvitation.kpi.pending')}
          value={summary?.pending ?? 0}
          icon={<Clock3 size={18} />}
          tone="warning"
          detail={t('settings.reviewInvitation.kpi.pendingHint')}
        />
        <KpiCard
          label={t('settings.reviewInvitation.kpi.sent')}
          value={summary?.sent ?? 0}
          icon={<CheckCircle2 size={18} />}
          tone="success"
          detail={t('settings.reviewInvitation.kpi.sentHint')}
        />
        <KpiCard
          label={t('settings.reviewInvitation.kpi.issues')}
          value={issueCount}
          icon={<AlertTriangle size={18} />}
          tone={issueCount > 0 ? 'danger' : 'info'}
          detail={t('settings.reviewInvitation.kpi.issuesHint')}
        />
        <KpiCard
          label={t('settings.reviewInvitation.kpi.optedOut')}
          value={summary?.optedOut ?? 0}
          icon={<Ban size={18} />}
          tone="neutral"
          detail={t('settings.reviewInvitation.kpi.optedOutHint')}
        />
      </div>
      ...
      >
        ...
        {deliveries.length === 0 ? (
          <StatePanel
            className="m-4"
            title={t('settings.reviewInvitation.emptyHistory')}
            description={t('settings.reviewInvitation.emptyHistoryDescription')}
          />
        ) : (
          <>
            <AdminTable
              ...
              mobileCard={(row) => ({
                ...
                meta: [
                  {
                    label: t('settings.reviewInvitation.columns.schedule'),
                    value: formatDateTime(row.dueAt),
                  },
                  {
                    label: t('settings.reviewInvitation.columns.products'),
                    value: t('settings.reviewInvitation.productProgress', {
                      reviewed: row.reviewedProductCount ?? 0,
                      total: row.productCount ?? 0,
                    }),
                  },
      ...
      >
        ...
        {optOuts.length === 0 ? (
          <StatePanel
            className="m-4"
            title={t('settings.reviewInvitation.emptyOptOuts')}
            description={t('settings.reviewInvitation.emptyOptOutsDescription')}
          />
        ) : (
          <>
            <AdminTable
              ...
              mobileCard={(row) => ({
                ...
                meta: [
                  {
                    label: t('settings.reviewInvitation.optOutColumns.date'),
                    value: formatDateTime(row.optedOutAt),
                  },
                ],
              })}
            />
            <div className="px-4">
              <PaginationControls
                pagination={optOutPagination}
                onPageChange={setOptOutPage}
                disabled={refreshing}
              />
            </div>
          </>
