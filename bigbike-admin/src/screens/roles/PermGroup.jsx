...
export function PermGroup({
  ...
  const grantedCount = isSuperAdmin
    ? total
    : group.permissions.filter((p) => activePerms.has(p.key)).length

  return (
    <CollapsibleSection
      title={t(group.groupKey, {
        defaultValue: MODULE_LABELS[group.moduleKey] || 'Nhóm quyền khác',
      })}
      open={open}
      ...
      badge={
        <span className="ml-auto shrink-0 text-xs font-medium text-muted-foreground tabular-nums">
          {t('roles.groupGrantedCount', {
            granted: grantedCount,
            total,
            defaultValue: '{{granted}}/{{total}} quyền',
          })}
        </span>
      }
    >
              ...
              >
                ...
                {requiredByKeys.length > 0 ? (
                  <span className="text-xs text-muted-foreground">
                    {t('roles.permissionRequiredBy', {
                      permissions: requiredByKeys
                        .map((key) => {
                          const keyLabel = PERM_LABEL_KEY_MAP[key]
                          return keyLabel
                            ? t(keyLabel, { defaultValue: t('roles.relatedPermission') })
                            : t('roles.relatedPermission')
                        })
                        .join(', '),
                    })}
                  </span>
                  ...
                  <span className="basis-full text-xs text-muted-foreground">
                    ...
                    {perm.requires
                      .map((key) => {
                        const keyLabel = PERM_LABEL_KEY_MAP[key]
                        return keyLabel
                          ? t(keyLabel, { defaultValue: 'quyền liên quan' })
                          : 'quyền liên quan'
                      })
                      .join(', ')}
                  </span>
