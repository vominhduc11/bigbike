...
export function RoleDetail({
  ...
  const [openGroups, setOpenGroups] = useState(
    () => new Set(catalog[0] ? [catalog[0].groupKey] : []),
  )
    ...
    <div className="roles-detail px-6 py-5">
      ...
      <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
        <div className="min-w-0">
          <div className="flex items-center gap-2.5 mb-0.5 flex-wrap">
            <h2 className="m-0 text-base font-bold text-foreground">{displayName}</h2>
            <Badge isSystem={role.isSystem} />
          </div>
          {showDesc && <p className="m-0 text-sm text-muted-foreground">{desc}</p>}
        </div>
      </div>

      {!editMode && canUpdate && !isSuperAdmin && !role.isSystem && assignedUserCount > 0 && (
        <Alert id={`delete-role-help-${role.id}`} tone="warning" size="sm" className="mb-3.5">
        ...
        <div className="flex items-start gap-2 px-3.5 py-2.5 mb-5 rounded-xs bg-primary/10 border border-primary/25">
          ...
          <p className="m-0 text-sm text-muted-foreground leading-relaxed">
            {t('roles.developerBanner', {
              defaultValue:
                'Vai trò kỹ thuật. Quyền của vai trò này được khoá không cho sửa, vì đây là thứ cho phép mở lại trang quản trị sau khi bảo trì — bỏ nhầm một quyền là không ai mở khoá được nữa.',
            })}
          </p>
        </div>
      )}
      ...
      {!isSuperAdmin && (
        <label className="flex items-center justify-end gap-2 mb-2 text-xs text-muted-foreground cursor-pointer">
          <Checkbox
            checked={showCodes}
            onCheckedChange={(c) => setShowCodes(c === true)}
          ...
          <div className="mb-6">
            ...
            {unknown.map((perm) => (
              <div key={perm} className="roles-perm-row">
                ...
                <span className="flex-1 text-sm text-foreground font-mono">{perm}</span>
      ...
      {(!editMode && canUpdate && !isPermissionLocked) || editMode ? (
        ...
        >
          {editMode ? (
            <>
              <Button variant="ghost" className="min-h-11" onClick={onCancelEdit} disabled={saving}>
                {t('roles.cancelBtn')}
              </Button>
              <Button
            ...
            <>
              ...
              {!role.isSystem && (
                <Button
                  ...
                  aria-describedby={
                    assignedUserCount > 0 ? `delete-role-help-${role.id}` : undefined
                  }
