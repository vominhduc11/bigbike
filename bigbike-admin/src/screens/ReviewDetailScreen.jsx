...
function PhotoButton({ url, index, alt, onOpen, t }) {
  ...
  return (
    ...
    >
      ...
      {state === 'error' ? (
        <span className="absolute inset-0 flex flex-col items-center justify-center gap-1 p-2 text-center text-xs text-danger">
          <X size={18} />
          {t('reviews.detail.imageLoadError')}
        </span>
      ) : null}
      <img
        ...
        className={
          state === 'error' ? 'hidden' : 'size-full object-cover transition group-hover:scale-105'
        }
      />
    </Button>
  )
...
export function ReviewDetailScreen({ reviewId, navigate, canUpdate, isSuperAdmin = false }) {
  ...
  const loadReview = useCallback(
    async ({ refresh = false } = {}) => {
      setState((current) => ({
        ...current,
        status: refresh ? current.status : 'loading',
        error: '',
        refreshing: refresh,
      }))
      try {
        ...
        setState((current) => ({
          status: refresh && current.item ? 'success' : 'error',
          item: refresh ? current.item : null,
          error: error.message,
          refreshing: false,
        }))
      }
  ...
  const handleDelete = useCallback(async () => {
    ...
    const confirmed = await showConfirm(
      t('reviews.deleteConfirmPermanent', {
        count: 1,
        defaultValue:
          'Xoá vĩnh viễn đánh giá này. Không thể hoàn tác; đánh giá và ảnh đính kèm sẽ bị xoá.',
      }),
  ...
  if (state.status === 'loading' && !state.item)
    return (
      <Screen>
        <StatePanel
          tone="info"
          title={t('reviews.detail.loading')}
  ...
  const actionButtons = (
    <div className="flex flex-wrap gap-2">
      {canUpdate && statusTargets.includes('APPROVED') ? (
        <Button
          type="button"
          variant="secondary"
          className="min-h-11"
          disabled={Boolean(pendingAction) || stale}
      ...
      {canUpdate && statusTargets.includes('SPAM') ? (
        <Button
          type="button"
          variant="outline"
      ...
      {canUpdate && statusTargets.includes('TRASH') ? (
        <Button
          type="button"
          variant="outline"
          className="min-h-11 text-danger"
      ...
      {canUpdate && statusTargets.includes('PENDING') ? (
        <Button
          type="button"
          variant="secondary"
          className="min-h-11"
      ...
      {!canUpdate ? (
        <span className="self-center text-sm text-muted-foreground">
          {t('reviews.detail.noActionPermission')}
        </span>
      ) : null}
      {canUpdate && review.status === 'TRASH' && !isSuperAdmin ? (
        <span className="self-center text-sm text-muted-foreground">
          {t('reviews.detail.superAdminDeleteHint')}
        </span>
      ) : null}
    </div>
  )
  ...
  return (
    <Screen>
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              variant="secondary"
              className="min-h-11"
              onClick={() => navigate(`/admin/reviews${readListQuery()}`)}
            >
            ...
            <Button
              ...
              variant="ghost"
              className="min-h-11"
              onClick={() => loadReview({ refresh: true })}
              disabled={state.refreshing}
            >
              <RefreshCw size={16} className={state.refreshing ? 'animate-spin' : ''} />
              {state.refreshing ? t('reviews.refreshing') : t('reviews.refresh')}
            </Button>
          </div>
        }
      />
      ...
      <div className="mb-6 grid gap-6 lg:grid-cols-3">
        <div className="grid gap-6 lg:col-span-2">
          <DetailSection title={t('reviews.detail.sectionReview')}>
            <dl className="m-0">
              <DetailRow label={t('reviews.colStatus')}>
              ...
              <DetailRow label={t('reviews.colAuthor')}>
                <span className="font-semibold">{authorName}</span>
              </DetailRow>
              <DetailRow label={t('reviews.detail.authorEmail')}>
                <span className="inline-flex flex-wrap items-center gap-2">
                  {formatText(review.authorEmail, t('reviews.detail.emailMissing'))}
                  {review.authorEmail ? (
                    <Button
                      type="button"
                      variant="ghost"
                    ...
                    >
                      <Copy size={16} />
                      {copyState === 'copied'
                        ? t('reviews.detail.emailCopied')
                        : t('reviews.detail.copyEmail')}
                    </Button>
                  ) : null}
                </span>
              </DetailRow>
              <DetailRow label={t('reviews.colDate')}>{formatDateTime(review.createdAt)}</DetailRow>
              <DetailRow label={t('reviews.detail.updatedAt')}>
                {formatDateTime(review.updatedAt)}
              </DetailRow>
            </dl>
            {copyState === 'error' ? (
              <p className="mt-3 mb-0 text-sm text-danger">{t('reviews.detail.emailCopyError')}</p>
            ) : null}
          </DetailSection>
          ...
          <DetailSection title={t('reviews.detail.sectionProduct')}>
            <dl className="m-0">
              ...
              <DetailRow label={t('reviews.detail.productSlug')}>
                {formatText(review.productSlug, t('reviews.detail.missingValue'))}
              </DetailRow>
              <DetailRow label={t('reviews.detail.productId')}>
                {formatText(review.productId, t('reviews.detail.missingValue'))}
              </DetailRow>
            </dl>
            {review.productId ? (
              <Button
                ...
                onClick={() => navigate(`/admin/products/${review.productId}`)}
              >
          ...
          <DetailSection
            title={t('reviews.detail.sectionPhotos')}
            description={t('reviews.detail.photoLimitHint')}
          >
              ...
              <div className="flex flex-wrap gap-3">
                {review.photos.map((url, index) => (
                  <PhotoButton
                    ...
                    url={url}
                    index={index}
                    alt={t('reviews.detail.photoAlt', {
                      index: index + 1,
                      author: authorName,
                      product: productName,
                    })}
                    onOpen={() => setPhotoIndex(index)}
                    t={t}
                  />
                ))}
              </div>
        ...
        <aside className="hidden h-fit lg:sticky lg:top-4 lg:block">
          <DetailSection
            title={t('reviews.detail.sectionActions')}
            description={
              canPermanentlyDeleteReview(review, isSuperAdmin)
                ? t('reviews.detail.permanentDeleteHint')
                : t('reviews.detail.lifecycleHint')
            }
          >
            {actionButtons}
          </DetailSection>
      ...
      <div className="lg:hidden">
        <StickyActionBar
          ariaLabel={t('reviews.detail.sectionActions')}
          info={
            canUpdate
              ? t('reviews.detail.mobileActionHint')
              : t('reviews.detail.noActionPermission')
          }
        >
          {actionButtons}
      ...
      {photoIndex !== null && review.photos?.length ? (
        <MediaPreviewLightbox
          items={review.photos.map((url, index) => ({
            publicUrl: resolveDisplayUrl(url),
            mimeType: 'image/jpeg',
            filename: `review-${review.id}-${index + 1}`,
            altText: t('reviews.detail.photoAlt', {
              index: index + 1,
              author: authorName,
              product: productName,
            }),
