...
export function LegacyDiscontinuedProductsScreen({ canUpdate }) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (error) =>
      setFormError(error?.message || 'Không thể lưu. Vui lòng kiểm tra lại các thông tin.'),
  })
    ...
    () => [
      {
        ...
        render: (item) => (
          <div className="min-w-0">
            <p className="m-0 font-medium text-foreground">{item.name || 'Chưa có tên'}</p>
      ...
      },
      {
        ...
        render: (item) =>
          item.imageUrl ? (
            <span className="text-sm text-success">Đã có ảnh</span>
          ) : (
            <span className="text-sm text-warning">Chưa có ảnh</span>
          ),
      },
              ...
              render: (item) => (
                <span className="inline-flex items-center gap-1">
                  <Button
                    ...
                    aria-label={item.enabled ? 'Tắt hiển thị' : 'Bật hiển thị'}
                    disabled={isTogglePending}
                    onClick={() => toggleProduct({ id: item.id, enabled: !item.enabled })}
                  >
  ...
  if (state.status === 'error') {
    return (
      <Screen>
        <StatePanel
          tone="danger"
          title="Không tải được danh sách hàng ngừng bán"
  ...
  return (
    <Screen>
      ...
      {!canUpdate ? (
        <ReadOnlyBanner warning="Bạn chỉ có quyền xem; không thể thêm, sửa hoặc bật/tắt trang hàng đã ngừng bán." />
      ) : null}
        ...
        >
          <div className="border-b border-border px-4 py-3">
            <h2 id="legacy-history-form-title" className="m-0 text-base font-semibold">
              {editing ? 'Sửa hàng cũ' : 'Thêm hàng cũ'}
            </h2>
          </div>
          <form onSubmit={submit} className="p-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <FormField
                label="Địa chỉ cũ (slug)"
                required
                helper="Chỉ phần sau /sp/ và trước .html, ví dụ gang-tay-cu."
              >
                <Input
                  value={form.slug}
                  maxLength={255}
              ...
              <FormField
                label="Tên tiếng Anh"
                helper="Bỏ trống nếu chưa có; website sẽ dùng tên tiếng Việt."
              >
                <Input
                  value={form.nameEn}
                  maxLength={255}
              ...
              <FormField
                label="Slug nhóm hàng"
                required
                helper="Dùng nhóm đang hiển thị để gợi ý tối đa 3 hàng thay thế đúng chủ đề."
              >
                <Input
              ...
              <FormField
                label="Đường dẫn ảnh đã kiểm chứng"
                helper="Chỉ dùng đường dẫn từ thư viện ảnh BigBike. Bỏ trống nếu chưa tìm được ảnh chính xác."
              >
                <Input
                  value={form.imageUrl}
                  maxLength={2048}
      ...
      <FilterBar ariaLabel="Lọc hàng đã ngừng bán">
        <FilterSearchInput
          ...
          placeholder="Tìm tên hoặc địa chỉ cũ"
      ...
      {state.status === 'success' && state.items.length === 0 ? (
        <StatePanel
          tone="neutral"
          title="Chưa có hàng cũ phù hợp"
          description="Thử đổi bộ lọc hoặc thêm một trang lịch sử mới."
          actionLabel={canUpdate ? 'Thêm hàng cũ' : undefined}
        ...
        <AdminTable
          ...
          caption="Danh sách trang mặt hàng đã ngừng bán"
