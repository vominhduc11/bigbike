...
function enabledBadge(enabled, t) {
  return enabled ? (
    <span className="inline-flex items-center gap-2 text-sm text-success">
      ...
      {t('legacyDiscontinued.statusVisible')}
    </span>
  ) : (
    <span className="inline-flex items-center gap-2 text-sm text-muted-foreground">
      <span className="h-2 w-2 rounded-full bg-muted-foreground" />
      {t('legacyDiscontinued.statusOff')}
    </span>
  )
}
...
export function LegacyDiscontinuedProductsScreen({ canUpdate }) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      closeForm()
      toast.success(
        t(editing ? 'legacyDiscontinued.updateSuccess' : 'legacyDiscontinued.createSuccess'),
      )
    },
  ...
  const columns = useMemo(
    () => [
      {
        key: 'name',
        label: t('legacyDiscontinued.colItem'),
        render: (item) => (
          <div className="min-w-0">
            <p className="m-0 font-medium text-foreground">
              {item.name || t('legacyDiscontinued.unnamed')}
            </p>
      ...
      {
        key: 'categorySlug',
        label: t('legacyDiscontinued.colCategory'),
        render: (item) => <span className="font-mono text-sm">{item.categorySlug}</span>,
      },
      {
        key: 'imageUrl',
        label: t('legacyDiscontinued.colImage'),
        render: (item) =>
          item.imageUrl ? (
            <span className="text-sm text-success">{t('legacyDiscontinued.hasImage')}</span>
          ) : (
            <span className="text-sm text-warning">{t('legacyDiscontinued.noImage')}</span>
          ),
      },
      {
        key: 'enabled',
        label: t('legacyDiscontinued.colStatus'),
        render: (item) => enabledBadge(item.enabled, t),
      },
      {
        key: 'updatedAt',
        label: t('legacyDiscontinued.colUpdated'),
        render: (item) => (
                  ...
                  primaryActions={[
                    {
                      key: 'edit',
                      label: t('common.edit'),
                      ariaLabel: t('legacyDiscontinued.editNamed', {
                        name: item.name || t('legacyDiscontinued.unnamed'),
                      }),
                      icon: Pencil,
                    ...
                    },
                    {
                      key: 'toggle',
                      label: t(
                        item.enabled ? 'legacyDiscontinued.disable' : 'legacyDiscontinued.enable',
                      ),
  ...
  const {
    ...
    toggle: toggleColumn,
    allColumns,
  } = useColumnVisibility(columns, 'columns:legacy-discontinued')
  ...
  if (state.status === 'error') {
    return (
      <Screen>
        <StatePanel
          tone="danger"
          title={t('legacyDiscontinued.loadError')}
  ...
  return (
    <Screen>
      <ScreenHeader
        ...
        actions={
          canUpdate ? (
            <Button className="min-h-11" onClick={openCreate} disabled={saveMutation.isPending}>
              <Plus size={16} />
              {t('legacyDiscontinued.create')}
            </Button>
          ) : null
        }
      />
      ...
      {formOpen ? (
        <section
          ...
          aria-labelledby="legacy-history-form-title"
        >
          <div className="border-b border-border px-4 py-3">
            <h2 id="legacy-history-form-title" className="m-0 text-base font-semibold">
              {t(editing ? 'legacyDiscontinued.editTitle' : 'legacyDiscontinued.createTitle')}
            </h2>
          </div>
          <form onSubmit={submit} className="p-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <FormField
                label={t('legacyDiscontinued.slugLabel')}
                required
                helper={t('legacyDiscontinued.slugHelp')}
              >
                <Input
                  value={form.slug}
                  maxLength={255}
                  onChange={(event) => changeForm('slug', event.target.value)}
                  placeholder="gang-tay-cu"
                  disabled={!canUpdate}
                />
              </FormField>
              <FormField label={t('legacyDiscontinued.nameLabel')} required>
                <Input
                  ...
                  placeholder={t('legacyDiscontinued.namePlaceholder')}
                  disabled={!canUpdate}
                />
              </FormField>
              <FormField
                label={t('legacyDiscontinued.nameEnLabel')}
                helper={t('legacyDiscontinued.nameEnHelp')}
              >
                <Input
                  value={form.nameEn}
                  maxLength={255}
                  onChange={(event) => changeForm('nameEn', event.target.value)}
                  placeholder={t('legacyDiscontinued.nameEnPlaceholder')}
                  disabled={!canUpdate}
                />
              </FormField>
              <FormField label={t('legacyDiscontinued.brandLabel')}>
                <Input
                  ...
                  maxLength={255}
                  onChange={(event) => changeForm('brandName', event.target.value)}
                  placeholder={t('legacyDiscontinued.optional')}
                  disabled={!canUpdate}
                />
              </FormField>
              <FormField
                label={t('legacyDiscontinued.categorySlugLabel')}
                required
                helper={t('legacyDiscontinued.categorySlugHelp')}
              >
                <Input
                  value={form.categorySlug}
                  maxLength={255}
                  onChange={(event) => changeForm('categorySlug', event.target.value)}
                  placeholder="gang-tay-xe-may-moto"
                  disabled={!canUpdate}
                />
              </FormField>
              <FormField
                label={t('legacyDiscontinued.imageUrlLabel')}
                helper={t('legacyDiscontinued.imageUrlHelp')}
              >
                <Input
                  value={form.imageUrl}
                  maxLength={2048}
                  onChange={(event) => changeForm('imageUrl', event.target.value)}
              ...
              <div className="flex items-center gap-3 md:col-span-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => changeForm('enabled', checked === true)}
                  disabled={!canUpdate}
                  id="legacy-enabled"
                />
                <label htmlFor="legacy-enabled" className="text-sm font-medium text-foreground">
                  {t('legacyDiscontinued.enabledLabel')}
                </label>
              </div>
            </div>
      ...
      >
        <FilterSearchInput
          ...
          placeholder={t('legacyDiscontinued.searchPlaceholder')}
          wrapperClassName="min-w-64 flex-1"
        />
        <FilterSelect
          value={query.enabled}
          onValueChange={(enabled) => setQuery((previous) => ({ ...previous, enabled, page: 1 }))}
          ariaLabel={t('legacyDiscontinued.colStatus')}
          options={[
            { value: 'ALL', label: t('legacyDiscontinued.allStatuses') },
            { value: 'true', label: t('legacyDiscontinued.statusVisible') },
            { value: 'false', label: t('legacyDiscontinued.statusOff') },
          ]}
        />
        ...
        >
          <RefreshCw size={16} className={state.isFetching ? 'animate-spin' : ''} />
          {t('common.refresh')}
        </Button>
      </ResponsiveFilterBar>
      ...
      {state.status === 'success' && state.items.length === 0 ? (
        <StatePanel
          tone="neutral"
          title={t('legacyDiscontinued.emptyTitle')}
          description={t('legacyDiscontinued.emptyDescription')}
          actionLabel={canUpdate ? t('legacyDiscontinued.create') : undefined}
          onAction={canUpdate ? openCreate : undefined}
        />
      ) : (
        <AdminTable
          ...
          mobileCard={(item) => ({
            ...
            meta: [
              { label: t('legacyDiscontinued.colCategory'), value: item.categorySlug || '—' },
              {
                label: t('legacyDiscontinued.colImage'),
                value: t(
                  item.imageUrl ? 'legacyDiscontinued.hasImage' : 'legacyDiscontinued.noImage',
                ),
              },
