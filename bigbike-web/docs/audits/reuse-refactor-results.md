...
| 2 paginator WP → `WpPagination`+`buildWpPageWindow` | **Confirmed-Deferred** | `tin-tuc` và `tim-kiem` dùng **thuật toán window khác nhau** (threshold/sentinel/neighbor khác → tập trang click khác cho cùng input) + markup khác (nav-vs-div, aria-label, `index` prop, glyph `…`-vs-`&hellip;`). Hợp nhất sẽ đổi hành vi. |
