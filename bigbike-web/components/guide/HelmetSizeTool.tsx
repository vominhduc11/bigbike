...
export default function HelmetSizeTool({ locale }: HelmetSizeToolProps) {
  ...
  return (
    <div className="mb-8 border border-border bg-white p-6 max-w-xl">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="relative flex items-center">
            <Input
              ...
              max="70"
              inputMode="numeric"
              className="pr-12 text-a4-content font-body font-normal"
            />
            <span className="absolute right-4 text-a5-meta text-muted-foreground select-none">cm</span>
          </div>
        </div>
        <Button
          onClick={handleCalc}
      ...
      {errorMsg && (
        <div className="border border-brand bg-brand-soft/10 border-l-4 border-l-brand p-4 text-a5-meta text-brand mb-4 font-body leading-relaxed">
          {errorMsg}
        </div>
      )}
        ...
        <div className="border border-border p-5 bg-muted">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {result.brands.map((brand) => (
              <span key={brand} className="border border-border bg-white px-3 py-1 text-a5-meta text-foreground font-body">
                {brand}
              </span>
            ))}
          </div>
