...
export function CartItemRow({
  ...
  return (
    <div
      className={cn(
        "relative grid grid-cols-[5rem_minmax(0,1fr)] gap-x-3 gap-y-3 py-5 transition-opacity lg:grid-cols-[130px_minmax(0,1fr)_auto_auto] lg:items-center",
    ...
    >
      <div>
        {item.image?.url ? (
          <MediaImage
            ...
            width={130}
            height={130}
            className="h-20 w-20 object-contain lg:h-32.5 lg:w-32.5"
          />
        ) : (
          <span className="flex h-20 w-20 items-center justify-center bg-secondary font-body text-a2-page font-semibold lg:h-32.5 lg:w-32.5">
      ...
      <div className="min-w-0 pr-12 lg:pr-0">
        <h3 className="m-0 font-body text-a3-section font-semibold leading-title">
          {item.productName}
        </h3>
        {item.variantName ? (
        ...
        {!item.available && (
          <p className="mb-0 mt-2 font-semibold text-destructive">{t("backorderNotice")}</p>
        )}
      </div>
      ...
      <div className="absolute right-0 top-5 lg:relative lg:right-auto lg:top-auto">
        <Button
          type="button"
          variant="ghost"
          size="icon"
