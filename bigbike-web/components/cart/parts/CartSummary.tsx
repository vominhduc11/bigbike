    ...
    <>
      <aside className="md:col-span-4">
        <div className="border border-border bg-background p-6 md:sticky md:top-24">
          <h2 className="mb-6 font-body text-a1-title font-semibold">{t("totalsHeading")}</h2>
        ...
        <div className="flex items-baseline justify-between gap-4 py-6">
          ...
          <strong className="font-body text-a1-title text-brand">{formatVnd(cart.totals.totalAmount)}</strong>
        </div>
      ...
      <div data-cart-mobile-checkout className="fixed inset-x-0 bottom-0 z-[var(--bb-z-bottom-nav)] border-t border-border bg-background shadow-[0_-8px_20px_rgba(0,0,0,0.12)] md:hidden">
        <div className="flex items-center gap-4 px-4 py-3 pb-[max(12px,env(safe-area-inset-bottom))]">
          <div className="min-w-0 flex-1">
            ...
            <strong className="block whitespace-nowrap font-body text-a3-section text-brand">{formatVnd(cart.totals.totalAmount)}</strong>
          </div>
          {hasUnavailable ? (
