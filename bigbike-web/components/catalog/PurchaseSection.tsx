...
type Props = {
  ...
  previewMode?: boolean;
  /** GMC deep link: render the selected variant's actual price, image and stock immediately. */
...
export function PurchaseSection({
  ...
  const [selectedOptions, setSelectedOptions] = useState<Record<string, string>>(() =>
    variantOptions(previewMode ? undefined : resolveLinkedVariant(product, initialVariantId)),
  );
  const [useLinkedVariant, setUseLinkedVariant] = useState(
    !previewMode && Boolean(initialVariantId),
  );
  ...
  const canBuy =
    !previewMode &&
    !isOutOfStock &&
    (!hasVariants || (!!selectedVariant && selectedVariant.isAvailable));
  ...
  const name = safeText(locale === "en" ? enName : product.name, product.name);
  ...
  const activeOriginBrandCountry =
    locale === "en"
      ? (enOriginBrandCountry ?? product.originBrandCountry)
      : product.originBrandCountry;
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                <p className="m-0 flex flex-wrap items-baseline gap-x-3">
                  <span className="font-body text-a1-title font-bold leading-tight text-brand">
                    {formatVndNumber(current)} ₫
                  </span>
                  {showOld ? (
                    <del className="font-body text-a4-content text-muted-foreground">
                ...
                <div className="mt-6">
                  <div>
                    <div className="flex items-center gap-4">
                      <label
                        htmlFor="bb-qty"
                        className="min-w-24 font-cta text-b4-action font-semibold uppercase text-foreground"
                      >
                        {tb("quantity")}
                      </label>
                      <QuantityStepper
                  ...
                  />
                  {addError ? (
                    <p className="mt-3 font-semibold text-destructive">{addError}</p>
                  ) : null}
