 ...
 * không", KHÔNG phải lặp lại thông số kỹ thuật. Admin quản theo từng sản phẩm (product.specStats,
 * V235/V256). HTML là nguồn render duy nhất — không còn lưới ô số liệu có cấu trúc để fallback. Đổi
 * ngôn ngữ qua `useLocalizedField("specStats")` (bản EN client refetch, giữ ISR/SEO). Rỗng → không
 * render gì.
 */
export function FeaturedSpecsBar({ viStatsHtml = "" }: { viStatsHtml?: string }) {
  const locale = useLocale();
  const t = useTranslations("Product");
  const enStatsHtml = useLocalizedField<string>("specStats");
