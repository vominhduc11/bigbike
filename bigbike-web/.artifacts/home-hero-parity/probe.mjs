// Home hero (#main-banner / .bb-main-banner) computed-style A/B probe.
// Usage:
//   node probe.mjs capture <out.json>   (against http://localhost:3001/)
...
async function capture(out) {
  ...
  for (const vp of VPS) {
    ...
    await page.waitForTimeout(1800); // swiper init + arrows/pagination render
    data[vp] = await page.evaluate(() => {
      ...
      const links = Array.from(document.querySelectorAll(".bb-main-banner-link"))
        .filter((el) => !el.closest(".swiper-slide-duplicate"));
      const link = links[0] || null;
      const picture = link ? link.querySelector("picture") : null;
      const img = link ? link.querySelector(".bb-main-banner-img") : document.querySelector(".bb-main-banner-img");
      const copy = link ? link.querySelector(".bb-main-banner-copy") : document.querySelector(".bb-main-banner-copy");
      ...
      return {
        container: PICK(container, [
          "position", "width", "height", "maxHeight", "overflowX", "overflowY", "backgroundColor",
        ]),
        link: PICK(link, [
          "position", "display", "width", "height", "color", "textDecorationLine", "overflowX", "overflowY", "backgroundColor",
        ]),
        ...
        prev: PICK(prev, [
          "position", "top", "left", "right", "zIndex", "display", "alignItems", "justifyContent",
          "width", "height", "paddingTop", "transform", "borderTopStyle", "borderTopWidth",
          "backgroundColor", "color", "cursor", "filter", "transitionProperty", "transitionDuration", "opacity",
        ]),
        next: PICK(next, ["left", "right", "width", "height"]),
        svgPrev: PICK(svgPrev, ["display", "flexShrink", "width", "height"]),
        pagination: PICK(pagination, [
          "position", "left", "bottom", "zIndex", "display", "alignItems", "columnGap", "width",
          "transform", "paddingBottom", "borderBottomStyle", "borderBottomWidth", "borderBottomColor",
          "color", "fontFamily", "fontSize", "lineHeight", "textAlign",
        ]),
      };
    });
