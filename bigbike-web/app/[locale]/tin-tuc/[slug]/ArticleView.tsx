    ...
    <div key="blog-detail-root">
      ...
      <div id="main-content">
        ...
        <article className={hasSidebar ? "min-w-0 md:col-span-8" : "min-w-0 md:col-span-12"}>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-6 border-y border-border py-5">
            <p className="m-0 font-body text-a4-content font-semibold text-muted-foreground">
              <Tr ns="Blog" k="shareWord" />
            </p>
            {/* Xem trước (admin): nút chia sẻ mở mạng xã hội ngoài → chỉ để nhìn, làm mờ
                + khóa bấm; trang thật vẫn là link chia sẻ bình thường. */}
            {previewMode ? (
              <>
                <span aria-label="Facebook" aria-disabled="true" className="text-muted-foreground/70 cursor-not-allowed">
                  <svg width="20" height="20" viewBox="0 0 320 512" fill="currentColor" aria-hidden="true">
                    <path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06h40.42V6.26S260.43 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72v70.62H22.89V288h81.39v224h100.17V288z" />
                  </svg>
                </span>
                <span aria-label="Twitter" aria-disabled="true" className="text-muted-foreground/70 cursor-not-allowed">
      ...
      {related.length > 0 ? (
        <section id="related" className="border-t border-border py-15">
          <Container>
            <h2 className="mb-6 font-body text-a1-title font-bold text-foreground">
...
function ArticleSidebarWidget({
  ...
  return (
    <section className="mb-8 p-4">
      <h2 className="mb-5 font-body text-a3-section font-bold text-foreground">{title}</h2>
      <div>
  ...
  return (
    <article className="flex border-b border-border py-5 first:pt-0 last:border-b-0">
      ...
      <div className="w-3/5 min-w-0 p-3">
        ...
        <p className="m-0 line-clamp-2 font-body text-a5-meta text-muted-foreground">{makeExcerpt(article, 70)}</p>
      </div>
    </article>
  );
}
