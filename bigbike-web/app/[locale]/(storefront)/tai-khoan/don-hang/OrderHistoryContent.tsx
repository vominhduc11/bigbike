...
export function OrderHistoryContent() {
  ...
  const error = queryError ? t("loadFailed") : "";

  const chipBase =
    "inline-flex h-9 items-center border border-border px-4 font-cta text-b4-action font-semibold uppercase transition-colors";
                  ...
                  <tr key={order.id} className="border-b border-border">
                    ...
                    <td className="py-4 align-top">
                      <Link
                        ...
                        className="inline-flex h-9 items-center justify-center bg-brand px-5 font-cta text-b4-action font-semibold uppercase text-white hover:bg-brand-hover"
                      >
                        {t("view")}
                      </Link>
                    </td>
          ...
          <ul className="m-0 flex list-none flex-col gap-3 p-0 md:hidden">
            {orders.map((order) => (
              <li key={order.id} className="border border-border bg-card p-4">
                ...
                <Link
                  href={toOrderDetailPath(order.id, locale)}
                  className="mt-3 inline-flex h-11 w-full items-center justify-center bg-brand px-5 font-cta text-b4-action font-semibold uppercase text-white hover:bg-brand-hover"
                >
                  {t("view")}
                </Link>
              </li>
