...
export function OrderHistoryContent() {
  ...
  const chipBase =
    "inline-flex h-9 items-center px-4 text-ui-14 max-md:text-ui-12 font-semibold uppercase font-cta border border-border transition-colors";
  ...
  return (
    <>
      ...
      {error && <p className="mb-4 text-ui-16 max-md:text-ui-14 text-brand">{error}</p>}
      ...
      {loading ? (
        ...
        <div className="py-[60px] text-center">
          <p className="m-0 text-ui-16 max-md:text-ui-14 text-muted-foreground">{t("empty")}</p>
        </div>
      ) : (
        <>
                  ...
                  <tr key={order.id} className="border-b border-border">
                    <td className="py-4 pr-4 align-top">
                      ...
                      {order.productNames && order.productNames.length > 0 && (
                        <p className="m-0 mt-1 line-clamp-2 text-ui-14 max-md:text-ui-12 text-muted-foreground">
          ...
          <ul className="m-0 flex list-none flex-col gap-3 p-0 md:hidden">
            {orders.map((order) => (
              <li key={order.id} className="border border-border bg-card p-4">
                ...
                <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-ui-16 max-md:text-ui-14">
