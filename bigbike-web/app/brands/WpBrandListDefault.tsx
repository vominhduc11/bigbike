...
export async function WpBrandListDefault({
  brands,
  pagination,
}: {
  brands: Brand[];
  ...
  if (brands.length === 0) {
    return <p className="woocommerce-info">{t("brandListEmpty")}</p>;
  }
  ...
  return (
    <>
      ...
      {pagination ? (
        <WpCategoryPagination page={pagination.page} totalPages={pagination.totalPages} baseHref={toBrandListPath()} />
      ) : null}
    </>
  );
}
