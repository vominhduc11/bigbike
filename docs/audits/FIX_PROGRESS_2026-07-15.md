...
| AUD-056 | Gỡ endpoint không consumer (grep 0 caller từng cái trước khi xóa) | ✅ | Quyết định #8, grep 0 caller từng endpoint trước khi xóa. Gỡ 5 nhóm: (1) `GET /checkout/options` + `CheckoutService.getOptions` + 2 DTO + matcher SecurityConfig + type chết web `CheckoutOptions`/`PaymentMethodOption`; (2) `GET /admin/translations/missing-required` — xóa cả controller/service/DTO; (3) `GET /address/provinces[+wards]` — xóa `VnAddressController`/`VnAddressService`/`VnAddressItem`/`vn-address.json` + matcher; (4) 7 endpoint inventory (grouped/movements×3/export.csv/availability×2) + service methods + 4 DTO + query repo mồ côi (giữ list + summary đang nuôi Dashboard); (5) `GET /admin/content/reference/categories` + service + DTO. Test cập nhật: Phase1F/1G/1H/1I, Phase1KInventoryP0Fix (thu gọn), ContentP1 — 6 suite 135/135 xanh. Docs cùng commit: API_CONTRACT (5 chỗ), API_FLOW_MAP, MODULE_CATALOG, PERMISSION_MATRIX (`inventory.write` không còn endpoint, không revoke seed), DATA_CONTRACT, WORKFLOW_OVERVIEW, BUSINESS_RULES PAY_RULE_001. |
