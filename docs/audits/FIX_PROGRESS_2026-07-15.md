...
| AUD-010 | Bổ sung điểm vào "Mua nhanh" trên web theo docs | ⏭ (đã revert theo owner) | Ban đầu thêm nút MUA NHANH + `QuickBuyDialog` (commit `c1a1b862`); **owner quyết định gỡ nút này** (sau `d6769dbe`) — khách đặt qua giỏ hàng. Đã xóa `QuickBuyDialog.tsx`, nút trong `BuyButtons`, state/dialog trong `PurchaseSection`, i18n `quickBuy*`; docs MODULE_CATALOG/API_CONTRACT ghi rõ endpoint `orders/quick-buy` còn nhưng không có điểm vào web. |
