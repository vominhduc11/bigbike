...
| AUD-010 | Bổ sung điểm vào "Mua nhanh" trên web theo docs | ⏭ (đã revert + gỡ hẳn theo owner) | Ban đầu thêm nút MUA NHANH + `QuickBuyDialog` (commit `c1a1b862`); **owner quyết định gỡ nút này**, sau đó yêu cầu **gỡ hẳn cả backend**. Đã xóa toàn bộ luồng "Mua nhanh": web (`QuickBuyDialog.tsx`, nút trong `BuyButtons`, state/dialog trong `PurchaseSection`, i18n `quickBuy*`, export chết `submitQuickBuy`/type `QuickBuyPayload`); backend (endpoint `POST /orders/quick-buy`, `CheckoutService.quickBuy`, DTO `QuickBuyRequest`, helper `buildLineItemFromProduct`/`tryParseUUID`, hằng `FLOW_QUICK_BUY`, matcher trong `SecurityConfig`, nhánh trong `RateLimitingFilter`, các test quick-buy trong Phase1F/1G, spec OpenAPI); docs (API_CONTRACT/API_FLOW_MAP/MODULE_CATALOG/BUSINESS_RULES/DATA_CONTRACT/STATE_MACHINES/USER_ROLES/GLOSSARY/INTEGRATION_GUIDE/ACCEPTANCE_CRITERIA/BUSINESS_PROCESS/PROJECT_OVERVIEW). Backend test-compile OK; Phase1F+1G 61/61 xanh. |
...
| AUD-028 | PDP tiếng Anh fallback về VI thay vì mất field | ✅ `78223032` | Giữ nội dung VI trong lúc payload EN đang tải/lỗi cho H1, taxonomy, trust, cam kết, ưu/nhược điểm, thông số, FAQ, mô tả, related/accessories, section phù hợp/size và CTA. Test fallback 2 ca mới; test liên quan 17/17; toàn web 212/212, lint 0 lỗi/6 warning cũ, build 37 trang. Căn cứ: API_CONTRACT §Product bilingual content (fallback từng field). |
...
| AUD-033 | Checkout/account: chuỗi + hotline/địa chỉ theo locale/settings | ✅ `04228061` | Checkout lấy hotline/địa chỉ/Zalo theo locale từ `site_settings`, gỡ số/người tư vấn hardcode; nút đặt hàng, timing, nhãn tỉnh/phường, email/SĐT và fallback order-detail dùng i18n. Test liên quan 4/4; toàn web 214/214, lint 0 lỗi/6 warning cũ, build 37 trang. Căn cứ: BUSINESS_RULES `CONTACT_PAGE_RULE_002/003`. |
| AUD-034 | Xóa vĩnh viễn danh mục: cảnh báo đủ số sản phẩm + danh mục con | ✅ `e4bbd37d` | Thêm phép xem trước ảnh hưởng dùng chung cho list/detail/bulk: đếm chính xác hợp cây danh mục + sản phẩm, loại lựa chọn cha/con bị trùng và chỉ xóa các gốc hiệu lực. Hộp xác nhận luôn hiện đồng thời cả 2 số; nếu không tải được ảnh hưởng thì không xóa. Docs: `BUSINESS_RULES.md` `CATEGORY_RULE_004`, `API_CONTRACT.md` Categories + OpenAPI. Test backend 3/3; admin 114/114; lint sạch; build pass; JSON locale/OpenAPI hợp lệ. |
| AUD-035 | Đổi VI/EN không làm mất draft Home Highlights | ✅ `ba1156a4` | Khi đổi VI/EN, màn hình kiểm tra trạng thái chưa lưu trước khi đồng bộ dữ liệu mới: chọn hủy giữ nguyên draft; chỉ bỏ draft sau xác nhận rõ. Thêm 3 test cho chốt đổi ngôn ngữ; admin 117/117, lint sạch, build pass; locale JSON hợp lệ. Không đổi API/business rule. |
...
| AUD-064 | Video mô tả/bài viết nhận YouTube/TikTok/Facebook theo AGENTS §14.3 | ✅ `9333ff16` | Mở lại khối video cho mô tả sản phẩm và bài viết; nhận `youtube/tiktok/facebook/upload`, kiểm URL đúng provider, chỉ cho upload nội bộ, chặn link rút gọn TikTok/Facebook và Vimeo; giữ URL legacy không đổi. Docs: `BUSINESS_RULES.md` `MEDIA_RULE_004`, `API_CONTRACT.md`, `DATA_CONTRACT.md`. Test backend 68/68; admin 119/119, lint sạch, build pass. |
...
| AUD-042 | Thống nhất quyền sửa tồn kho (`products.update` vs `inventory.write`) | ✅ `d1632b31` | Chốt theo business docs + code: form sản phẩm gửi `forceOutOfStock`/`variants[].isAvailable`, backend suy ra `stockState`, dùng `products.update`. `inventory.write` chỉ bảo vệ 2 endpoint availability phụ không có UI caller. Đồng bộ BUSINESS_RULES, API_CONTRACT, DATA_CONTRACT, PERMISSION_MATRIX; docs-only. |
| AUD-043 | Thống nhất rule WebSocket (role vs `orders.read`) theo code thực tế | ✅ `7cffbe61` | Chốt theo code: mọi admin `ACTIVE` có JWT hợp lệ được CONNECT; chỉ role có `orders.read` mới SUBSCRIBE `/topic/admin/orders`, kiểm tra lại trạng thái/quyền ở từng SUBSCRIBE. Đồng bộ `BUSINESS_RULES` + `PERMISSION_MATRIX`; docs-only. Kiểm tra `git diff --check` và đối chiếu `WebSocketConfig`/client admin đạt. |
| AUD-044 | Dọn docs theo mô hình Còn/Hết thủ công + COD duy nhất | ✅ `fb9fd9a2` | Đồng bộ canonical docs theo quyết định #7/#10: Còn/Hết do admin bật tay, bán/hủy không decrement/restore; đơn storefront mới chỉ COD, BACS/null chỉ tương thích đơn cũ; giao hàng miễn phí cả trong và ngoài hệ thống. Docs-only. Test backend mục tiêu: `Phase1FCheckoutApiTest` 46/46 và `Phase1HAdminOrderApiTest#cancelOrder_processingPaid_succeeds_andStockUntouched` 1/1; `git diff --check` đạt. |
| AUD-045 | State machine + migration/version notes hết mâu thuẫn/stale | ✅ `e9d347fd` | Đồng bộ state machine sản phẩm, giao hàng và thông báo theo code hiện hành; chốt notification đã đọc riêng theo từng admin và migration mới nhất V340. Test mục tiêu: `AdminNotificationServiceTest` 3/3, fulfillment 4/4; `git diff --check` đạt. Docs-only, chưa đụng OpenAPI. |
| AUD-046 | Sửa test backend khóa contract cũ (quantity/refund/CSRF/auto-paid) | ✅ `c1a1b862` | Các test stale đã được sửa trong `c1a1b862`, `080c5ac4`, `7e96af4d`: không decrement/restore quantity, không refund endpoint/status, checkout có CSRF, không auto-cancel BACS và ON_HOLD→PROCESSING không auto-paid. Xác minh mới: `Phase1FCheckoutApiTest` + `Phase1HAdminOrderApiTest` đạt 97/97. Full backend: 1.045 test, còn đúng 38 lỗi nền đã biết (29 failures + 9 errors), không có lỗi mới và không thuộc hai suite AUD-046. |
| AUD-048 | ARCHITECTURE bỏ wishlist + mô tả account pages đúng build | ✅ `8534df76` | Bỏ wishlist khỏi nhóm CSR; mô tả đúng auth/tài khoản/địa chỉ/đơn cá nhân là shell tĩnh, dữ liệu theo user tải ở client. Chỉ route handler redirect `dat-hang/order-received/[id]` là dynamic. `npm run build` đạt, tạo 37 trang tĩnh; `git diff --check` đạt. Docs-only. |
...
| AUD-056 | Gỡ endpoint không consumer (grep 0 caller từng cái trước khi xóa) | ✅ `4e335480` | Quyết định #8, grep 0 caller từng endpoint trước khi xóa. Gỡ 5 nhóm: (1) `GET /checkout/options` + `CheckoutService.getOptions` + 2 DTO + matcher SecurityConfig + type chết web `CheckoutOptions`/`PaymentMethodOption`; (2) `GET /admin/translations/missing-required` — xóa cả controller/service/DTO; (3) `GET /address/provinces[+wards]` — xóa `VnAddressController`/`VnAddressService`/`VnAddressItem`/`vn-address.json` + matcher; (4) 7 endpoint inventory (grouped/movements×3/export.csv/availability×2) + service methods + 4 DTO + query repo mồ côi (giữ list + summary đang nuôi Dashboard); (5) `GET /admin/content/reference/categories` + service + DTO. Test cập nhật: Phase1F/1G/1H/1I, Phase1KInventoryP0Fix (thu gọn), ContentP1 — 6 suite 135/135 xanh. Docs cùng commit: API_CONTRACT (5 chỗ), API_FLOW_MAP, MODULE_CATALOG, PERMISSION_MATRIX (`inventory.write` không còn endpoint, không revoke seed), DATA_CONTRACT, WORKFLOW_OVERVIEW, BUSINESS_RULES PAY_RULE_001. |
| AUD-057 | Web: dọn raw controls / arbitrary values trong active UI/CSS | ⬜ | |
| AUD-058 | Admin: dọn named CSS / hardcode / raw buttons | ⬜ | |
| AUD-060 | Gỡ 2 dead search export trong `public-api.ts` | ✅ `d6769dbe` | Xóa `search()`+`searchSuggest()` + 3 type (SearchResults/SearchTypeFilter/SearchQuery) — 0 caller (dropdown dùng route `/app/api/search-suggest`). Lint sạch |
| AUD-066 | Gỡ `GET /api/v1/search` + sửa docs API_FLOW_MAP | ✅ `d6769dbe` | Quyết định #8 (grep 0 caller): gỡ mapping `/search` + `parseTypes` + overload 3-arg `GlobalSearchService.search` + matcher SecurityConfig + nhánh RateLimitingFilter; docs API_FLOW_MAP/API_CONTRACT/MODULE_CATALOG/PERMISSION_MATRIX cập nhật (giữ `/search-suggest`) |
...
| AUD-068 | Gỡ `GET .../settings/{settingKey}` không caller | ✅ `d6769dbe` | Quyết định #8 (grep 0 caller): gỡ mapping GET-by-key + `AdminSettingsService.getByKey`; giữ PATCH-by-key (đang dùng) + GET list; docs API_CONTRACT ghi Removed |
...
| AUD-077 | Gỡ `REDIRECT_HIT_TRACKING` khỏi `.env.example` | ✅ `d6769dbe` | 0 reader trong code (grep toàn repo); xóa dòng khỏi `.env.example` + `.env.vps.example` |
