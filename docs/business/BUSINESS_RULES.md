...
- PRODUCT_RULE_009: `service/admin/ProductImportService.java` (`parseJson` + pipeline import dùng chung; `exportCurrentCatalogAsTemplateJson`/`toExportRequest` cho xuất JSON đầy đủ — nhánh đọc/xuất CSV đã gỡ hẳn 2026-07-06), `api/admin/AdminProductImportController.java` (`/api/v1/admin/products/import/validate|commit|export`, không còn tham số `type`), `persistence/repository/catalog/ProductJpaRepository.java` (`findAllBySkuIgnoreCase`), `persistence/repository/catalog/ProductVariantJpaRepository.java` (`findBySkuIgnoreCase`), `V30__add_inventory_tracking.sql` (`stock_movements.product_variant_id ... on delete cascade` — lý do phải đối chiếu biến thể theo SKU trước khi ghi), bộ kit `product-template/` (`mau-day-du.json` + `HUONG-DAN.md`).
