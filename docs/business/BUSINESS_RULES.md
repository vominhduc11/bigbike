...
> Áp dụng cho **sản phẩm, danh mục, thương hiệu, bài viết, cài đặt (site settings)** (mọi thực thể song ngữ). Bổ sung/điều chỉnh các `*_RULE_001`/`*_RULE_002` bên dưới. **Cài đặt** (V309) dùng cơ chế khoá khác hình dạng (boolean per-row thay vì mảng JSON) — xem ngoại lệ dưới `TRANSLATION_RULE_002` và `SETTINGS_RULE_001`.

- `TRANSLATION_RULE_001`: **Lưu tiếng Việt → tự sinh bản tiếng Anh.** Khi admin lưu một thực thể song ngữ, hệ thống **tự dịch các trường tiếng Việt sang tiếng Anh** (Google Gemini) cho những trường **chưa bị khoá** (xem `TRANSLATION_RULE_002`). Kết quả cuối luôn là: **tiếng Anh (ô chưa khoá) bám đúng tiếng Việt mới nhất** — nhưng để đỡ tốn chi phí/thời gian, admin **chỉ thực sự gọi Gemini cho trường/khối nào (a) nội dung tiếng Việt đã đổi so với lúc mở form, HOẶC (b) bản tiếng Anh tương ứng đang trống** (đề phòng lần dịch trước lỗi/thiếu); trường/khối không đổi và đã có tiếng Anh thì bỏ qua, không gọi lại — đây là tối ưu chi phí, **không đổi hành vi quan sát được** (EN vẫn luôn đúng theo VI mới nhất khi cần). So sánh "đã đổi" dùng bản chụp (snapshot) form lúc mở/lưu lần trước, không áp dụng khi tạo mới (chưa có gì để so sánh → dịch mọi trường có nội dung, như trước đây). Việc dịch chạy **phía MÁY CHỦ** qua endpoint nội bộ `POST /api/v1/admin/translate` (khoá Gemini server-side, **không** lộ ra trình duyệt admin; tắt "thinking" — `thinkingBudget=0` — giống lượt dịch bù hàng loạt, để giảm độ trễ). Dịch **không bao giờ chặn lưu**: khi chưa cấu hình khoá / Gemini lỗi → endpoint trả map rỗng, admin **giữ nguyên** bản tiếng Anh hiện có (không xoá). Quyền gọi endpoint: bất kỳ trong `products.update` / `catalog.update` / `content.update` / `settings.write` (thêm ở V309 cho màn Cài đặt). `CONFIRMED_FROM_CODE`
- `TRANSLATION_RULE_002`: **Khoá ô tiếng Anh admin sửa tay (manual override).** Mỗi thực thể lưu một danh sách `en_overrides` (JSON) = các trường/khối tiếng Anh **admin đã chỉnh tay**. Khi lưu: trường **có** trong danh sách → **giữ nguyên** bản tiếng Anh admin nhập; trường **không** có → **dịch lại từ tiếng Việt** (luôn bám tiếng Việt mới nhất). Độ chi tiết: **trường vô hướng** khoá theo từng ô (`name`, `description`, `seoTitle`…); **khối lặp** (`specifications`, `faqs`, `commitments`, `descriptionBlocks`…) khoá theo **cả khối** (`section:<tên>`). Sửa tiếng Việt sau này ở ô **chưa khoá** → bản tiếng Anh ô đó tự dịch lại; ô **đã khoá** đứng yên. `en_overrides` round-trip opaque qua admin (lưu DB, trả về khi mở form). **Ngoại lệ — Cài đặt (site settings, V309):** mỗi setting đã là **1 row độc lập** trong `site_settings` (không có trường/khối lồng nhau như Product/Category/Brand/Article) nên khoá bằng **1 cột boolean `en_locked`** thay vì mảng JSON `en_overrides` — cùng ngữ nghĩa (đã khoá = giữ nguyên khi dịch lại, chưa khoá = dịch lại theo VI mới nhất), chỉ khác hình dạng lưu trữ. Xem `SETTINGS_RULE_001`. `CONFIRMED_FROM_CODE`
...
>
> Evidence: `service/integration/GeminiTranslationService.java`, `api/admin/AdminTranslateController.java` (`TRANSLATE_PERMISSIONS`), `service/admin/EnOverridesCodec.java`, cột `en_overrides` (V296), cột `site_settings.en_locked` (V309), admin `src/lib/geminiTranslate.js` (`translateSettingsForm`) + `src/lib/adminApi.js (translateFields)`, `screens/settings/SettingField.jsx` + `screens/SettingsScreen.jsx`.
...
- `PRODUCT_RULE_007`: **Đăng sản phẩm MỚI phải nhập đủ 13 ô bắt buộc.** Khi admin **tạo mới** một sản phẩm (KHÔNG áp dụng khi sửa sản phẩm cũ), phải điền đủ **13 ô**: **Tên, Danh mục, Thương hiệu, SKU, Đường dẫn (slug), Giới tính, Giá bán lẻ, Ảnh đại diện, Mô tả ngắn, Mô tả sản phẩm, FAQ (≥ 1 câu), Số liệu nổi bật (≥ 1 dòng), Dải tin cậy (≥ 1 dòng — `trustBadges`, dải badge trên tên sản phẩm)**. _(Ô "Quick Answer / câu trả lời nhanh" — làm lại ở V300 sau khi từng bị gỡ hoàn toàn ở V253 — vẫn là ô **tùy chọn**, KHÔNG nằm trong 13 ô bắt buộc này.)_ **Sửa sản phẩm cũ** vẫn lưu nhanh được — các ô đó chỉ là **nhắc nhở**, không chặn lưu. Quy tắc này **chặt hơn** publish-gate `PRODUCT_RULE_005` (7 trường) và **chỉ kích hoạt ở luồng tạo mới**. Thực thi **thuần ở frontend admin** (Zod `createProductSchema` `.superRefine` theo cờ `isCreate`; checklist "required" trong form tạo mới) — **backend KHÔNG enforce** field-completeness ở tầng API (tránh vỡ test / luồng import). `CONFIRMED_FROM_CODE`
...
- `SETTINGS_RULE_001` (2026-07-03): **Cài đặt (site settings) nhập song ngữ bằng nút VI/EN, không còn 2 ô xếp chồng.** Trước rule này, mỗi setting dịch được (`isTranslatableSetting()` — nhóm `general`/`public_home`/`public_hero`/`seo`, kiểu chữ) hiện **cả 2 ô** (VI + EN) cùng lúc trong màn Cài đặt, không khoá tay, không tự dịch. Nay màn Cài đặt mirror 1:1 cơ chế của Sản phẩm: **1 ô hiển thị tại 1 thời điểm**, đổi qua nút VI/EN ở header admin (`useContentLang()`); setting **không** dịch được (ảnh/số/điện thoại/boolean/ngân hàng…) luôn hiện đúng 1 giá trị tiếng Việt, không đổi theo nút ngôn ngữ. Khi bấm Lưu, hệ thống **tự động dịch VI→EN ngầm** (`TRANSLATION_RULE_001`) cho các setting dịch-được **chưa bị khoá** trong lô đang lưu, dùng chung endpoint `POST /api/v1/admin/translate` (quyền `settings.write` thêm vào `TRANSLATE_PERMISSIONS`). Gõ tay vào ô tiếng Anh (đang xem EN) → **khoá field đó ngay lúc gõ** (`TRANSLATION_RULE_002`, cột boolean `site_settings.en_locked`, V309) — lần lưu sau không tự dịch đè; khoá **persist xuống DB per-row**, không phải theo phiên làm việc trình duyệt. Không có thao tác "mở khoá" — một khi admin đã tự sửa tay, ô đó đứng yên vĩnh viễn cho tới khi admin tự sửa VI/EN lại. `CONFIRMED_FROM_CODE`

Evidence:

- `SiteSettingEntity.java` (cột `en_locked`, V309 — `V309__add_en_locked_to_site_settings.sql`)
- `AdminSiteSettingResponse.java` / `UpdateSiteSettingRequest.java` / `BatchUpdateSettingsRequest.BatchSettingUpdate` (field `enLocked`)
- `AdminSettingsService.java` (`updateSetting`/`batchUpdateSettings` ghi `enLocked` theo giá trị FE gửi lên — presence-flag, null = giữ nguyên khoá cũ, giống `enOverrides` của Product)
- `AdminTranslateController.java` (`TRANSLATE_PERMISSIONS` thêm `settings.write`)
- `bigbike-admin/src/screens/settings/SettingField.jsx` (ô đơn theo `useContentLang()`, khoá khi gõ tay ở chế độ EN)
- `bigbike-admin/src/screens/SettingsScreen.jsx` (`handleSave` gọi `translateSettingsForm` trước `batchUpdateSettings`, state `enLocked` theo key)
- `bigbike-admin/src/lib/geminiTranslate.js` (`translateSettingsForm` — biến thể flat-field, không có block lồng nhau như `translateProductForm`)
- `screens/settings/constants.js` (`TRANSLATABLE_GROUPS`, `isTranslatableSetting()` — không đổi phạm vi field nào được dịch)
