...
- `MEDIA_RULE_004` (owner decision 2026-06-25; đồng bộ code 2026-07-15) — **Khối video trong mô tả sản phẩm và thân bài viết** nhận đúng 4 provider: `youtube`, `tiktok`, `facebook`, `upload`. Ba provider nhúng phải dùng link video đầy đủ thuộc host được duyệt; `upload` phải trỏ về video trong MinIO/đường media nội bộ. Link rút gọn TikTok (`vt.tiktok.com`, `vm.tiktok.com`) và Facebook (`fb.watch`) bị từ chối vì không xác định được video nếu không theo redirect; Vimeo/nền tảng khác cũng bị từ chối. URL video legacy đã lưu được giữ nguyên khi không đổi để bản ghi cũ vẫn sửa được. `CONFIRMED_FROM_OWNER_DECISION` — căn cứ `AGENTS.md` §14.3.
...
- `PRODUCT_RULE_009` (2026-07-04; **chuyển JSON-only 2026-07-06** — nhánh CSV đã gỡ hẳn, nhập lẫn xuất chỉ còn JSON; **shape lồng song ngữ 2026-07-07**): **Nhập sản phẩm hàng loạt từ file JSON — hành vi bắt buộc.** File là **mảng object `ProductImportRow`** (mỗi object = 1 sản phẩm) — mỗi cột song ngữ gộp thành 1 object lồng ngay tại cột đó (vd `name: {nameVI, nameEN}`, `seo: {titleVI, titleEN, descriptionVI, descriptionEN, canonicalUrl}`), không còn khối `translations` rời. `ProductImportRowMapper` chuyển đổi mỗi dòng sang đúng shape `UpsertProductRequest`+`ProductTranslationRequest` mà API sửa 1 sản phẩm đã dùng, trước khi vào chung 1 luồng xử lý — 2 DTO đó không đổi. Danh mục/thương hiệu trong file ghi bằng **slug**, backend tra `findBySlug` — không tồn tại thì báo lỗi dòng đó, **không tự tạo**. Đối chiếu sản phẩm đã có: **SKU sản phẩm ưu tiên, fallback slug**; SKU sản phẩm trùng nhiều bản ghi → lỗi dòng đó (không đoán). **Biến thể đối chiếu theo SKU** (JSON có thể gửi kèm `variants[].id` làm gợi ý nhưng chỉ dùng sau khi xác nhận biến thể thuộc đúng sản phẩm) trước khi ghi — hệ thống luôn tự tra theo SKU để cập nhật đúng chỗ thay vì tạo mới, tránh xoá nhầm biến thể cũ và làm đứt các liên kết lịch sử. Sản phẩm **mới** luôn vào `DRAFT` bất kể `publishStatus` trong file ghi gì. Sản phẩm **đã có**: `publishStatus` được áp dụng **trừ khi** giá trị là `PUBLISHED` hoặc giá trị legacy/không hợp lệ (`HIDDEN`/`ARCHIVED`/`PENDING`/`PRIVATE`/không nhận dạng được) — các trường hợp này bị bỏ qua kèm cảnh báo (mã `IGNORED`, giữ nguyên `publishStatus` hiện có của sản phẩm), không tự động đăng bán qua nhập file (vì luồng cập nhật sản phẩm không chạy lại `validatePublishReadiness`, chỉ endpoint đăng bán riêng mới chạy). `HIDDEN` không còn là trạng thái chủ động từ 2026-07-07 (`PRODUCT_RULE_005`) nên nay được xử lý **giống hệt** `ARCHIVED`/`PENDING`/`PRIVATE`, không còn "đi qua bình thường" như trước. Bước xem trước (validate) gọi thẳng `CatalogRequestValidator.validateProductRequest(..., preview=false, ...)` — **không** dùng `previewProduct` (chế độ preview cố tình bỏ qua bắt buộc tên tiếng Anh và kiểm tra trùng SKU/slug liên sản phẩm, nên không phản ánh đúng những gì lưu thật sẽ chặn). Thiếu tên tiếng Anh → lỗi dòng đó, không tự lấy tên tiếng Việt thay thế. Ảnh: link đã thuộc kho MinIO thì giữ nguyên; link ngoài hoặc để trống → bỏ qua (không tải về), đánh dấu "thiếu ảnh" — áp dụng khi **tạo mới**. **Cập nhật sản phẩm đã có (owner chốt 2026-07-06): ảnh/video KHÔNG BAO GIỜ bị nạp file ghi đè** — `image` (ảnh đại diện), `gallery` (bộ sưu tập ảnh), `videos` (video sản phẩm) ở cấp sản phẩm, và `imageUrl`/`imageAlt`/`gallery` của **từng biến thể đã khớp SKU với biến thể có sẵn**, đều bị bỏ qua bất kể file ghi gì — dữ liệu ảnh/video đang lưu trong hệ thống luôn được giữ nguyên. Biến thể **mới** (SKU chưa từng tồn tại) không có ảnh cũ để giữ nên vẫn lấy ảnh từ file bình thường. Muốn đổi ảnh/video của sản phẩm đã có phải sửa trực tiếp ở trang quản trị, không qua nạp file. **Xuất (`GET /import/export`)** trả về JSON **đầy đủ (full-fidelity)** cùng shape với file nhập (`ProductImportRow[]`, category/brand là slug, related/accessory là SKU, gồm cả biến thể/ảnh/khối mô tả/FAQ/badge/cam kết/video…) → tải về sửa bất kỳ trường nào rồi nạp lại khép kín; các khoá bỏ trống khi nạp lại giữ nguyên dữ liệu cũ (presence-flag). `CONFIRMED_FROM_CODE`
