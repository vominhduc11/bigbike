...
- `product.sku` is the product-level **SKU** — required to save (draft or publish) when the product has **no variants**; optional once the product has ≥1 variant, since it is not the selling code in that case. See `PRODUCT_RULE_005`. `CONFIRMED_FROM_CODE`
...
- `PRODUCT_RULE_005` (viết lại 2026-07-07 — thay cho quy tắc cũ chỉ áp dụng lúc tạo mới, xem `PRODUCT_RULE_007`): **Điều kiện bắt buộc theo trạng thái nháp/xuất bản và có/không biến thể — áp dụng như nhau cho tạo mới lẫn sửa.**
  ...
  - **Sản phẩm CÓ ≥1 biến thể** — bắt buộc cả Lưu nháp lẫn Đăng: Tên, Đường dẫn, Danh mục, Thương hiệu, Giới tính; và với **mỗi biến thể**: SKU. SKU/Giá niêm yết **cấp sản phẩm** KHÔNG bắt buộc khi đã có biến thể. **Giá niêm yết mỗi biến thể** (từ 2026-07-07, xem `PRODUCT_RULE_013`) chỉ bắt buộc khi sản phẩm **không có** Giá niêm yết cấp sản phẩm hợp lệ (`> 0`) để làm "giá chung" thay thế — có giá chung hợp lệ thì biến thể được phép bỏ trống Giá niêm yết riêng (tự dùng giá chung). **Ảnh đại diện sản phẩm** và **ảnh đại diện màu của mỗi biến thể** chỉ bắt buộc khi **Đăng**.
