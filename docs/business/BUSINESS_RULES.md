...
- `PRODUCT_RULE_006`: **"Hiển thị trên web" — admin bật/tắt các section nằm TRONG khối Tab của trang chi tiết sản phẩm (PDP), opt-in.** Bảng công tắc **chỉ quản 5 section dạng tab**: `description, specifications, faqs, videos, reviews` (Mô tả / Thông số / FAQ / Video / Đánh giá). Một section **chỉ hiện khi VỪA được bật VỪA có nội dung** — bật mà trống vẫn không hiện. **Các khối NGOÀI tab KHÔNG quản ở bảng này** (luôn hiện khi có nội dung, không bật/tắt được): ô số liệu nổi bật (`specStats`), dải tin cậy trên tên (`trustBadges`), khối cam kết dưới nút mua (`commitments`), **ưu/nhược điểm (`prosCons`) — khối RIÊNG cố định ngay dưới mô tả (V251)**, khối "Mua tại BigBike.vn" (`trust`), sản phẩm tương tự (`related`), phụ kiện bán kèm (`accessories`); cùng khối mua hàng (ảnh/tên/giá/nút mua), breadcrumb, dải liên hệ chân trang, và `contentBottom` (nội dung SEO dưới). Tất cả nhóm này chỉ gate theo "có nội dung". **Sản phẩm tạo MỚI**: 5 tab mặc định **tắt hết** (opt-in — admin tự bật). **Sản phẩm cũ chưa cấu hình**: cột `section_visibility` = NULL → web giữ **hành vi legacy = hiện theo nội dung**; lần đầu admin lưu lại, form **seed bật-sẵn theo nội dung hiện có** (reviews seed bật) rồi ghi explicit nên web không đổi. Lưu trữ: opaque JSON string `{sectionKey: boolean}` trên `products.section_visibility` (như `size_guide`); backend chỉ truyền qua, admin serialize / web parse (`lib/utils/section-visibility`). **Lịch sử:** bảng này từng quản 16 section (gồm cả khối ngoài tab + 3 mục `prosCons`/`suitability`/`sizeGuide`), kèm kéo-thả đổi thứ tự (`_order`). Đã thu hẹp về 5 tab. `suitability`/`sizeGuide` thành **khối trong "Mô tả sản phẩm"** (theo visibility của `description`). **`prosCons` (Ưu/Nhược điểm) đã TÁCH RA lại (V251)** thành khối RIÊNG cố định ngay dưới mô tả, ngoài tab — KHÔNG nằm trong bảng công tắc, chỉ gate theo "có nội dung", thứ tự cố định: đặt **ngay dưới mô tả và ngay TRÊN "Sản phẩm tương tự"**. Các khoá ngoài-tab + `_order` trong map cũ được **bỏ qua an toàn** khi đọc. Mục **"Tùy chỉnh tab" (V231) đã gỡ khỏi form admin**: cấu hình tab theo từng sản phẩm **không bao giờ được web áp dụng** kể từ lần dựng lại PDP (V236) — web render theo bố cục cố định + nhãn i18n, và `ProductView` truyền `tabs={[]}` cho `ProductTabsSection`. Cột `products.product_tabs` và DTO vẫn còn (dormant, không có UI sửa). Gate visibility ở **cả web** (server render), không phải chỉ ràng buộc admin. `CONFIRMED_FROM_CODE`
