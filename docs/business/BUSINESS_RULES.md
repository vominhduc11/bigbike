...
- `ORDER_RULE_002` — `paymentMethod` no longer affects completion. New web orders are always stored with `paymentMethod = COD` — the storefront shows exactly one fixed method, COD, and the API rejects other codes (owner decision 2026-07-15, see `PAY_RULE_001` — supersedes the 2026-06-23 "store null" model). Legacy orders carrying `BACS`/`null` are still read and displayed normally. (History: until 2026-06-23 a `COD` order could not be completed until `paymentStatus = PAID`; that guard was removed with the owner decision to reconcile payment offline.) `CONFIRMED_FROM_CODE`
- `ORDER_RULE_003` — `fulfillmentType = DELIVERY` orders cannot transition to `COMPLETED` unless `fulfillmentStatus = DELIVERED`. Reason: a delivery order cannot be "complete" before it has actually been delivered. Admin must walk fulfillment through `UNFULFILLED → PROCESSING → SHIPPED → DELIVERED` via `PATCH /admin/orders/{id}/fulfillment` first — jumping straight from `UNFULFILLED` to `DELIVERED` is rejected (409), including for legacy orders with a `null` `fulfillmentStatus`, which are treated as the `UNFULFILLED` baseline for this check (fixed 2026-07-06 — see `STATE_MACHINES.md` §8). (Every order is now a `DELIVERY` order — the in-store `IN_STORE` fulfillment type was retired with POS, 2026-06-23.) Backend message: `Chỉ được hoàn thành đơn giao hàng sau khi đã giao thành công.` `CONFIRMED_FROM_CODE`
- `ORDER_RULE_004` — Orders with `paymentStatus = PAID` **can be cancelled directly** (no refund step). Refunds were removed platform-wide (2026-06-23); the admin reconciles the money manually outside the system. **Cancelling does NOT restore stock** — inventory is manual boolean availability (owner decision 2026-06-23, FIX_PROMPT_2026-07-15.md §2 #7): selling never decrements and cancelling never restores; the admin toggles Còn/Hết by hand. (`PARTIALLY_PAID` removed in V114; `REFUNDED` removed 2026-06-23 — the prior cancel-blocking guard and its `POST /admin/orders/{id}/refund` requirement no longer exist.) `CONFIRMED_FROM_CODE`
...
- `bigbike-admin/src/screens/ProductDetailScreen.jsx` — `getPublishReadiness` (publish gate: required theo bảng draft/publish × có/không biến thể; "Nên bổ sung" = warning), `PublishChecklistModal` (ẩn nút đăng khi còn blocker), `TAB_SECTIONS.general` (PRODUCT_RULE_005)
...
- PRODUCT_RULE_009: `service/admin/ProductImportService.java` (`parseJson` + pipeline import dùng chung; import luôn set `DRAFT`; `discardExportOnlyImportFields` — bỏ qua publish/media/relations/variant id khi nhập Nháp; `checkImportDescriptionBlocks` — chỉ nhận block mô tả sản phẩm trong template; `preserveExistingMedia` — copy ảnh/gallery cũ vào từng biến thể đã khớp SKU để `applyVariants` không xoá nhầm; `exportCurrentCatalogAsTemplateJson`/`toExportRow` cho xuất JSON đầy đủ — nhánh đọc/xuất CSV đã gỡ hẳn 2026-07-06), `api/admin/AdminProductImportController.java` (`/api/v1/admin/products/import/validate|commit|export`, không còn tham số `type`), `persistence/repository/catalog/ProductJpaRepository.java` (`findAllBySkuIgnoreCase`), `persistence/repository/catalog/ProductVariantJpaRepository.java` (`findBySkuIgnoreCase`, `findByIdsWithGallery` — eager-fetch gallery cho biến thể cũ vì `commitImport` không chạy trong 1 transaction), `V30__add_inventory_tracking.sql` (`stock_movements.product_variant_id ... on delete cascade` — lý do phải đối chiếu biến thể theo SKU trước khi ghi), bộ kit `product-template/` (`mau-day-du.json` + `HUONG-DAN.md`).
...
- `PAY_RULE_001`: **COD is the single storefront payment method** (owner decision 2026-07-15, FIX_PROMPT_2026-07-15.md §2 #10 — supersedes the 2026-06-23 "send no paymentMethod / store null" model). The checkout page displays exactly one fixed method — "Thanh toán khi nhận hàng (COD)" — with no selection step, always sends `paymentMethod = "COD"`, and new orders are stored with `paymentMethod = COD`. The API normalises an omitted value to `COD` and rejects any other explicit code (including `BACS`). (`GET /api/v1/checkout/options` was removed 2026-07-15, AUD-056 — with a single fixed method there is nothing to fetch.) BACS is no longer offered on the storefront — existing orders carrying `BACS`/`null` are still read and displayed normally. There is no automatic payment gateway — all payment is reconciled manually by the admin. `CONFIRMED_FROM_CODE`
- `PAY_RULE_002`: Manual-confirm reconciliation. New online orders are created in `PROCESSING` with `paymentStatus = UNPAID`; the admin reconciles the money offline and marks the order paid via `PATCH /admin/orders/{id}/payment-status` whenever convenient — payment does not gate completion (`ORDER_RULE_001`). No payment redirect, no provider webhook. New orders can no longer start in `ON_HOLD` via BACS (COD-only, see `PAY_RULE_001`); legacy BACS orders already in `ON_HOLD` follow the normal order state machine. The Alepay/ZaloPay online-gateway plan was dropped. `CONFIRMED_FROM_CODE`
