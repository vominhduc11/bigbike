...
- `RETURN_RULE_007`: **REFUNDED via RMA requires full-order coverage.** Transitioning a return to `REFUNDED` (from `RECEIVED` or `INSPECTING`) is only allowed when, for every order line item, the running sum of non-rejected return quantities equals the original ordered quantity. Backend rejects partial-coverage attempts with `ConflictException` code `RETURN_NOT_FULL_COVERAGE`. Rationale: `RefundService.applyRefund` is full-refund-only since V114 (`PARTIALLY_PAID` removed) and operates on the whole order — calling it for a partial-RMA would over-refund and over-restore stock/serials for items the customer kept. Partial returns must close via `COMPLETED`; any standalone refund must go through `POST /admin/orders/{id}/refund`. `CONFIRMED_FROM_CODE`
