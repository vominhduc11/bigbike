...
|---|---|---|---|---|---|---|
| Product | `publishStatus` | `DRAFT`, `PUBLISHED`, `HIDDEN`, `ARCHIVED`, `PENDING`, `PRIVATE`, `TRASH` | Controlled publish transitions; soft-delete to `TRASH`; restore `TRASH -> DRAFT`. | Backend validator | `CONFIRMED_BACKEND_ENFORCED` | `PublishStatus.java`, `AdminMutationValidators.java`, `AdminCatalogMutationService.java`, `CatalogReadService.java` |
...
| Inventory / Stock | `stockState`, quantity fields | `IN_STOCK`, `LOW_STOCK`, `OUT_OF_STOCK`, `PREORDER`, `CONTACT_FOR_STOCK` | Quantity changes recompute stock state except admin-controlled preorder/contact states. | Backend policy/service | `CONFIRMED_BACKEND_ENFORCED` for recompute; full serial lifecycle `NEEDS_VERIFICATION` | `ProductStockState.java`, `InventoryPolicyService.java`, `CheckoutService.java`, `AdminOrderService.java`, `AdminReturnService.java` |
...
| Content Article/Page | `publishStatus` | Same `PublishStatus` enum; admin controller regex exposes `DRAFT`, `PUBLISHED`, `HIDDEN`, `ARCHIVED`; mutation service uses shared transition validator. | Publish transitions enforced on update; delete archives content. | Backend service | `CONFIRMED_BACKEND_ENFORCED`; public filtering `NEEDS_VERIFICATION` | `AdminContentController.java`, `AdminContentMutationService.java`, `AdminMutationValidators.java` |
...
Không khẳng định terminal tuyệt đối vì code cho phép `ARCHIVED -> DRAFT` và `TRASH -> DRAFT`.
...
| `DRAFT` | `ARCHIVED` | Admin / role có `products.update` | Product exists. | Product không public. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
...
| `PUBLISHED` | `ARCHIVED` | Admin / role có `products.update` | Product exists. | Product không public. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java`, `CatalogReadService.java` |
...
| `HIDDEN` | `ARCHIVED` | Admin / role có `products.update` | Product exists. | Archived/non-public. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
...
| `ARCHIVED` | `DRAFT` | Admin / role có `products.update` | Product exists. | Re-open as draft. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `ARCHIVED` | `TRASH` | Admin / role có `products.update` | Product exists. | Soft-delete. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `PENDING` | `PUBLISHED` | Admin / role có `products.update` | WP-import/review state. | Product public. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `PENDING` | `DRAFT` | Admin / role có `products.update` | WP-import/review state. | Product internal draft. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `PENDING` | `TRASH` | Admin / role có `products.update` | WP-import/review state. | Soft-delete. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `PRIVATE` | `PUBLISHED` | Admin / role có `products.update` | WP-import/private state. | Product public. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `PRIVATE` | `DRAFT` | Admin / role có `products.update` | WP-import/private state. | Product draft. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `PRIVATE` | `HIDDEN` | Admin / role có `products.update` | WP-import/private state. | Product hidden. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
| `PRIVATE` | `TRASH` | Admin / role có `products.update` | WP-import/private state. | Soft-delete. | `CONFIRMED_BACKEND_ENFORCED` | `AdminMutationValidators.java` |
...
|---|---|---|---|---|
| `DRAFT` | `HIDDEN` | Not in allowed transition set. | Backend rejects with `INVALID_STATE_TRANSITION`. | `AdminMutationValidators.java` |
| `DRAFT` | `PENDING` / `PRIVATE` | Not in allowed transition set. | Backend rejects. | `AdminMutationValidators.java` |
| `PUBLISHED` | `DRAFT` | Not allowed directly; must go through supported states if business wants. | Backend rejects. | `AdminMutationValidators.java` |
| `HIDDEN` | `DRAFT` | Not in allowed transition set. | Backend rejects. | `AdminMutationValidators.java` |
| `ARCHIVED` | `PUBLISHED` | Must move to `DRAFT` first per current validator. | Backend rejects. | `AdminMutationValidators.java` |
| `TRASH` | anything except `DRAFT` | Restore from trash lands in draft. | Backend rejects. | `AdminMutationValidators.java` |
| any state | same state | No-op allowed by validator; not a transition. | Backend returns no validation error. | `AdminMutationValidators.java` |
...
| `PENDING` | `CANCELLED` | Admin / `orders.write` | Order exists. | Set `cancelledAt`, restore stock, audit, notification, websocket. | `CONFIRMED_BACKEND_ENFORCED` | `AdminOrderService.java` |
...
| `ON_HOLD` | `CANCELLED` | Admin / `orders.write` | Order exists. | Set `cancelledAt`, restore stock, audit, notification, websocket. | `CONFIRMED_BACKEND_ENFORCED` | `AdminOrderService.java` |
...
| `PROCESSING` | `COMPLETED` | Admin / `orders.write` | Order exists. | Set `completedAt`, audit, notification, websocket. | `CONFIRMED_BACKEND_ENFORCED` | `AdminOrderService.java` |
| `PROCESSING` | `CANCELLED` | Admin / `orders.write` | Order exists. | Set `cancelledAt`, restore stock, audit, notification, websocket. | `CONFIRMED_BACKEND_ENFORCED` | `AdminOrderService.java` |
...
| `COMPLETED` | `REFUNDED` | Admin / `orders.write` | Order exists; transition requested or full refund can set if allowed. | Restore stock when status becomes `REFUNDED`, notification, audit, websocket. | `CONFIRMED_BACKEND_ENFORCED` | `AdminOrderService.java` |

### Forbidden Transitions

| From | To | Reason | Enforcement | Evidence |
|---|---|---|---|---|
| `COMPLETED` | `PENDING` / `PROCESSING` / `ON_HOLD` / `CANCELLED` / `FAILED` | Not in allowed transition map. | Backend throws conflict. | `AdminOrderService.java` |
...
- Refund requires paid/partially paid order and refund amount within refundable amount.
...
| From | To | Actor / Role | Preconditions | Side Effects | Enforcement | Evidence |
|---|---|---|---|---|---|---|
| quantity `> threshold` | `IN_STOCK` | System | Recompute stock state and current state not `PREORDER`/`CONTACT_FOR_STOCK`. | Variant stockState updated. | `CONFIRMED_BACKEND_ENFORCED` | `InventoryPolicyService.java` |
| quantity `1..threshold` | `LOW_STOCK` | System | Recompute stock state and current state not admin-controlled. | Variant stockState updated. | `CONFIRMED_BACKEND_ENFORCED` | `InventoryPolicyService.java` |
| quantity `<= 0` | `OUT_OF_STOCK` | System | Recompute stock state and current state not admin-controlled. | Variant stockState updated. | `CONFIRMED_BACKEND_ENFORCED` | `InventoryPolicyService.java` |
| any computed state | `PREORDER` | Admin | Admin-controlled stock state. | Recompute will not overwrite. | `STATUS_ONLY` / `CONFIRMED_BACKEND_ENFORCED` for no-overwrite behavior | `ProductStockState.java`, `InventoryPolicyService.java` |
| any computed state | `CONTACT_FOR_STOCK` | Admin | Admin-controlled stock state. | Recompute will not overwrite. | `STATUS_ONLY` / `CONFIRMED_BACKEND_ENFORCED` for no-overwrite behavior | `ProductStockState.java`, `InventoryPolicyService.java` |
...
| `PREORDER` / `CONTACT_FOR_STOCK` | computed stock state via recompute | These are admin-controlled and not overwritten by recompute. | Backend policy skip. | `InventoryPolicyService.java` |
...
- Shared `PublishStatus` enum includes `DRAFT`, `PUBLISHED`, `HIDDEN`, `ARCHIVED`, `PENDING`, `PRIVATE`, `TRASH`.
- Admin content controller only accepts list filter statuses: `DRAFT`, `PUBLISHED`, `HIDDEN`, `ARCHIVED`.
- Content mutation service uses shared publish transition validator, so `PENDING`, `PRIVATE`, `TRASH` may be technically possible if DTO accepts enum values. Needs DTO/schema audit.
