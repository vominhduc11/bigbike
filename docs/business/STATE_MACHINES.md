...
| Inventory / Stock | `stockState`, quantity fields | `IN_STOCK`, `LOW_STOCK`, `OUT_OF_STOCK` | Quantity changes luôn recompute stock state tự động. | Backend policy/service | `CONFIRMED_BACKEND_ENFORCED` | `ProductStockState.java`, `InventoryPolicyService.java`, `CheckoutService.java`, `AdminOrderService.java`, `AdminReturnService.java` |
