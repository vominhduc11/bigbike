# Module Catalog

## Public Platform Modules

| Module | Surface | Current implementation | Status | Evidence |
|---|---|---|---|---|
| Catalog browse | web, backend, mobile | Products, categories, brands, articles, pages, settings, menus, sliders, home videos. Nội dung sản phẩm + danh mục + thương hiệu song ngữ Việt/Anh (V136/V137) — đọc theo `lang`, tiếng Anh tùy chọn, fallback về tiếng Việt theo từng trường. Storefront `bigbike-web` dùng next-intl + cookie `NEXT_LOCALE`. Đợt 3 (V137): bilingual category/brand đã hoàn tất. | `CONFIRMED_FROM_CODE` | public controllers, `public-api.ts`, `api_endpoints.dart`, `V136__add_product_bilingual_content.sql`, `V137__add_category_brand_bilingual_content.sql`, `bigbike-web/i18n/request.ts`, `bigbike-web/components/layout/LanguageSwitcher.tsx` |
| Page hero banners | web, backend, admin | Admin-managed hero (image + title + description + kicker) on every public content page. Backed by `PageEntity` for CMS pages and by `public_hero` settings group for listing pages (`/san-pham`, `/brands`, `/tin-tuc`) — each listing page has its own background AND gear illustration, falling back to shared defaults. Managed in admin via the dedicated **Banner trang** screen (`BannerScreen.jsx`) with a live composite preview. | `CONFIRMED_FROM_CODE` | `PageEntity.java` hero fields (V98), `SettingDefinitionRegistry` `hero_*` keys, `BannerScreen.jsx`, `bigbike-web/components/wp/WpCategoryHero.tsx` |
| Search | web, backend, mobile | `GET /api/v1/search` and `GET /api/v1/search-suggest` | `CONFIRMED_FROM_CODE` | `PublicSearchController.java`, `SecurityConfig.java`, clients |
| Cart | web, backend, mobile | Guest/customer cart with CSRF-protected mutations | `CONFIRMED_FROM_CODE` | `CartController.java`, `CartService.java`, tests |
| Checkout | web, backend, mobile | Cart checkout and quick buy with idempotency. No shipping-method or payment-method choice; orders carry no shipping fee (owner decision 2026-06-23, `SHIP_RULE_001`). | `CONFIRMED_FROM_CODE` | `CheckoutService.java`, tests, clients |
| Customer account | web, backend, mobile | Profile, addresses, orders | `CONFIRMED_FROM_CODE` | customer controllers, clients |
| Wishlist | web, backend | Customer wishlist — add/remove products, list product IDs and paginated products. Stored in `wishlist_items` (V103). No mobile client wrapper yet. | `CONFIRMED_FROM_CODE` | `CustomerWishlistController.java`, `WishlistItemEntity.java`, `V103__create_wishlist_items_table.sql` |
| Product comparison | web | Browser-local comparison list — max 3 products, same-category only, no login. Add/remove via compare buttons on product cards and the product detail page; a floating compare bar; `/so-sanh` renders a side-by-side table of specifications, price, rating, stock and variant options. No backend storage (uses `localStorage`); reuses the existing product detail endpoint. | `CONFIRMED_FROM_CODE` | `bigbike-web/lib/compare-context.tsx`, `bigbike-web/lib/compare-storage.ts`, `bigbike-web/components/catalog/ComparisonTable.tsx`, `bigbike-web/app/so-sanh/page.tsx` |
| Vietnam address lookup | web, backend, mobile | Province -> district -> ward lookup | `CONFIRMED_FROM_CODE` | `VnAddressController.java`, web/mobile helpers |

## Admin Platform Modules

| Module | Surface | Current implementation | Status | Evidence |
|---|---|---|---|---|
| Catalog admin | admin, backend | Product/category/brand CRUD and related reads. PDP authoring (V229–V231): mô tả dạng khối song ngữ (Việt + Anh đều dùng trình dựng khối), thông số "đưa lên ô nổi bật" (tối đa 4), và **quản lý tab PDP theo từng sản phẩm** (ẩn/hiện, đổi thứ tự, đổi tên, thêm tab tự do dựng bằng khối) — xem trước bố cục thật qua live preview. | `CONFIRMED_FROM_CODE` | `AdminCatalogController.java`, `ProductDetailScreen.jsx`, `ProductTab`/`ProductTabsConverter`, migrations `V229`–`V231` |
| Order admin | admin, backend | Order list/detail/status/payment/note workflows | `CONFIRMED_FROM_CODE` | `AdminOrderController.java`, admin app |
| Customer admin | admin, backend | Customer list/detail/update | `CONFIRMED_FROM_CODE` | `AdminCustomerController.java` |
| Media admin | admin, backend | Upload/list/detail/update/delete/restore media | `CONFIRMED_FROM_CODE` | `AdminMediaController.java`, `AdminMediaService.java` |
| Settings admin | admin, backend | Site settings read/update | `CONFIRMED_FROM_CODE` | `AdminSettingsController.java`, tests |
| Menu admin | admin, backend | Menu-item CRUD and reorder inside the four system slots (`primary`, `footer`, `guide`, `policy`). The `policy` slot (V226) drives the `/chinh-sach` sidebar. Menu containers themselves are system-defined — admins cannot create new locations or delete the system slots. See `MENUS_SYSTEM_SLOT_FIX_REPORT.md`. | `CONFIRMED_FROM_CODE` | `AdminMenuController.java`, `MenuLocations.java`, `V84__seed_system_menu_slots.sql`, `V226__seed_policy_menu_slot.sql`, tests |
| Inventory admin | admin, backend | Standalone "Kho hàng" admin screen removed 2026-06-23 (grouped list / availability-toggle UI / CSV export gỡ khỏi admin). Còn/Hết is now toggled per-variant inside the product edit form (`isAvailable`, via `products.update`). Backend summary endpoint (`totalItems`/`inStockCount`/`outOfStockCount`) is kept and powers the Dashboard out-of-stock alert (links to Products filtered `stockState=OUT_OF_STOCK`). Grouped-list / `PATCH .../availability` / CSV endpoints remain in `AdminInventoryController` but are no longer surfaced in admin UI. Quantity model removed V261; only Còn/Hết (no "sắp hết"). | `CONFIRMED_FROM_CODE` | `AdminInventoryController.java`, `AdminInventoryService.java` |
| Returns/refunds admin | — | Removed platform-wide 2026-06-23 (return/RMA + refund features dropped). | `REMOVED` | — |
| Redirect admin | admin, backend | Redirect CRUD and internal redirect support | `CONFIRMED_FROM_CODE` | `AdminRedirectController.java`, `SecurityConfig.java` |
| ~~POS~~ | — | **REMOVED (owner decision 2026-06-23, online-only).** The point-of-sale / walk-in module was dropped entirely: admin POS screen, `POST /admin/pos/orders` + `GET /admin/pos/products/search` endpoints, `AdminPosController` / `PosOrderService`, and the `pos.read` / `pos.write` / `pos.price_override` / `pos.sell_below_cost` permissions. BigBike now only takes online orders. | `REMOVED` | — |
| Admin order WebSocket | admin, backend | Subscribe to `/topic/admin/orders` for order events | `CONFIRMED_FROM_CODE` | `WebSocketConfig.java`, `AdminOrderWsService.java`, `adminWebSocket.js` |
| Audit logs admin | admin, backend | Read-only paginated activity log with filters (actorType, resourceType, action, date range). Enriches actor name and resource label. Permission: `audit-logs.read`. | `CONFIRMED_FROM_CODE` | `AdminAuditLogController.java`, `AdminAuditLogService.java` |
| ~~Shipping admin~~ | — | **REMOVED (owner decision 2026-06-23, `SHIP_RULE_001`).** The shipping zone/method CRUD module was dropped entirely: admin screen, `/api/v1/admin/shipping/*` endpoints, `shipping_zones`/`shipping_methods` tables (migration `V264`), and the `shipping.read`/`shipping.write` permissions. Online orders carry no shipping fee. | `REMOVED` | `V264__remove_shipping_methods.sql` |
| Notification center admin | admin, backend | Persistent admin notifications (`admin_notifications`, V102) — list unread, mark-read, mark-all-read. Complements the `/topic/admin/orders` WebSocket feed so offline admins do not miss events. Gated by `orders.read`. | `CONFIRMED_FROM_CODE` | `AdminNotificationController.java`, `AdminNotificationService.java`, `V102__create_admin_notifications_table.sql` |

## Inventory And Receiving Subdomains

| Subdomain | Current implementation | Status | Evidence |
|---|---|---|---|
| Stock movement timeline | **Dormant (V261).** Movement types `IN`/`OUT`/`ADJUSTMENT`/`RETURN` still exist in schema but no movements are written for sales/restores; inventory is a boolean availability toggle. | `CONFIRMED_FROM_CODE` | `AdminInventoryService.java`, `V261__inventory_availability_toggle.sql` |
| Stock movement serials | Removed (serial feature removed platform-wide 2026-06-23); the `stock_movement_serials` table was dropped. Quantity adjustments themselves were also removed (V261 — boolean availability toggle replaced them). | `REMOVED` | `V259__remove_serial_management.sql`, `V261__inventory_availability_toggle.sql` |
| Stock receipt schema | Dropped in V120 (business decision 2026-05-16). Tables were schema-only and never built. (Inventory is now a boolean availability toggle — no receiving flow, V261.) | `REMOVED` | `V120__drop_stock_receipt_tables.sql` |

## Mobile Coverage Notes

| Topic | Current status | Evidence |
|---|---|---|
| Public catalog/search/address/cart/checkout/account endpoints wrapped in mobile client | `CONFIRMED_FROM_CODE` | `bigbike_mobile/lib/core/api/api_endpoints.dart` |
| Verify-email implemented in mobile (`VerifyEmailScreen` + route `/xac-nhan-email`; sau đăng ký điều hướng tới màn xác nhận; nút gửi lại gọi `resend-verification`) | `CONFIRMED_FROM_CODE` | `bigbike_mobile/lib/features/auth/verify_email_screen.dart`, `app_router.dart`, `api_endpoints.dart`, `CustomerAuthController.java` |
| Home-videos wrapper missing | `CODE_ONLY_NOT_DOCUMENTED` | `SecurityConfig.java`, `bigbike-openapi.json`, `api_endpoints.dart` |
