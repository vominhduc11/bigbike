...
| Notification center (admin read/unread) | Persistent `admin_notifications` table (V102); `AdminNotificationService` + `AdminNotificationController` provide a permission-scoped recent list, exact unread count and per-admin mark-all-read. WS push supplements persistent store; a daily job removes rows older than six months without changing per-admin read markers. | `CONFIRMED_FROM_CODE` | `AdminNotificationController.java`, `AdminNotificationRetentionCleanupService.java`, `NOTIFICATION_RULE_001` |
| Backup / restore / data retention runbook | **Built and proven 2026-09-06.** Sao lưu tự động ra NAS riêng của chủ shop qua mạng riêng Tailscale: dữ liệu bán hàng mỗi giờ, kho ảnh và cấu hình vận hành mỗi ngày. Giữ 48 bản giờ / 30 bản ngày / 12 bản tháng, bản quá hạn tự xoá. Mỗi bản có dấu kiểm tra và được đọc ngược lại từ NAS để xác nhận. Hỏng lượt nào, hoặc quá 24 giờ không có bản mới, đều tự báo động qua Telegram và hộp thư nội bộ; thêm một tin tổng kết mỗi sáng để việc sao lưu không thể chết âm thầm. Đã diễn tập khôi phục thật cho cả dữ liệu bán hàng lẫn kho ảnh, không đụng dữ liệu đang chạy. | `CONFIRMED_FROM_CODE_AND_TEST` | `BACKUP_RESTORE_RUNBOOK.md`, `scripts/ops/backup-to-nas.sh`, `scripts/ops/restore-from-nas.sh`, `docs/audits/BACKUP_NAS_SETUP_2026-09-06.md` |
