...
|---|---|---|---|
| Invoice / e-invoice (hóa đơn điện tử) | No `invoice` entity, no service, no provider integration (Misa/VNPT/SInvoice/Easyinvoice). Order cancellation does not affect any invoice. | `NOT_FOUND_IN_REPO` | Required by Nghị định 123/2020/NĐ-CP for legal entities selling retail. NEEDS_BUSINESS_CONFIRMATION on which provider to integrate. |
| Bank-transfer manual reconcile (BACS) | Online checkout chỉ hỗ trợ `COD` và `BACS` (`CheckoutService.ALLOWED_PAYMENT_METHODS`). Provider là `INTERNAL`; admin patch `paymentStatus`/`paidAmount` thủ công sau khi nhận tiền. | `MANUAL_BY_DESIGN` | Risk còn lại: data-entry errors khi đối soát thủ công COD/BACS. |
| External payment provider / webhook | Không có cổng thanh toán tự động. Kế hoạch tích hợp Alepay/ZaloPay đã bị bỏ — checkout không còn chấp nhận `ALEPAY`/`ZALOPAY`, không có redirect, không có webhook. | `NOT_FOUND_IN_REPO` | Mọi tiền vào đều do admin đối soát thủ công. Xem `INTEGRATION_GUIDE.md`. |
