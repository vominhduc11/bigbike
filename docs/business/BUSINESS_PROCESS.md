...
| Legacy bank-transfer reconcile (BACS) | Storefront mới chỉ nhận `COD`; `BACS` bị từ chối ở checkout/quick-buy. Admin vẫn đọc và đối soát thủ công các đơn BACS cũ qua `paymentStatus`/`paidAmount`. | `LEGACY_COMPATIBILITY` | Không quảng bá BACS như lựa chọn thanh toán mới; giữ khả năng vận hành dữ liệu cũ. |
