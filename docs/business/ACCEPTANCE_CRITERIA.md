...
| Brand logo standardization | New/replaced brand logos use the admin 1:1 crop flow when needed and are saved only as transparent PNG, at least 400×400 and at most 300 KB; the backend rechecks the stored object. Existing logos remain usable with non-blocking quality warnings, and missing public logos render initials instead of the BigBike logo. | `MEDIA_RULE_011`/`MEDIA_RULE_012`, `BRAND_RULE_006`, `DATA_CONTRACT.md` “Brand logo quality and storage marker”, Brand admin/web/backend tests | `REQUIRED_FOR_2026-08-28` |
