...
| Notification center (admin read/unread) | Persistent `admin_notifications` table (V102); đã-đọc riêng từng admin qua `admin_notification_reads` (V339, AUD-018/019); `AdminNotificationController` với list + mark-all-read (endpoint mark-read từng cái đã gỡ 2026-07-15, AUD-067). | `CONFIRMED_FROM_CODE` | `AdminNotificationController.java`, `V339__admin_notification_reads.sql` |
