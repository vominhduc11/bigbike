...
| `GET /api/v1/admin/products/import/export` | Round-trip export: the full 43-column template shape (one row per product/variant, category/brand emitted as slug) for "download current catalog → edit → re-import." Deliberately separate from `GET /api/v1/admin/reports/products/export` (13-column reporting overview, gated by `reports.export` instead). |

**CSV row shape** mirrors `product-import-template.csv`: one "SẢN PHẨM CHÍNH" row per product plus zero or more "BIẾN THỂ" child rows grouped by an internal file-local key column (`Mã sản phẩm nội bộ`, column 2) — see the template file itself for the full 43-column layout. Five display-only/never-read columns from the pre-2026-07-04 46-column layout were removed: `Mã biến thể nội bộ` (variant identity in CSV is always SKU-matched, never id-matched — see below), `Danh mục (tên hiển thị, chỉ để đối chiếu)` / `Thương hiệu (tên hiển thị, chỉ để đối chiếu)` (only the slug columns are parsed), `Tên biến thể hiển thị (hệ thống tự sinh)` (server-derived, `PRODUCT_RULE_008`), and `Đơn vị tiền tệ` (always VND, `PRODUCT_RULE_011`).

**Related/accessory products — columns 42 and 43 (2026-07-05):** `Sản phẩm liên quan (SKU, nhiều SKU cách nhau bằng |)` and `Phụ kiện bán kèm (SKU, nhiều SKU cách nhau bằng |)` (main product row only, blank for variant rows). Each CSV cell is a `|`-separated list of tokens copied straight into `relatedProductIds`/`accessoryProductIds` as raw text at CSV-parse time; **actual resolution happens later in `processRow`, shared with the JSON path** — see `resolveRelatedAndAccessoryRefs` below.

**JSON row shape** is `UpsertProductRequest` byte-for-byte, deserialized directly (no separate import DTO) — **except** `categoryId`/`brandId` are interpreted as **slugs**, not literal database IDs, identically to the CSV path (neither a shop owner nor an LLM can know real internal IDs). **`relatedProductIds`/`accessoryProductIds` get the same treatment (2026-07-05):** since a shop owner/LLM authoring JSON by hand can't know real IDs either, `processRow` re-resolves every entry in both arrays — for **both** the CSV and JSON paths — by trying, per token: exact product ID (so existing literal-ID JSON payloads keep working unchanged) → SKU (case-insensitive) → slug. A token that doesn't resolve to exactly one product (not found, or a SKU shared by more than one product) is dropped with a soft `NOT_FOUND`/`AMBIGUOUS` warning rather than failing the row. **Neither path can use this mechanism to explicitly clear an existing list to empty:** if the original array was non-empty but every token failed to resolve, the field is reset to untouched (matches `buildGalleryList`'s same all-invalid-entries fallback for image columns) rather than wiping it to `[]` — only a genuinely empty input (JSON literal `[]`, or a CSV cell with no tokens at all) passes through as a real clear. **Ordering matters for cross-referencing products new to this same file:** rows commit sequentially and each commit is a real, immediately-queryable save (see "one bad row never rolls back rows already saved" above), so a row can reference the SKU/ID of a product defined earlier in the same file, but not one defined later. `validateImport` (dry-run) runs the identical resolution logic against the database as it stands *before* the file is committed, so a forward reference that will succeed on commit may still show a `NOT_FOUND` warning during preview — expected, not a bug.
...
**Description → block conversion (2026-07-05, CSV path only):** the CSV `Mô tả chi tiết` columns (vi/en, raw HTML) are parsed with `BodyBlockParser` and written into `descriptionBlocks`/`descriptionBlocksEn` alongside the raw `description`/`translations.en.description` string, so imported products get the same structured block-editor content a manually-authored product would (editable block-by-block in admin afterward) instead of only the opaque legacy HTML string. This mirrors the presence-flag pipeline in `AdminCatalogMutationService.applyProductPatch` — sending the parsed blocks makes the renderer own the `description`/`description_en` columns for that row (see "Product description blocks — `descriptionBlocks` (V139)" below). The **JSON row shape is unaffected** — it is `UpsertProductRequest` deserialized byte-for-byte, so a JSON row must include `descriptionBlocks`/`descriptionBlocksEn` itself to get block content; omitting them keeps the legacy HTML-only path exactly as before.

`BodyBlockParser` also recognizes `<div class="bb-feature">` (the staggered image+text "so le" layout) and decomposes it into a `FeatureBlock` (image url/alt, eyebrow, heading, paragraphs, list). As of 2026-07-05, `FeatureBlock.url` is no longer `@NotBlank` (see "Product description blocks" below) — a div with real text but a placeholder `<img>` (no `src`, drafted before photos are shot) still becomes an editable **text-only** feature block; the web renders it full-width with no image column, and `DescriptionBlockRenderer` omits the `<figure>/<img>` from the legacy HTML entirely rather than emitting an empty `src`. The div only falls back to opaque HTML when it has neither a real image nor any text at all (matches the admin block editor's own save-time filter in `product-detail/constants.js`).

Status: `CONFIRMED_FROM_CODE`

Evidence: `AdminProductImportController.java`, `ProductImportService.java` (`bodyBlockParser.parseHtmlToBlocks`, `resolveRelatedAndAccessoryRefs`, `resolveProductRefTokens`), `ApiErrorDetail`/`ImportRowResult`/`ImportReportResponse`, `ProductJpaRepository.findAllBySkuIgnoreCase`/`findBySlug`/`findById`, `ProductVariantJpaRepository.findBySkuIgnoreCase`, `V30__add_inventory_tracking.sql` (`stock_movements` cascade), `BodyBlockParser.java`. See `BUSINESS_RULES.md` `PRODUCT_RULE_009`.
