...
`GET /api/v1/admin/admin-users` accepts `q` for accent-insensitive, case-insensitive, literal token matching across display name and email. `%`, `_` and `\\` are literal characters so the quick-search “Xem tất cả” link cannot widen the result set.
...
Public catalog/facet search keeps its existing case-insensitive, accent-insensitive token-AND semantics. The admin product list, export and quick-search path use the same identifier fields but retain every entered word (including Vietnamese words that are stop words for the assistant); each entered token must match at least one of the Vietnamese/English product name, Vietnamese/English slug, product SKU, or variant selling SKU. Short descriptions, detailed descriptions, structured description blocks, SEO copy, and other article/content fields are explicitly outside the search scope. The assistant's stop-word behavior remains isolated to its own `ProductSearchTerms.tokens` path.
...
The optional `q` search is accent-insensitive and keeps every entered token; it matches the article title, English title, slug, English slug, excerpt or English excerpt. `%`, `_` and `\\` are literal characters.
...
- **Ảnh thương hiệu** (`POST/PATCH /api/v1/admin/brands`): 2 field ảnh đơn `logo`, `banner`. `banner` dùng shape `ImageAssetRequest`: `url`, `alt`, `width`, `height`, `mimeType`. `logo` nhận thêm `mediaId` tùy chọn để liên kết bản ghi media nội bộ. Cả 2 đi qua whitelist kho ảnh MinIO theo `BUSINESS_RULES.md` `MEDIA_RULE_002`, có tha URL cũ đang lưu trên bản ghi; URL mới ngoài `/media/`·`/media-proxy/`·base MinIO bị chặn `400 INVALID_VALUE` với `field` = `logo.url`/`banner.url`. Riêng logo mới hoặc đổi URL nhận JPEG/JPG, PNG hoặc WebP, tối thiểu 400×400 và tỉ lệ 1:1 sai lệch tối đa 1% theo `BUSINESS_RULES.md` `MEDIA_RULE_011`; không có trần dung lượng riêng của logo, còn giới hạn chung Media Library vẫn áp dụng cho file tải trực tiếp. Một chiều nhỏ hơn 400 bị chặn. Nền trong suốt chỉ là cảnh báo chất lượng, không khóa lưu. Máy chủ kiểm bytes/metadata thực tế của media, không tin metadata request. URL cũ không đổi được grandfathered để sửa field khác. Gửi `url: ""` để xóa ảnh. **`alt` phải được gửi kèm mỗi lần lưu**: field không gửi bị ghi `null`, nên form quản trị luôn gửi cả `url` và `alt` (audit 2026-07-28 — trước đó form bỏ `alt`, làm mất chữ thay-thế-ảnh admin vừa nhập ngay ở lần lưu đó).
- **Nhập logo từ URL** (`POST /api/v1/admin/brands/logo/import-url`): cần `media.write`, body `{ "url": "https://...", "altText"?, "folderId"? }`, trả `201` với `ApiDataResponse<AdminMediaDetailResponse>`. Máy chủ chỉ nhận `http`/`https`, chặn địa chỉ nội bộ/metadata và SSRF, kiểm tra lại redirect, giới hạn nguồn tải về ở 10 MB và xác minh JPEG/PNG/WebP bằng content thực tế. Ảnh nhập có thể chưa vuông để admin đưa qua khung cắt; ảnh có một chiều nhỏ hơn 400 bị từ chối. Máy chủ tải object vào MinIO và chỉ trả `mediaId`/URL nội bộ, không bao giờ lưu URL website ngoài vào Brand. Endpoint dùng cùng hạn mức và giới hạn request đang áp dụng cho media admin.
...
|---|---|
| `POST /api/v1/admin/media` | `multipart/form-data`: `file` bắt buộc, `altText` tùy chọn. Nhận JPEG/PNG/WebP/MP4, tối đa **200 MB**; file rỗng, MIME khai báo không thuộc allowlist hoặc content detection không khớp chính xác MIME khai báo → `400 VALIDATION_ERROR`. Không tin phần mở rộng; GIF/SVG đổi tên thành `.jpg` vẫn bị từ chối. Ảnh raster được nén theo `MEDIA_RULE_006`; upload trùng checksum tái sử dụng bản ghi có sẵn và đưa về `ACTIVE`. |
| `GET /api/v1/admin/media` | Danh sách phân trang. Query: `page` (1..), `size` (1..100), `q`, `mimeType` (lọc theo tiền tố tương thích), `mimeTypes` (CSV các MIME chính xác, dùng cho bộ chọn ảnh/video), `status`, `storageProvider`, `usageFilter=USED|UNUSED`, `uploadedFrom`, `uploadedTo`, `minSize`, `maxSize`, `minWidth`, `minHeight`, `sort=createdAt|fileSize|title|usageCount`, `dir=asc|desc`, `folderFilter=<uuid>|NONE`, `tag`. Không truyền `status` thì loại `DELETED`. |
...
`GET /api/v1/admin/quick-search?q=...` phục vụ ô tìm kiếm nhanh trên thanh trên của Admin. Đây là endpoint chỉ đọc, dùng envelope chuẩn `ApiDataResponse<AdminQuickSearchResponse>`, và yêu cầu ít nhất một quyền đọc trong `orders.read`, `products.read`, `customers.read`, `catalog.read`, `content.read` hoặc `admin-users.read`. Người có quyền một phần chỉ nhận các nhóm tương ứng; nếu không có quyền nào, server trả `403`.

`q` là từ khóa gốc nhân viên nhập, sau khi trim phải có ít nhất một ký tự và tối đa 100 ký tự. Chuẩn hóa tìm kiếm bỏ dấu tiếng Việt, không phân biệt hoa thường và escape `\\`, `%`, `_` theo nghĩa đen. Endpoint không dùng bộ từ bỏ qua của Trợ lý BigBike.

`data.groups` là map tùy quyền với các key `orders`, `products`, `customers`, `categories`, `brands`, `articles`, `adminUsers`. Mỗi nhóm có shape:
...
`state` là `READY` hoặc `ERROR`; `total` là tổng chính xác của nhóm, hoặc `null` khi nhóm lỗi; `items` tối đa năm phần tử. Khi `ERROR`, `errorCode` chỉ là mã ổn định để client dịch thông báo, không trả exception hoặc thông tin truy vấn nội bộ. Nhóm mà người dùng không có quyền được bỏ khỏi map hoàn toàn; nhóm được phép nhưng gặp lỗi vẫn phải giữ lại với `state=ERROR`.

Các trường kết quả tối thiểu:

- `orders`: `id`, `orderNumber`, `status`, `customerName`, `shippingRecipientName`, `customerEmail`, `customerPhone`, `totalAmount`, `currency` và `matchedField`.
- `products`: `id`, `name`, `sku`, `matchedField` và `matchedVariants[]`; mỗi phần tử phiên bản có `id`, `sku`, `name` dẫn xuất và `options[]`.
- `customers`: `id`, `displayName`, `email`, `phone`, `status` và `matchedField`.
- `categories`/`brands`: `id`, tên, slug và `matchedField`.
- `articles`: `id`, tiêu đề, slug và `matchedField`.
- `adminUsers`: `id`, `displayName`, email, role, status và `matchedField`.

Mọi nhóm xếp theo hạng khớp toàn bộ → khớp từ đầu → khớp bên trong chuỗi, sau đó theo giá trị hiển thị đã chuẩn hóa và ID ổn định. Không dùng `createdAt` làm tiêu chí ưu tiên của kết quả nhanh. Kết quả sản phẩm được tính theo số sản phẩm, dù một sản phẩm có thể có nhiều phiên bản khớp.

**Đường sang danh sách:** client truyền nguyên từ khóa qua `?search=...` tới list tương ứng: `/admin/orders`, `/admin/products`, `/admin/customers`, `/admin/categories`, `/admin/brands`, `/admin/content` hoặc `/admin/admin-users`. Các API list phải áp dụng cùng chuẩn bỏ dấu/escape và trường tìm kiếm để kết quả “Xem tất cả” không khác kết quả nhanh. Không thay đổi payload list sản phẩm hiện có; `matchedVariants` chỉ thuộc response quick search.

Status: `OWNER_CONFIRMED_FROM_HANDOFF_2026-08-28`

Evidence: `AdminQuickSearchController`, `AdminQuickSearchService`, `AdminOrderSupport`, `AdminCustomerService`, `ProductFilterSpecifications`, `ProductSearchTerms`, `PERMISSION_MATRIX.md`, `DATA_CONTRACT.md`.
...
| Method | Path | Permission | Current behavior | Status | Evidence |
|---|---|---|---|---|---|
| `GET` | `/api/v1/admin/customers` | `customers.read` | Paginated customer list for the admin Customers screen. Query: `page` (≥1, default 1), `size` (1-100, default 20), `q` (matches display name, first/last name, email or phone with accent-insensitive substring semantics), `status` (one of the Customer Status Enum values below; unknown values return `400 VALIDATION_ERROR`), `synthetic` (boolean — filters on `isSynthetic`, see below), `emailVerified` (boolean — `true` = `emailVerifiedAt IS NOT NULL`, `false` = `IS NULL`). Each item includes `orderCount`/`totalSpent` aggregated in one per-page query (no N+1) and excluding `CANCELLED` orders. `%`, `_` and `\\` are literal search characters. | `CONFIRMED_BACKEND_ENFORCED` | `CUSTOMER_RULE_001`, `CUSTOMER_RULE_006`, `ADMIN_SEARCH_RULE_002`, `ADMIN_SEARCH_RULE_005`, `AdminCustomerController.java`, `AdminCustomerService.listCustomers` |
...
Response `data`: `{ mode, reason, greeting, quickPrompts, maxTurns, contacts, images }`.

- `mode`: `AI|CONTACT`; `reason` là mã an toàn public.
- `contacts`: `{hotline,zaloUrl,messengerUrl,zaloDisplay,messengerDisplay}` từ setting contact; chỉ hiện panel khi khách bấm Gặp nhân viên.
- `images`: `{enabled,maxBytes,maxPerTurn,maxPerConversation,dailyLimit,disclosure}`. Các giới hạn cố định là 8 MB, 1/lượt, 3/hội thoại, 20/ngày.
...
Response `data`: `{ conversationId, assistantMessageId, mode, reason, answer, answerFormat, resultKind, turnCount, maxTurns, remainingTurns, products, crossSellProducts, images, clarification, nextStep, handoff, handoffRecommended, actions, contacts, channelState, continuation }`.

- `actions` chỉ là allowlist server-owned; `CONTACT_STAFF` mở panel Gặp nhân viên. Không có `leadOffer`, `leadPrompt`, interaction id hoặc proof gắn đơn.
- `handoff` có `{id,status,requestedAt,assignedDisplayName?,withinBusinessHours,nextOpenAt?,businessHoursText}`. `WAITING` không khóa input; `ACTIVE` chuyển ô nhập cho nhân viên.
- Customer turn có trần owner đặt (mặc định 40); clarification/retry/staff/system không tính. Gần trần có handoff hoặc continuation cùng thread với context đã làm sạch, không trả hard `TURN_LIMIT`.
- Transaction không giữ lock/connection trong lúc provider chờ. Storefront hủy chờ sau 75 giây và luôn giữ nút Gặp nhân viên.
...
Một logical turn có deadline backend 65 giây và tối đa bốn lần gọi provider. Khi timeout, quá tải, `429`, `5xx`, lỗi mạng hoặc payload rỗng/không hợp lệ, backend chỉ thử lại **Gemini 3.7 Flash** khi còn budget. Không đổi model và không retry để né safety/content refusal. Nếu hết budget, response là lời xin lỗi lịch sự có action `CONTACT_STAFF`; không tự tạo handoff.
...
Body, ownership, idempotency và result giống endpoint thường. Server chỉ phát `progress` mã `UNDERSTANDING|CHECKING_PRODUCTS|FINALIZING` rồi đúng một event `result` với response hoàn chỉnh đã qua guard. Không stream token/model draft, telemetry hoặc nội dung chưa kiểm duyệt.

### `POST /api/v1/chat/handoffs`

Body: `{ requestId, conversationId?, locale:"vi|en", trigger:"BUTTON|MESSAGE", visitorToken? }`. Nếu chưa có conversation, backend tạo conversation 0 lượt thuộc caller; nếu có thì xác minh ownership customer/visitor. Retry trả row đang sống. Persist trước, sau commit mới push admin realtime và queue email. Ngoài giờ trả lịch/lần mở cửa kế tiếp, không yêu cầu hoặc lưu thông tin liên hệ. Endpoint không gọi AI/tăng turn.

### Public session, history, image và realtime
...
| `GET` | `/api/v1/chat/conversations/{id}/messages?afterSequence=0` | Chỉ own customer hoặc signed visitor; trả history, staff label, channel state, handoff, continuation và image metadata an toàn. |
...
| `POST` | `/api/v1/chat/realtime-token` | Body `{conversationId,visitorToken}`; token 5 phút chỉ subscribe đúng customer queue. |
| `POST multipart` | `/api/v1/chat/images` | `requestId`, optional `conversationId`, `lang`, file. Chỉ khi image feature bật; validate ownership/idempotency/JPG-PNG-WebP/MIME-decode/re-encode/1 ảnh/8 MB/3 hội thoại/20 ngày, lưu private. |
| `GET` | `/api/v1/chat/images/{id}/content` | Đúng customer/visitor token; stream `private,no-store`, không presign/public URL. |

Image message attach dùng `imageIds` trên message endpoint. Ảnh `PENDING` phải thuộc caller/conversation và chưa dùng lần khác; retry không tính lại quota vision. History/response image metadata không có object key, filename hoặc hash. Mã lỗi public: `CHAT_IMAGE_DISABLED`, `CHAT_IMAGE_TOO_LARGE`, `CHAT_IMAGE_UNSUPPORTED_TYPE`, `CHAT_IMAGE_INVALID`, `CHAT_IMAGE_TURN_LIMIT`, `CHAT_IMAGE_CONVERSATION_LIMIT`, `CHAT_IMAGE_NOT_FOUND`.

### Admin chat history và handoff

| Method | Path | Permission / contract |
|---|---|---|
| `GET` | `/api/v1/admin/chat/conversations` | `chat.read`; pagination/date filter. Item chỉ có summary hội thoại, channel/handoff state, turn/quota-relevant result summary và timestamps; không lead, revenue, attribution, model/cost/token/latency telemetry. |
| `GET` | `/api/v1/admin/chat/conversations/{id}` | `chat.read`; conversation/messages/images/handoff và source/result kind cần xử lý, không contact lead, feedback hay attribution/telemetry model. |
...
| `GET` | `/api/v1/admin/chat/handoffs` | `chat.read`; `{waitingCount,items}` cho `WAITING|ACTIVE`, oldest first, waiting seconds/assignee/business-hours state. |
| `POST` | `/api/v1/admin/chat/handoffs/{id}/claim` | `chat.reply`; conditional `WAITING → ACTIVE`; concurrent claim trả `409` cùng tên người đang nhận. |
| `POST` | `/api/v1/admin/chat/conversations/{id}/messages` | `chat.reply`; body `{requestId,content}`; chỉ current assignee `ACTIVE`, persist `STAFF` rồi push customer realtime. |
| `POST` | `/api/v1/admin/chat/handoffs/{id}/return-to-ai` | `chat.reply`; `ACTIVE → RETURNED_TO_AI`. |
| `POST` | `/api/v1/admin/chat/handoffs/{id}/close` | `chat.reply`; `ACTIVE → CLOSED`; optional locale. |
| `GET` | `/api/v1/admin/chat/images/{id}/content` | `chat.read`; stream private bytes. |
| `POST` | `/api/v1/admin/chat/product-image-index/rebuild` | `settings.write`; rebuild cục bộ idempotent; không gọi model/không ghi chi phí. |
| `GET` | `/api/v1/admin/chat/product-image-index` | `settings.read`; indexed/eligible/missing/failed/version/last run, không vector/cost/model. |
...
| `ai_assistant_conversation_turn_limit` | `INTEGER` | `40`; owner chỉnh 10–100. |
...
| `ai_assistant_greeting` | `LONG_TEXT` | Câu chào VI/EN. |
| `ai_assistant_quick_prompts` | `LONG_TEXT` | 3–4 nút gợi ý VI/EN. |
| `ai_assistant_handoff_email_enabled` | `BOOLEAN` | `true`; không ảnh hưởng realtime/hàng chờ. |
| `ai_assistant_handoff_email_recipient` | `STRING` | email override hợp lệ hoặc blank dùng `BIGBIKE_MAIL_ADMIN`. |
| `ai_assistant_business_hours` | `JSON` | Lịch hàng chờ theo `Asia/Ho_Chi_Minh`; invalid fail-closed. |
| `ai_assistant_image_enabled` | `BOOLEAN` | `false`; công tắc ảnh độc lập với chat chữ. |

Các key không `publicAllowed`. Không có setting model, fallback, pricing/cost warning, model evaluation, lead capture, proactive, template/abbreviation editor, memory days hoặc image quota owner-editable.
