...
| `GET /api/v1/admin/products/import/export` | Full JSON export: the current catalog serialized as a `ProductImportRow[]` JSON array for "download current catalog → inspect/edit → re-import". Export includes `publishStatus`, product `image`/`gallery`/`videos`, variant `id`/cover media/`gallery`, `relatedProductIds`, and `accessoryProductIds`. Category/brand are emitted as slug; related/accessory products are emitted as SKU. On re-import, bulk import still saves Draft and deliberately ignores the export-only/media/relation groups listed above. Served as `application/json`, filename `bigbike-products.json`. Deliberately separate from `GET /api/v1/admin/reports/products/export` (CSV reporting overview, gated by `reports.export` instead). |
