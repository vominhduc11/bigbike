...
Sau availability/master/quota gate, greeting/help, FAQ rõ ràng, shop-info rõ ràng, đơn của chính khách đã đăng nhập, lead decline, guest-order, handoff, off-topic xác định được và câu mơ hồ (so sánh/đổi ngân sách chưa có đủ dữ kiện) trả bằng fast-path backend, không gọi Gemini. Tin tức/bài viết không thuộc phạm vi Bi cũng là off-topic, không được đổi thành tìm sản phẩm. Các câu hỏi còn lại gửi câu hỏi hiện tại cùng function declarations cố định và danh sách nhỏ tên/mã chuẩn của nhóm hàng, thương hiệu công khai hiện hành; không gửi sản phẩm, giá, tồn kho, PII hay lịch sử hội thoại. Ngoại lệ nội bộ duy nhất là `RECENT_VERIFIED_PRODUCTS`, tối đa ba slug trần từ card đã trả ở lượt gần nhất; nó chỉ cho phép `get_product` đúng slug trong allowlist và backend kiểm tra lại trước khi thực thi. Tham chiếu “mẫu này/cái này/nó/this one/it” có đúng một slug thì được resolve trực tiếp bằng dữ liệu sản phẩm đã xác minh; có nhiều slug thì backend hỏi khách chọn tên mẫu, không gọi AI để đoán. Model tự hiểu ý định và chọn tool, backend không phân loại trước một “expected tool” thay model mà chỉ kiểm tra call có liên quan đến câu hỏi, đúng schema/quyền/giới hạn rồi mới thực thi. Khi công tắc `ai_assistant_search_ai_interpretation_enabled=true`, `search_products` dùng tham số AI đã được đối chiếu: giá luôn lấy từ parser câu khách hoặc context giá hợp lệ; giá AI tự đặt/sai bị bỏ. Danh mục/thương hiệu phải khớp metadata công khai duy nhất, không khớp thì bỏ riêng; tên/model/mã mới do AI đưa, màu hoặc size không có trong câu khách cũng bị bỏ riêng. Nếu sau đó không còn điều kiện tìm an toàn, tool trả yêu cầu làm rõ thay vì quét rộng. Sắp xếp vẫn do backend quyết theo `CHAT_RULE_016`; mọi lượt nới phạm vi giữ disclosure bắt buộc theo `CHAT_RULE_018`. Khi công tắc tắt, `search_products` chạy chính xác cơ chế kiểm chứng cũ. Backend có thể giữ context rút gọn, không PII của phiên (`category`, `brand`, dải giá, slug card công khai và cờ chờ đăng nhập) để ràng buộc tool của lượt tiếp theo; khi khách đổi danh mục thì các filter/slugs kế thừa phải bị xoá trừ phần khách nêu lại. Nếu bộ lọc kế thừa tạo kết quả rỗng, backend có thể bỏ riêng bộ lọc đó, chạy lại phạm vi hiện tại và phải nói rõ; không được bỏ lọc của lượt hiện tại. Nếu dùng lại dải giá cho câu hỏi sản phẩm tiếp nối, answer phải nói rõ dải đó đang được áp dụng. Chỉ các bộ lọc đã áp dụng và kết quả tối thiểu của tool đi trong `functionResponse`, không có lịch sử conversation hay câu khách cũ. Không gửi catalog dump, customer identity hoặc dữ liệu DB ngoài function response tối thiểu của tool vừa chạy.

Function registry có sáu tool: `search_products`, `list_categories`, `get_product`, `get_policy`, `get_shop_info`, `get_my_orders`. `list_categories` không có argument, chỉ trả mỗi nhóm hàng công khai `{name,sellableProductCount}`; tổng tính từ hàng đang bán, không kèm sản phẩm/giá/tồn kho chi tiết. `capture_lead` không expose cho Gemini; ghi lead vẫn chỉ qua endpoint consent bên dưới. Backend reject unknown tool, parallel calls, JSON/kiểu sai, argument ngoài allowlist, SQL/table/column, và mọi identity do model truyền. Một lượt orchestration thực thi tối đa 2 tool và tối đa 3 Gemini provider requests; lần thứ hai chỉ được dùng cho `search_products -> get_product`. Guard sửa cục bộ câu xưng hô/số liệu còn có căn cứ trước khi từ chối; recovery ưu tiên câu trả lời deterministic từ kết quả tool đã xác minh trước `CONTACT_FALLBACK`, đồng thời chỉ ghi log chẩn đoán không chứa nội dung/PII. Sau một `search_products` đã xác minh, backend có thể kết thúc lượt bằng câu trả lời deterministic `TOOL` cho số tổng/card, chi tiết một mẫu, không tìm thấy đúng tên/model/slug, hoặc tầm giá không có hàng kèm phương án gần nhất và disclosure. Trường hợp này không dùng prose cuối của model và không đổi public response shape; nếu đã gọi provider để chọn tool thì vẫn tính một logical AI call. Khi backend phải bỏ tầm giá hoặc mở rộng tìm kiếm theo `CHAT_RULE_018`, tool gắn cờ disclosure bắt buộc; `ChatResponseGuard` từ chối final không nói rõ việc này thay vì để model che mất bộ lọc đã nới. Final do model không được suy ra “tất cả”, “chỉ có N” hoặc không có toàn kho từ tối đa ba card; chỉ terminal outcome đã hậu kiểm mới được kết luận đúng tên/model. Tên mẫu trong prose chỉ lấy từ card/kết quả tool và tối đa ba tên; prose không nhắc giá ngoài card. `mode="AI"` vẫn có thể kèm câu xin lỗi/fallback an toàn khi provider, công cụ hoặc guard gặp vấp kỹ thuật; trường hợp này không đóng hội thoại và không trừ lượt. `mode="CONTACT"` nghĩa là hội thoại đã đóng, client phải khoá ô nhập và nút gửi. Setting/config/quota không khả dụng, chuyển nhân viên theo `handoffRecommended`, hai lượt lệch chủ đề liên tiếp hoặc hết lượt đều trả `CONTACT`; lỗi nội bộ và end reason chi tiết không được lộ ra storefront.
