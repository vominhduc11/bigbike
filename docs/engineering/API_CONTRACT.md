...
Body có `@Valid`: `{ "conversationId": "uuid|null", "requestId": "uuid|null", "message": "1..1000 ký tự", "lang": "vi|en" }`. `conversationId` bỏ trống ở lượt đầu; `requestId` tùy chọn để tương thích client cũ nhưng client mới phải sinh một UUID cho mỗi logical turn và giữ nguyên khi retry. Cùng `requestId` trả lại đúng kết quả đã lưu, không tăng lượt/quota, không gọi provider hoặc tạo tin nhắn lần hai. Mỗi conversation tối đa 12 customer messages. Backend tuần tự hoá thao tác ghi của cùng conversation; các hội thoại khác nhau chạy đồng thời. Deadline toàn lượt backend là 65 giây; widget hủy chờ sau 75 giây và không ẩn nút Gặp nhân viên.

Response `data`: `{ conversationId, mode, reason, answer, answerFormat, resultKind, turnCount, maxTurns, remainingTurns, products, handoffRecommended, leadPrompt, actions, contacts }`. `answerFormat` thuộc `PLAIN_TEXT|MARKDOWN`; `resultKind` thuộc `ANSWER|PRODUCT_RESULTS|CLARIFICATION|REFUSAL|CONTACT`. Markdown chỉ cho đoạn văn, in đậm, danh sách và bảng; cấm HTML, mã, ảnh, link và URL. `products` tối đa 8 phần tử `{slug,name,imageUrl,retailPrice,salePrice,currency,stockState}` và chỉ đến từ catalog đang bán. So sánh trực tiếp tối đa ba mẫu. `leadPrompt=true` tối đa một lần/conversation. `actions` là danh sách hữu hạn do backend tạo, không nhận URL từ model. `CONTACT` luôn kèm `contacts`; quota/provider/config failure không trả 5xx cho widget.

Widget storefront giữ snapshot tối thiểu theo `CHAT_RULE_026`, gồm `conversationId`, `requestId` đang chờ/thử lại, tin nhắn đã hiển thị, sản phẩm/actions, `remainingTurns`, trạng thái `AI|CONTACT` và cờ lời mời liên hệ. Snapshot hết hạn cố định sau 24 giờ, xoá khi khách bấm xoá hoặc đăng xuất. Retry gửi lại đúng `conversationId` và `requestId`; khôi phục không gọi lại AI hoặc phát lại lời mời liên hệ.
...
`POST /api/v1/cart/items` nhận thêm `assistantConversationId: uuid|null`. Backend chỉ lưu nguồn cho dòng giỏ khi sản phẩm thật sự đã xuất hiện trong assistant message của đúng hội thoại/caller; giá trị không hợp lệ bị bỏ, không mở quyền đọc chat. Khi `POST /api/v1/checkout` thành công, backend ghi attribution theo từng order line từ chat bằng giá cuối cùng đã xác nhận. Phát lại checkout không tạo bản ghi attribution trùng.
...
| `GET` | `/api/v1/admin/chat/stats` | `date=YYYY-MM-DD` tùy chọn; trả `{date,aiCalls,conversations,leads,unanswered,contentRefusals,dailyLimit,remainingAiCalls,inputTokens,outputTokens,thinkingTokens,providerRequests,averageLatencyMs,estimatedCostUsd,assistedOrders,assistedRevenue}`. |
...
| `ai_assistant_daily_limit` | `INTEGER` | `120` | Trần lượt trả lời có gọi Gemini/ngày giờ Việt Nam; 0 tắt phần AI. Không có trần tiền tháng. |
