...
Evidence: `AdminProductImportController.java`, `ProductImportService.java` (`parseJson`, `exportCurrentCatalogAsTemplateJson`/`toExportRequest`, `resolveRelatedAndAccessoryRefs`, `resolveProductRefTokens`), `ApiErrorDetail`/`ImportRowResult`/`ImportReportResponse`, `ProductJpaRepository.findAllBySkuIgnoreCase`/`findBySlug`/`findById`, `ProductVariantJpaRepository.findBySkuIgnoreCase`, `V30__add_inventory_tracking.sql` (`stock_movements` cascade). See `BUSINESS_RULES.md` `PRODUCT_RULE_009`.
