...
| Method | Path | Current purpose | Response shape | Status | Evidence |
|---|---|---|---|---|---|
| `GET` | `/api/v1/search-suggest` | Lightweight typeahead product/article suggestions. `q` is required after trim and has at most 100 characters; accepts optional `limit`, and `lang=vi|en` (default `vi`). Matching and displayed text follow `lang`, with field-level fallback to Vietnamese. Product/article items retain canonical `slug` plus optional `slugEn` so the storefront can build the correct localized URL. Product suggestions use the same customer search scope, word-boundary matching, coverage and relevance order as `GET /api/v1/products?q=...`. The final query word is the word the customer is still typing and matches by **word prefix**, so results narrow monotonically and never collapse to empty mid-typing (`kom` ⊇ `komi` ⊇ `komine`). Article matching follows the same word-boundary/prefix rule as products, is accent-insensitive and treats `%`, `_`, `\\` literally. | `ApiDataResponse<SearchPayload>` | `OWNER_CONFIRMED_2026-09-07` | `SEARCH_RULE_001`–`SEARCH_RULE_004`, `PublicSearchController.java`, `GlobalSearchService.java` |
...
Public catalog/facet search follows `SEARCH_RULE_001`–`SEARCH_RULE_002`: it is case-insensitive and accent-insensitive, retains every entered customer word, matches at word boundaries with the final (in-progress) word matched by prefix, and searches Vietnamese/English product name, slug, product SKU, variant selling SKU, variant option **values**, localized brand and localized category. Variant option **names** (`Kích cỡ`, `Size`, `Màu sắc`, `Color`…), **size-scale option values** (`M`, `L`, `S`, `XL`…) and the size fragment inside a selling SKU are outside the search scope (`OWNER_CONFIRMED_2026-09-07`) — they match most of the catalog and produce noise rather than intent; colour values remain searchable. Prefix matching applies only to customer-readable text (name, brand, category, option values); slug and SKU fields match whole words only, so a one-letter query cannot reach a product through a code fragment. Sellable product markers make the general storefront concepts `sản phẩm`/`hàng mới` searchable without dropping terms; the storefront's fixed `nón` → `mũ` synonym is separate from the Assistant processor. It first returns complete query coverage, then strict-majority coverage, and never applies the Trợ lý BigBike stop-word set. Short descriptions, detailed descriptions, structured description blocks, SEO copy, and other article/content fields are explicitly outside the product search scope. Facets use the same matching semantics as the product list.
...
### Article list — customer search query `q`

`GET /api/v1/articles?q=...` accepts the same trimmed maximum length of 100 characters as product search. It matches public article title and excerpt without Vietnamese accents and without case sensitivity, using the **same word-boundary rule as product search**, with the final query word matched by word prefix (`SEARCH_RULE_002`). Articles no longer match a fragment sitting inside another word. `%`, `_` and `\\` are literal input, never SQL wildcard syntax. The SQL predicate remains a substring prefilter and the word-boundary rule is applied in the service layer, so PostgreSQL and the H2 test profile agree. The header typeahead and the full `/tim-kiem` page call this endpoint with the active locale; the full page renders matching articles below product results. Status: `OWNER_CONFIRMED_2026-09-07` — `SEARCH_RULE_004`.
...
**Thư mục media:** `GET /api/v1/admin/media-folders` (`media.read`); `POST /api/v1/admin/media-folders`, `PATCH /api/v1/admin/media-folders/{id}`, `DELETE /api/v1/admin/media-folders/{id}` (`media.write`). Create/update nhận `{ name, slug?, description?, parentId? }`; tên tối đa 120 ký tự, slug tối đa 160, mô tả tối đa 2000. API không nhận hoặc sinh `systemKey`: mã có sẵn của thư mục hệ thống chỉ là metadata phản hồi, không đổi khi rename/reparent và không được giao diện hiển thị.

Cây chỉ có cấp gốc và một cấp con. Create chỉ nhận `parentId` của thư mục cấp gốc. Update/reparent chỉ nhận thư mục cha cấp gốc hoặc `null`; không được chọn chính folder, một folder con, hay đưa folder đang có folder con xuống dưới một folder khác. Backend chặn mọi trường hợp sai trước khi ghi với `409` theo mã ổn định: `MEDIA_FOLDER_HAS_CHILDREN`, `MEDIA_FOLDER_INVALID_PARENT`, `MEDIA_FOLDER_DEPTH_LIMIT` hoặc `MEDIA_FOLDER_NAME_DUPLICATE`; giao diện dịch các mã này theo ngôn ngữ đang dùng. Thư mục mới hoặc được chuyển có `sortOrder` ở cuối nhóm đích.

Mọi thư mục lá, bao gồm thư mục hệ thống, đều xóa được. Xóa đặt `folder_id` của media về `null`, không xóa row/object media; folder còn con bị từ chối. Khi một folder hệ thống bị xóa, `systemKey` bị xóa cùng folder nên media mới của nhóm tự động tương ứng giữ ở `folder_id = null`. Response trả `parentId`, `depth`, `systemKey`, `sortOrder` và số file gộp của cây con. `folderFilter=<uuid>` bao gồm cả thư mục con; `NONE` vẫn là `folder_id IS NULL`.
...
|---|---|---|
| Phù hợp nhất | `relevance`, or the default when `q` is present and no explicit non-relevance sort is requested | Customer search relevance from `SEARCH_RULE_002`; then availability, newest and stable id |
...
## Manual Maintenance API — removed 2026-08-30

The manual admin-lock contract has been removed completely. There is no replacement API:

- `GET/PUT /api/v1/admin/maintenance` no longer exists.
- Admin STOMP destinations `/topic/admin/maintenance` and `/app/admin/maintenance/uploads` no longer exist.
- The `423 MAINTENANCE_ACTIVE` global admin-write response, maintenance DTOs, upload lease and realtime lock status are no longer part of the contract.
- `POST /api/v1/checkout` and all customer commerce endpoints remain independent of this removal.
...
### Public session, history và image
