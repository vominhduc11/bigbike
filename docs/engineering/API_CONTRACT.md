...
| Admin review moderation | status update/delete | `reviews.write` | Moderate/delete review and write audit log entries with actor id, review snapshot before/after, request IP, and user-agent when available | `CONFIRMED_FROM_CODE` | `AdminReviewController.java`, `AdminReviewService.java` |
