...
| `POST /api/v1/orders/quick-buy` | Creates order directly from one product/variant request. The variant must belong to the requested product (404 otherwise). Same availability, address and COD payment rules as `POST /checkout`. **No storefront entry point** — the "MUA NHANH" PDP button was removed (owner decision, reversing AUD-010); the endpoint remains but is not reached from the web UI (customers order through the cart). | `CONFIRMED_FROM_CODE` | `CheckoutService.quickBuy` |
