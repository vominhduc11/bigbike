...
| `products.update` | Create / patch / publish / soft-delete / restore product, inventory adjust. | `AdminCatalogController` (products POST/PATCH/DELETE/RESTORE), `AdminInventoryController` (variants/adjust) | `/admin/products/new`, `/admin/products/{id}` (`canUpdate`), `/admin/inventory` (`canUpdate`) | `CONFIRMED_BACKEND_ENFORCED` + `CONFIRMED_FRONTEND_GUARD` | `AdminCatalogController.java:107,117,127,144,152,157`, `AdminInventoryController.java:96`, `bigbike-admin/src/App.jsx:174,381` |
...
| Admin SPA | `/admin/products/{id}` | `products.read` (view) / `products.update` (edit) | Yes (`canUpdate` prop) | `AdminCatalogController` GET/PATCH/PUBLISH/DELETE/RESTORE `/products/{id}` | `CONFIRMED_BACKEND_ENFORCED` + `CONFIRMED_FRONTEND_GUARD` | `bigbike-admin/src/App.jsx:173, 327-329`, `AdminCatalogController.java:93,107,111,121,139,152` |
...
|---|---|---|---|---|---|---|
| Products | `products.read` | `products.update` | `products.update` (delete = soft delete, restore = TRASH -> DRAFT) | `SUPER_ADMIN`, `ADMIN`, `SHOP_MANAGER` (read+update); `EDITOR` (read only) | `CONFIRMED_BACKEND_ENFORCED` | `AdminCatalogController.java`, `AdminRolePermissions.java` |
