...
| `products.read` | `SUPER_ADMIN` (wildcard), `ADMIN`, `SHOP_MANAGER`, `EDITOR` | `GET /api/v1/admin/products`, `GET /api/v1/admin/products/{id}`, product presence topic; also one accepted read permission for `GET /api/v1/admin/product-assignment`; plus the product-editor reference reads `GET /api/v1/admin/attributes[/{id}/values]`, `GET /api/v1/admin/size-scales` and `GET /api/v1/admin/size-scale-groups[?includeInactive]` | `V49__create_roles_permissions_tables.sql`, `V121__realign_inventory_warranty_permissions.sql`, `V363__restore_admin_product_read_permission.sql`, `AdminCatalogController.java`, `WebSocketConfig.java` |
| `products.update` | `SUPER_ADMIN` (wildcard), `ADMIN`, `SHOP_MANAGER` | `POST/PATCH /api/v1/admin/products`, `POST /api/v1/admin/products/preview`, `PATCH /api/v1/admin/products/{id}/publish`, `DELETE /api/v1/admin/products/{id}[/permanent]`, `POST /api/v1/admin/products/{id}/restore`, `POST /api/v1/admin/products/homepage-blocks`, `POST /api/v1/admin/products/import/validate`, `POST /api/v1/admin/products/import/commit`, `GET /api/v1/admin/products/import/export/{id}`; also required for editing/saving the dedicated `/admin/featured-products` workspace, which additionally requires `products.read` to load and search products; also every catalog reference-data mutation reached from the product editor — `POST/PATCH/DELETE /api/v1/admin/attributes[-values]`, `POST/PATCH/DELETE /api/v1/admin/size-scales` and `POST/PATCH/DELETE /api/v1/admin/size-scale-groups` | `V49__create_roles_permissions_tables.sql`, `AdminCatalogController.java`, `AdminProductImportController.java`, `App.jsx`, `FeaturedProductsScreen.jsx` |
...
Size scales, size filter groups and variant attributes deliberately have **no permission code of their own**: they are reference data edited inside the product editor, so whoever may edit a product may edit them (`products.read` to view, `products.update` to change). Adding a separate code would split one screen across two authorization boundaries.
