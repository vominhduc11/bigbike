...
`một widget chat duy nhất -> availability/master/quota gate -> context rút gọn không PII của phiên + local fast-path nếu rõ -> Gemini nhận câu hỏi hiện tại + danh sách nhỏ nhóm hàng/thương hiệu công khai + sáu function declarations cố định để tự chọn tool -> backend registry validate schema/relevance/quyền/giới hạn (không preselect expected tool) -> backend service/repository thực thi với bộ lọc ngữ cảnh -> functionResponse có tổng đã xác minh + số card -> {terminal `TOOL` outcome đã xác minh | Gemini final} -> guard sửa cục bộ mệnh đề có căn cứ rồi kiểm duyệt -> lưu customer + assistant message + context rút gọn -> response hiện tại; thẻ Hotline–Zalo–Messenger mở inline trong cùng luồng khi khách bấm Gặp nhân viên`

`search_products -> parser/ràng buộc backend theo CHAT_RULE_015..023 + metadata danh mục/thương hiệu VI/EN hiện hành -> đối chiếu riêng từng tham số AI -> CatalogReadService -> published + sellable + priced -> tối đa 3 cards`; công tắc vận hành quyết định nhánh mới hay nhánh cũ. Ở nhánh mới, Gemini có thể diễn giải cách nói tự nhiên, backend giữ giá parser/context, danh mục/thương hiệu công khai hợp lệ và định danh khách đã nêu; tham số sai bị bỏ riêng, không làm hỏng lượt. Thứ tự vẫn do backend quyết; card cùng khớp ở lượt trước được ưu tiên lại nếu vẫn thuộc kết quả hiện tại. Khi khoảng giá không có kết quả, backend chọn một mẫu sát mép dưới và một mẫu sát mép trên trước khi tạo terminal disclosure. Context rút gọn có thể kế thừa tầm giá và phạm vi đã hậu kiểm cho một câu hỏi sản phẩm tiếp nối, nhưng câu trả lời phải nói rõ tầm giá đang được áp dụng và Gemini không nhận lịch sử hội thoại. Khi backend đã cấp tổng số cho đúng phạm vi của lượt, function response có thể gồm tổng cùng nhóm, tổng trong tầm giá và số card đang hiện; guard chỉ cho phép Gemini lặp lại đúng số tổng hoặc số card có nhãn hiển thị. Khi tìm hụt đúng tên/model/slug, có đúng một mẫu hỏi có hàng/size/màu hoặc tầm giá không có hàng nhưng có phương án gần nhất, backend tạo terminal `TOOL` answer an toàn; khi cần detail khác, vòng thứ hai duy nhất là `get_product` với slug đã được lượt hiện tại xác minh.
