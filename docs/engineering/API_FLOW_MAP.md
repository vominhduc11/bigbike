...
`widget -> availability -> FAQ/tool cố định -> (khi cần) Gemini -> lưu assistant message -> thẻ sản phẩm thật`

`get_my_orders -> CustomerPrincipal từ SecurityContext -> OrderReadService.listCustomerOrders -> chỉ status/placedAt/totalAmount`

`lead có consent -> ChatLeadService -> chat_leads + admin notification`; AI tắt/thiếu khoá/hết trần/lỗi -> `CONTACT` cùng dữ liệu Hotline/Zalo/Messenger hiện có.
