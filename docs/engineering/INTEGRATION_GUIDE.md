...
| Retry/failure | Logical turn has 65 seconds and at most 4 provider calls. Timeout, overload, `429`, `5xx`, network, empty or invalid provider payload only retry the same `gemini-3.7-flash` while budget remains. Safety/content refusal is not retried. Final technical failure returns polite `CONTACT_STAFF` action; it never silently changes model or creates handoff. |
...
| Safety/privacy | Guard limits output to 10 sentences/2,000 characters and verified product cards. Conversation retention is 90 days; first-party visitor memory is 30 days, disclosed/toggleable/deletable, never IP/fingerprint. No chat/question/token/phone is logged. |
...
| Staff handoff | Persist `WAITING`, then after commit notify STOMP/email. Claim is atomic; `ACTIVE` blocks assistant. Reconnect reads history; email/WS failure does not lose queue/message. Outside business hours gives next opening time without contact capture. |
| Image input | Feature is separately switchable, default off. Upload sends only re-encoded bytes, not EXIF/filename; private MinIO bucket plus ownership/`chat.read` stream. Contract is 1 image/turn, 3/conversation, 20/day, 8 MB. Image intent uses the same fixed model and catalog matching remains verified/local with “trông giống”. |
