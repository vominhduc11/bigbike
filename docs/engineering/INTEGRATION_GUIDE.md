...
| WebSocket/STOMP | Admin order, inventory, review, customer, and edit-presence channels are live; order list, pending-order sidebar badge, Dashboard inventory alert, Review list/summary, Customer list/summary, and order/product edit screens consume the feeds | `CONFIRMED_FROM_CODE` | `WebSocketConfig.java`, `AdminOrderWsService.java`, `AdminInventoryWsService.java`, `AdminReviewWsService.java`, `AdminCustomerWsService.java`, `AdminPresenceService.java`, `OrderListScreen.jsx`, `AdminShell.jsx`, `DashboardScreen.jsx`, `ReviewListScreen.jsx`, `CustomerListScreen.jsx`, `OrderDetailScreen.jsx`, `ProductDetailScreen.jsx` |
...
- **Server-side compression before storage** (`MEDIA_RULE_006`): all 3 raster upload paths (admin media original, review photo, customer avatar) run through `ImageCompressionService` (Thumbnailator-backed) before the `putObject` call — admin media original capped at 2000px wide, review photo at 1600px wide, avatar center-cropped to 400×400, all at JPEG quality 0.85. Never upscales, preserves the alpha channel for PNG/WebP, and fails soft (returns the original bytes untouched) on SVG/GIF or a decode error — an upload is never blocked by a compression failure. Response `width`/`height`/file size reflect the compressed bytes actually written to MinIO. `CONFIRMED_FROM_CODE`
