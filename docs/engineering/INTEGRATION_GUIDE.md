...
| SMTP mail | Transactional email path exists when env is configured. All mail goes through `EmailDispatchService.send(to, subject, template, ctx)` rendering a Thymeleaf template under `templates/email/`. Templates include customer `password-reset`, `password-change-alert`, order confirmation, and **admin `admin-invite`** (set-password link for newly invited admins; degrades gracefully — when no `JavaMailSender` bean is configured, `isEnabled()` is false and the caller surfaces the invite link instead). The shop-owned internal recipient is declared once as required `BIGBIKE_MAIL_ADMIN`; order and chat handoff notifications use that shared value when no explicit chat setting override is present. Startup rejects a missing/invalid value. A successful SMTP call means only that the provider accepted the message for processing, not that the recipient received it. | `CONFIRMED_FROM_CODE` + `CONFIRMED_FROM_CONFIG` | `EmailDispatchService.java`, `AdminInviteService.java`, `templates/email/admin-invite.html` |
...
fallback. Both new-order notifications and staff-handoff notifications read the same
resolved value; a blank `ai_assistant_handoff_email_recipient` setting continues to mean
"use the shared deployment mailbox". The backend fails during startup when
...
| Aspect | Contract |
|---|---|
| Client | Plain Spring `RestClient`, `x-goog-api-key` header. Model trả lời được snapshot từ setting đã validate; fallback nhanh lấy từ registry. Deadline model chính 35 giây và toàn logical turn 65 giây. Retry/fallback cùng nằm trong trần 4 provider requests và cùng daily slot/requestId; không retry `4xx` khác, schema, safety hoặc guard. |
...
| Tool boundary | Gemini chỉ chọn bảy tool đọc cố định; registry backend validate **từng call độc lập** về tên/schema/sequence/quyền/slug. Call sai trả mã lỗi làm sạch cho model và không hủy call hợp lệ khác; nếu tất cả sai chỉ được một lần chọn cách tra khác trong trần 4 request. `search_articles` chỉ đọc tối đa ba bài PUBLISHED đúng locale và trả nội dung chung đã loại dữ kiện động/URL. Tối đa 3 tool/lượt. `capture_lead` không expose; identity chỉ lấy từ server session. Alias tìm hàng được quét với catalog thật; alias va chạm bị reject. |
...
| Cost controls | Fast-path, input refusal và làm rõ chạy local với `ai_called=false`; câu hỏi/câu trả lời clarification không tính trần substantive. Trần conversation mặc định 40, owner chỉnh 10–100; gần trần tạo successor/hand-off thay vì hard stop. Một logical AI turn tối đa 4 provider requests vẫn dùng một daily slot; daily 400 và cảnh báo chi phí giữ độc lập. |
| Transaction boundary | Chỉ transaction ngắn được phép tải/khóa conversation, lưu customer turn và hoàn tất assistant turn. Mọi provider wait nằm ngoài transaction/kết nối DB; các tool đọc dùng transaction read-only riêng và đóng trước provider request kế tiếp. Cùng conversation vẫn tuần tự bằng khóa tiến trình có giới hạn trên deployment một backend hiện tại. |
| Failure | Disabled, empty credential, exhausted daily quota hoặc provider failure trả `CONTACT`; input/Gemini safety trả `REFUSAL` và giữ hội thoại. Lỗi tool chỉ loại call tương ứng. Khi không còn kết quả an toàn, câu khách thấy phải nói đang bận và mời thử lại/gặp nhân viên, không được nói shop không bán sản phẩm. Stream không phát draft, chỉ progress code soạn sẵn và final đã guard. |
| Privacy/retention | Conversation rows expire after 90 days. First-party random visitor memory slides at most 30 days, is disclosed/toggleable/deletable, and never uses IP/fingerprint. Browser keeps token/sequence/UI state, not authoritative transcript; logout isolates account history. Login merges only current signed visitor. No chat/question/phone/token is logged. |
| Sales policy | Stage/objection/next-step/lead/cross-sell/sanitizer chạy local quanh orchestration hiện có; không thêm provider pass. Accessory lấy relation owner-curated, order tool chỉ đọc own-account line snapshots. |
| Staff handoff | Persist `WAITING`; after commit phát `/topic/admin/chat` và email. Claim là conditional update/row lock; `ACTIVE` chặn assistant. Staff REST mutation persist trước rồi broker hiện có push `/user/queue/chat`; reconnect đọc history `afterSequence`. Customer STOMP token ngắn hạn chỉ nằm CONNECT header. SMTP/WS lỗi không mất queue/message và không log content/contact/token. |
| Attribution | Product card proof được ký server-side, không PII; product view dùng server time và token 168 giờ. Cart/checkout revalidate token, product, caller và timestamp; không tin local clock hoặc client conversation id đơn lẻ. |
| Model discovery | Backend gọi `GET /v1beta/models` bằng chính `GEMINI_API_KEY`, chỉ nhận model hỗ trợ `generateContent`, bỏ preview/experimental khỏi danh sách bật cho khách, rồi giao với registry giá/capability có `effectiveFrom/effectiveTo`. Không model/pricing nào được hardcode ở UI. Nếu discovery lỗi, UI hiện lỗi và model đang chạy; không tự suy từ trí nhớ. |
| Evaluation | Bộ đề là tài nguyên versioned, đã che PII và có expected facts do người kiểm chứng; chạy ngoài controller khách, dùng usage category `EVALUATION`, hard cap 2 USD/run và không dùng `chat_ai_daily_usage`. Scorer chỉ so rule/fact/latency/cost bằng mã xác định; Gemini không tự chấm Gemini. |
| Image input | Chỉ gửi bản ảnh đã re-encode, không EXIF/tên tệp gốc, cùng prompt ranh giới B2–B4. Vision output là tín hiệu ứng viên, không là bằng chứng sản phẩm/giá/thông số. Mọi card phải resolve lại catalog sống. Unsafe block dừng trước vision/search. |
| Image matching | Backend dựng dấu vân tay cục bộ từ ảnh chính trong bucket media nội bộ và lưu theo product/media/source hash/version. Lượt ảnh khách chỉ tính dấu vân tay trong bộ nhớ, so bằng ngưỡng hình–màu–tỷ lệ cùng khoảng cách với mẫu thứ hai rồi bỏ; không gọi embedding provider, không lưu vector ảnh khách và không phát sinh phí index. Vision chỉ phân loại intent/nhóm. |
| Private storage | Ảnh chat ở bucket `MINIO_CHAT_PRIVATE_BUCKET`, startup gỡ anonymous-read policy và mỗi upload fail-closed nếu bucket vẫn có policy. Backend stream object qua endpoint đã kiểm ownership/`chat.read`; object name ngẫu nhiên, không chứa tên tệp/PII. Bucket media công khai hiện hành không được dùng cho ảnh khách. |

Environment: `GEMINI_API_KEY`, `BIGBIKE_CHAT_MODEL=gemini-2.5-flash` (bootstrap/fallback khi setting chưa có), `BIGBIKE_CHAT_FALLBACK_MODEL=gemini-2.5-flash-lite`, `BIGBIKE_CHAT_PRIMARY_TIMEOUT_SECONDS=35`, `BIGBIKE_CHAT_TIMEOUT_SECONDS=65`, `BIGBIKE_CHAT_MODEL_CATALOG_CACHE_MINUTES=15`, `BIGBIKE_CHAT_EVALUATION_MAX_COST_USD=2.00`, `MINIO_CHAT_PRIVATE_BUCKET=bigbike-chat-private`. Pricing runtime đến từ registry có ngày hiệu lực, không còn lấy một cặp giá env chung để so model. Khai báo template ở `.env.example`, `.env.vps.example` và compose; `.env` local cùng `.env.vps` VPS không commit và không được trộn lẫn.
