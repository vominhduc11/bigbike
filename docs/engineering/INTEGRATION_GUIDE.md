...
| Aspect | Contract |
|---|---|
| Client | Plain Spring `RestClient`, `x-goog-api-key` header, fixed 5s connect / 20s read timeout, `gemini-2.5-flash`. Không có network retry; chỉ có đúng một retry cấp ứng dụng khi final bị guard loại duy nhất vì `WRONG_TONE` và còn slot dự phòng của daily limit. |
| Request | Request đầu chỉ có current question và năm function declarations cố định. Request tiếp theo chỉ nối exact model function call cùng function response tối thiểu của backend trong cùng logical turn. Never full catalog, SQL, customer id/email/address, API key hoặc unrelated conversation history. |
...
| Tool boundary | Gemini chỉ chọn `search_products|get_product|get_policy|get_shop_info|get_my_orders`; registry backend validate schema/sequence/quyền rồi mới gọi service/repository cố định. `capture_lead` không expose; customer identity chỉ lấy từ server session. Unknown/extra/identity/SQL arguments và parallel calls đều fail closed — không call nào chạy khi chưa qua validate. Ở request đầu, fail closed nghĩa là bỏ cả turn; ở detail hop (tùy chọn) nghĩa là bỏ qua hop đó rồi trả lời bằng search results đã grounded. |
| Generation | `thinkingBudget: 0`, `maxOutputTokens: 400`; request đầu dùng function-calling mode `ANY` để câu hỏi đã qua fast-path luôn nhận một function call trong allowlist, request detail dùng `ANY` với `allowedFunctionNames=[get_product]`, final request giữ declarations để nối đúng stateless history nhưng đặt mode `NONE`. Final request **bắt buộc** kèm `responseMimeType: application/json` + `responseSchema` bốn field (`answer`/`offTopic`/`handoffRecommended`/`leadPrompt`, có `propertyOrdering`); backend vẫn parse/validate schema nghiêm ngặt sau đó. Verified 2026-08-10: mode `NONE` không gọi function nào nên structured output **chạy được cùng function declarations trên `gemini-2.5-flash`** — đo trực tiếp 0/8 reply đúng contract khi thiếu, 8/8 khi có (khẳng định cũ "chỉ Gemini 3 mới kết hợp được" là sai). System prompt bắt buộc câu VI tự xưng “em”, gọi khách “anh/chị”, không gọi khách là “em”; guard kiểm lại điều này. System prompt hướng model tới 3–5 câu, nhưng phải cho phép câu đúng 2 câu vì guard theo `CHAT_RULE_007` chấp nhận 2–5; prompt vẫn cấm ghi giá/tiền tệ trong `answer`. Tối đa 2 tool executions/3 provider requests cho một orchestration, chỉ cho chuỗi thứ hai `search_products -> get_product`. Final bị `WRONG_TONE` được chạy lại một lần với chỉ dẫn sửa xưng hô, không gửi history hay draft đã bị loại. Detail hop là **tùy chọn**: turn detail không dùng được (parallel `get_product` calls, prose, nhiều part) chỉ bị **bỏ qua** kèm log `detail step skipped` rồi đi thẳng final bằng search results đã grounded — không có call chưa validate nào được chạy và turn không rơi về `CONTACT`. Sau `search_products`, một terminal outcome deterministic đã hậu kiểm (đúng mẫu không có kết quả; một mẫu đang bán khi hỏi có hàng/size/màu; hoặc phương án gần nhất sau khi hụt tầm giá) có thể bỏ final provider và trả `TOOL`; mọi reply model không parse được, safety-blocked, thiếu grounding, timeout hoặc không qua guard đều trở thành fallback an toàn trong khung chat, không recovery model text bằng snapshot trung gian. |
| Cost controls | FAQ/shop-contact/order summary đã đăng nhập/guest-order/clarification fast-path locally; maximum 12 turns/conversation; 10 messages/min/IP. Một logical turn có 1–3 provider requests nội bộ vẫn dùng một daily slot theo `Asia/Ho_Chi_Minh`. Một retry sửa xưng hô được thử sau `WRONG_TONE` dùng thêm một slot, lưu bằng `ai_retry_count=1` trên assistant message duy nhất và tăng `ai_call_count` tương ứng; retry chỉ bắt đầu khi còn đủ slot cho cả lượt hiện tại lẫn lượt sửa. |
