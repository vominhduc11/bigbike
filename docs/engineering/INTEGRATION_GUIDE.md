...
| Tool boundary | Gemini chỉ chọn sáu tool đọc cố định; registry backend validate tên/schema/sequence/quyền/slug trước khi chạy. Tối đa 3 tool/lượt, cho phép call hợp lệ song song hoặc nối tiếp. `capture_lead` không expose; identity chỉ lấy từ server session. Alias tìm hàng được quét với catalog thật; alias va chạm bị vô hiệu hoá. |
...
| Cost controls | Fast-path và input refusal chạy cục bộ; tối đa 12 lượt/conversation, mặc định 120 AI lượt/ngày, không có trần tiền tháng. Một logical turn tối đa 4 provider requests vẫn dùng một slot. Lưu input/output/thinking token, request count, latency và chi phí ước tính; giá mặc định Flash là USD 0.30/M input và USD 2.50/M output gồm thinking, có thể cấu hình khi bảng giá đổi. |
