...
Kiến trúc render: **ISR on-demand + SSG + CSR, cùng render theo request khi phụ thuộc query URL.** Product landing từ GMC cần lựa chọn biến thể đúng ngay trong HTML (`GMC_RULE_004`). `CONFIRMED_FROM_CODE`

| Loại | Áp dụng | Cơ chế |
|---|---|---|
| **ISR on-demand** (admin content) | Route động admin quản lý không đọc request query ở server, các route tĩnh theo slug | `generateStaticParams()` trả **`[]`** → **KHÔNG prebuild/gọi API lúc build**; trang sinh khi truy cập lần đầu rồi cache, revalidate qua `next.tags` (`product:{slug}`, `category:{slug}`, `brand:{slug}`, `article:{slug}`…). (Trang thông tin/chính sách `gioi-thieu`/`lien-he`/`huong-dan*`/`chinh-sach/*`/catch-all `[slug]` **không** còn thuộc nhóm này — chuyển sang tĩnh, xem hàng dưới; module pages + guide-page gỡ 2026-06-24.) |
| **Dynamic per request** (query-dependent) | `product/[slug]`, alias `internal/product/[slug]`, `danh-muc/[slug]`, `brands/[slug]`, `sp`, `tim-kiem`, và các route đọc `searchParams` ở server | `searchParams` dựng đúng lựa chọn biến thể/bộ lọc/phân trang trong HTML đầu tiên. PDP gốc, alias PDP và các route catalog khai `force-dynamic`; request render lấy dữ liệu hiện hành, không cache chung HTML mang lựa chọn của URL khác. Cấu hình route phải khai trực tiếp tại từng alias. |
...
| **Dynamic (ƒ) route handlers** | Route handlers/API routes, gồm `/google-merchant.xml` và redirect tương thích `dat-hang/order-received/[id]` | Xử lý request/redirect ở server. Nguồn GMC đọc catalog mới không cache; page phụ thuộc query nằm ở hàng Dynamic per request. |
...
Header giữ nguyên fragment khi đổi ngôn ngữ cả hai chiều. Với URL có fragment, dùng điều hướng tài liệu cùng tab, thêm một mục lịch sử mới, để tránh cache điều hướng nối fragment lần hai khi quay về URL đã mở. URL không có fragment tiếp tục dùng điều hướng trong ứng dụng. Quay lại/Tiến tới phải phục hồi đúng URL và ngôn ngữ.
