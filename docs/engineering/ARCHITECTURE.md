...
| **Static + ISR** (shell) | Trang chủ + các trang archive/list: `san-pham`, `tim-kiem`, `tin-tuc`, `brands`, + **10 trang thông tin/chính sách tĩnh** (Giới thiệu, Liên hệ, Hướng dẫn + 3 trang con, 4 trang chính sách) | Shell render tĩnh; dữ liệu admin (hero/menu/settings) fetch với `next.tags` + revalidate on-demand. **10 trang info/policy** nay **đóng cứng** trong web — nội dung song ngữ từ `lib/content/static-pages.json` (`static-pages.ts`), route giữ URL cũ (`/gioi-thieu`, `/lien-he`, `/huong-dan/{mua-hang\|size-mu\|size-gang-tay}`, `/chinh-sach/{slug}`, catch-all `/[slug]`); **không gọi backend** (module pages + guide-page gỡ 2026-06-24, bảng `pages`/`guide_page_layout` drop V271). `/lien-he` còn fetch `listPublicSettings` (tag `settings`) cho thông tin liên hệ dùng chung; `/chinh-sach` còn đọc menu `policy` cho sidebar. |
