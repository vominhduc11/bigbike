...
| **Static + ISR** (shell) | Trang chủ + các trang archive/list: `san-pham`, `tim-kiem`, `tin-tuc`, `brands`, + nội dung tĩnh (giới thiệu, liên hệ, bảo hành, hướng dẫn…) | Shell render tĩnh; dữ liệu admin (hero/menu/settings) fetch với `next.tags` + revalidate on-demand. |
