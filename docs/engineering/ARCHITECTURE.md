...
| Loại | Áp dụng | Cơ chế |
|---|---|---|
| **ISR on-demand** (admin content) | Route động admin quản lý: `product/[slug]`, `brands/[slug]`, `danh-muc-san-pham/[slug]`, `tin-tuc/[slug]`, `[slug]` (CMS page), `chinh-sach/[slug]`, `huong-dan/[...sub]` | `generateStaticParams()` trả **`[]`** → **KHÔNG prebuild/gọi API lúc build**; trang sinh khi truy cập lần đầu rồi cache, revalidate qua `next.tags` (`product:{slug}`, `category:{slug}`, `brand:{slug}`, `article:{slug}`, `page:{slug}`…). |
| **Static + ISR** (shell) | Trang chủ + các trang archive/list: `san-pham`, `tim-kiem`, `tin-tuc`, `brands`, + nội dung tĩnh (giới thiệu, liên hệ, bảo hành, hướng dẫn…) | Shell render tĩnh; dữ liệu admin (hero/menu/settings) fetch với `next.tags` + revalidate on-demand. **`/lien-he`**: phần thân do admin dựng qua trình dựng trang Liên hệ — fetch `getContactPageLayout` (tag `contact-page`, `page:lien-he`) + `listPublicSettings` (tag `settings`); khi admin lưu, backend revalidate `page:lien-he` + `settings`. |
