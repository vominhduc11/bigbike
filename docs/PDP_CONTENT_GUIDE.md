...
Trang chi tiết sản phẩm (`bigbike-web` — `components/catalog/ProductView.tsx`) render các khối theo đúng thứ tự sau (khối tự ẩn khi không có dữ liệu). Thứ tự bám sơ đồ "thứ tự đầy đủ trang sản phẩm" của chủ shop. **Thứ tự GIỐNG NHAU trên desktop và mobile.** Khác biệt duy nhất: cụm **"Thông số"** (#8 Thông số kỹ thuật · #9 FAQ · #10 Video · #11 Đánh giá) — **mobile gom 4 khối này vào MỘT widget tab** đặt ngay sau Bảng size; **desktop render giãn, xếp chồng** (không tab). Mọi khối còn lại (Tính năng chi tiết, Ưu/Nhược + Sản phẩm tương tự, Phù hợp với ai, Bảng size, Trust, Phụ kiện) xếp chồng flat ở cả hai màn. "Tính năng chi tiết" (#3) nay flat, KHÔNG còn nằm trong widget tab.
...
| 4 | Ưu điểm & Nhược điểm | `positiveNotes` / `negativeNotes` | 2 cột. Gộp chung MỘT khối `prosConsRelated` với "Sản phẩm tương tự" (#5). Khối flat → hiện ngay sau Tính năng chi tiết trên CẢ desktop lẫn mobile. |
| 5 | **Sản phẩm tương tự — "Xem thêm lựa chọn"** | `relatedProducts` | Cùng loại (auto theo tag). Đặt NGAY sau Ưu/Nhược điểm (gộp chung khối `prosConsRelated`): khách vừa đọc nhược điểm/giá → thấy ngay lựa chọn thay thế, giữ khách lại site. Hiện ở vị trí này trên CẢ desktop lẫn mobile. |
| 6 | **Phù hợp với ai** | `descriptionBlocks` (khối `suitability`) | Danh sách thẻ (đối tượng + lời khuyên + link nội bộ tùy chọn). **Tách RA khỏi luồng mô tả**, render khối flat riêng ngay sau "Sản phẩm tương tự". Tự hiện khi có nội dung. *(Trước V246 dùng field `suitabilityAdvisory`; nay là khối `suitability` trong `descriptionBlocks`.)* |
| 7 | Bảng size | `descriptionBlocks` (khối `sizeGuide`) | Khối flat riêng, có điều kiện (chỉ hiện khi SP có size). **Tách RA khỏi luồng mô tả**, render ngay sau "Phù hợp với ai", **ngay trước cụm Thông số**. Tự hiện khi có nội dung. *(Trước V246 dùng field `sizeGuide`; nay là khối `sizeGuide` trong `descriptionBlocks`.)* |
| 8 | Thông số kỹ thuật | `specifications` | **Cụm Thông số** — desktop: khối flat xếp chồng; mobile: tab đầu trong widget. (Lắp đặt `installationGuide` V242 — lưới các bước — chèn cạnh đây nếu có.) |
| 9 | FAQ | `faqs` | Cụm Thông số — desktop: khối flat; mobile: tab trong widget. |
| 10 | Video sản phẩm | `videos` | Cụm Thông số — desktop: khối flat; mobile: tab trong widget; dùng chung `HomeVideoCarousel`. |
| 11 | Đánh giá | `ReviewsSection` | Cụm Thông số — `id="reviews"`; desktop: khối flat; mobile: tab cuối trong widget. |
| 12 | Trust block "Mua tại BigBike.vn" | site settings | Lưới ô tự động (Giá · Kho · Liên hệ · Địa chỉ). Giá/Kho realtime (cùng nguồn nút mua); **Liên hệ = Hotline + Zalo** và **Địa chỉ** auto từ `site_settings`. Trống Zalo/Hotline/Địa chỉ thì auto bỏ ô đó. **(V276)** Đã gỡ 3 ô admin nhập tay Bảo hành/Giao hàng/Đổi size (`product_purchase_lines`) — khối giờ thuần tự động, admin không nhập gì. |
