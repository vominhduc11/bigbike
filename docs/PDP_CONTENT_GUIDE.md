...
Trang chi tiết sản phẩm (`bigbike-web` — `components/catalog/ProductView.tsx`) render các khối theo đúng thứ tự sau (khối tự ẩn khi không có dữ liệu). Thứ tự bám sơ đồ "thứ tự đầy đủ trang sản phẩm" của chủ shop. **Thứ tự GIỐNG NHAU trên desktop và mobile.** Khác biệt duy nhất: cụm **"Thông số"** (#9 Thông số kỹ thuật · #10 FAQ · #11 Video · #12 Đánh giá) — **mobile gom 4 khối này vào MỘT widget tab** đặt ngay sau Bảng size; **desktop render giãn, xếp chồng** (không tab). Mọi khối còn lại (Quick Answer, Tính năng chi tiết, Ưu/Nhược + Sản phẩm tương tự, Phù hợp với ai, Bảng size, Trust, Phụ kiện) xếp chồng flat ở cả hai màn. "Tính năng chi tiết" (#4) nay flat, KHÔNG còn nằm trong widget tab.

| # | Khối | Nguồn dữ liệu | Ghi chú |
|---|---|---|---|
| 1 | Gallery + thông tin mua hàng | `WpPurchaseSection` | Hàng sao có link "Viết đánh giá đầu tiên" → cuộn tới khối Đánh giá (`#reviews`). Khối "cam kết" (`product_commitments`, V232) hiển thị **dưới nút mua** trong khu này. |
| 2 | Specs Dashboard (4 ô số liệu) | `specStats` | — |
| 3 | **Quick Answer** | `quickAnswerSummary` | Đoạn tóm tắt AIO **40–60 từ** để Google/AI trích dẫn — câu đầu nói thẳng sản phẩm là gì + cho ai + nổi bật điều gì; văn bản thường, không định dạng. Render **blockquote** ngay sau Specs Dashboard, ngay trước "Tính năng chi tiết". Song ngữ (vi canonical + `_en`), max 600 ký tự, detail-only. Tự ẩn khi rỗng — không có công tắc bật/tắt riêng. *(Có ở V236, gỡ hoàn toàn ở V253, làm lại ở V300 — 2026-07-02.)* |
| 4 | Tính năng chi tiết | `descriptionBlocks` | Khối full-trang flat (hiện CẢ desktop lẫn mobile; đã kéo RA khỏi widget tab). Khối `feature` tự co theo nội dung admin đã nhập — **4 quy tắc hiển thị**: (1) đủ ảnh + chữ → 2 cột ảnh–chữ so le trên desktop (mobile xếp dọc), `side`=`auto` tự xen kẽ trái/phải; (2) chỉ có chữ → chữ tràn full width, không chừa nửa cột trống; (3) chỉ có ảnh → ảnh full width; (4) trống hoàn toàn → bỏ qua, không sinh section. Admin đăng chữ trước, thêm ảnh sau → web tự chuyển thành 2 cột. Cơ chế "ghép ngầm" cũ (tự gom `image`+`text` liền nhau) đã GỠ — muốn 2 cột phải dùng khối `feature`. "Phù hợp với ai"/"Bảng size" nhập ở **2 card riêng** (không còn là khối thêm trong trình dựng mô tả); dữ liệu vẫn lưu dạng khối `suitability`/`sizeGuide` trong `descriptionBlocks`, web **TÁCH RA** render thành khối #7/#8 riêng. |
| 5 | Ưu điểm & Nhược điểm | `positiveNotes` / `negativeNotes` | 2 cột. Gộp chung MỘT khối `prosConsRelated` với "Sản phẩm tương tự" (#6). Khối flat → hiện ngay sau Tính năng chi tiết trên CẢ desktop lẫn mobile. |
| 6 | **Sản phẩm tương tự — "Xem thêm lựa chọn"** | `relatedProducts` | Cùng loại (auto theo tag). Đặt NGAY sau Ưu/Nhược điểm (gộp chung khối `prosConsRelated`): khách vừa đọc nhược điểm/giá → thấy ngay lựa chọn thay thế, giữ khách lại site. Hiện ở vị trí này trên CẢ desktop lẫn mobile. |
| 7 | **Phù hợp với ai** | `descriptionBlocks` (khối `suitability`) | Danh sách thẻ (đối tượng + lời khuyên + link nội bộ tùy chọn). **Tách RA khỏi luồng mô tả**, render khối flat riêng ngay sau "Sản phẩm tương tự". Tự hiện khi có nội dung. *(Trước V246 dùng field `suitabilityAdvisory`; nay là khối `suitability` trong `descriptionBlocks`.)* |
| 8 | Bảng size | `descriptionBlocks` (khối `sizeGuide`) | Khối flat riêng, có điều kiện (chỉ hiện khi SP có size). **Tách RA khỏi luồng mô tả**, render ngay sau "Phù hợp với ai", **ngay trước cụm Thông số**. Tự hiện khi có nội dung. *(Trước V246 dùng field `sizeGuide`; nay là khối `sizeGuide` trong `descriptionBlocks`.)* |
| 9 | Thông số kỹ thuật | `specifications` | **Cụm Thông số** — desktop: khối flat xếp chồng; mobile: tab đầu trong widget. (Lắp đặt `installationGuide` V242 — lưới các bước — chèn cạnh đây nếu có.) |
| 10 | FAQ | `faqs` | Cụm Thông số — desktop: khối flat; mobile: tab trong widget. |
| 11 | Video sản phẩm | `videos` | Cụm Thông số — desktop: khối flat; mobile: tab trong widget; dùng chung `HomeVideoCarousel`. |
| 12 | Đánh giá | `ReviewsSection` | Cụm Thông số — `id="reviews"`; desktop: khối flat; mobile: tab cuối trong widget. |
| 13 | Trust block "Mua tại BigBike.vn" | site settings | Lưới ô tự động (Giá · Kho · Liên hệ · Địa chỉ). Giá/Kho realtime (cùng nguồn nút mua); **Liên hệ = Hotline + Zalo** và **Địa chỉ** auto từ `site_settings`. Trống Zalo/Hotline/Địa chỉ thì auto bỏ ô đó. **(V276)** Đã gỡ 3 ô admin nhập tay Bảo hành/Giao hàng/Đổi size (`product_purchase_lines`) — khối giờ thuần tự động, admin không nhập gì. |
| 14 | Hoàn thiện bộ bảo hộ — cross-sell | `accessories` (admin curate) | Khác loại (găng/áo giáp/giày) để tăng AOV; render một lần ở cuối luồng marketing |

Sticky mua-hàng (mobile), "Đã xem gần đây", dải liên hệ giữ nguyên ở cuối.

> **Đã gỡ công tắc "Hiển thị trên web" (2026-06-22):** trước đây admin bật/tắt riêng 5 phần Mô tả/Thông số/FAQ/Video/Đánh giá (`sectionVisibility`, V245) và SP mới mặc định tắt hết — dễ gây "nhập nội dung mà không hiện". Nay **mọi khối hiện thuần theo nội dung** (giống các khối còn lại). Đã gỡ ở admin (ô nhập) + web (gating); trường backend `section_visibility` giữ ngủ yên (không còn tác dụng).

**Thanh nhảy-mục MOBILE (anchor nav tổng):** một thanh nổi ở đầu phần nội dung (xuất hiện sau khi khu mua hàng `#pdp-overview` cuộn khỏi tầm nhìn) liệt kê **mọi khối có nội dung theo đúng thứ tự trên** — Mô tả · Ưu/Nhược · Phù hợp · Bảng size · Thông số · FAQ · Video · Đánh giá · Mua tại BigBike · Bán kèm — bấm để nhảy tới. Khối flat dùng id `pdp-*`; cụm Thông số dùng id panel trong widget tab (`tab-more_infomation`/`tab-faq`/`tab-videos`/`reviews`). Widget tab cụm Thông số KHÔNG còn thanh nav riêng (`hideAnchorNav`) để tránh 2 thanh chồng nhau. Chỉ hiện ở mobile (`md:!hidden`); desktop không có thanh này.
