...
>
> Đây là **bản tóm tắt mirror** của [AGENTS.md](AGENTS.md) được tinh chỉnh dành riêng cho **Antigravity (Gemini-based AI Coding Assistant)**. 
> Khi có sự mâu thuẫn giữa các file, **AGENTS.md thắng**. Mỗi rule dưới đây trỏ tới section của [AGENTS.md](AGENTS.md) để xem chi tiết/ví dụ.
...
1. **Đọc đúng docs liên quan** (tra mapping dưới), chỉ đọc section bạn đụng đến — không đọc cả file.
...
**Mọi ẢNH của dữ liệu admin quản lý BẮT BUỘC nằm trong MinIO, không trỏ link ra ngoài.** Áp dụng cho product, category, brand, banner/hero, content/blog/policy, settings, v.v. Mọi URL ảnh phải trỏ về object MinIO (`/media/...`).

- **Ngoại lệ — VIDEO nhúng YouTube + TikTok + Facebook:** video được phép dán link YouTube, TikTok hoặc Facebook làm nguồn. Web tự dựng iframe embed. Link rút gọn bị reject.
- **Ngoại lệ — trang TĨNH đóng cứng trong code:** asset nằm trong repo (`bigbike-web/public/...`) — ảnh **KHÔNG bắt buộc** nằm MinIO. Vẫn cấm hotlink ảnh host ngoài.

**Cấm:** hotlink **ảnh** từ host ngoài; embed video nền tảng ngoài YouTube/TikTok/Facebook; cho admin nhập URL ảnh ngoài làm nguồn media.
...
## ⚠️ Quy tắc & Hướng dẫn dành riêng cho Antigravity (Gemini Agent)

Khi thực thi công việc trên repository này, hãy tuân thủ nghiêm ngặt các quy định về việc sử dụng các tool đặc thù của Antigravity:

### 1. File Modification Tools (Chỉnh sửa source code)
- **replace_file_content**: Dùng để chỉnh sửa một block code liên tục duy nhất trong file.
- **multi_replace_file_content**: Dùng khi cần thực hiện nhiều thay đổi không liên tục trên cùng một file. Không thực hiện các cuộc gọi song song (parallel) cho cùng một file.
- **write_to_file**: Chỉ dùng khi tạo file mới hoặc ghi đè toàn bộ một file cực kỳ nhỏ. **CẤM ghi đè toàn bộ nội dung** của các file code lớn hiện tại để tránh tốn token và giảm rủi ro lỗi cú pháp.

### 2. Command Line Execution (Chạy command)
- **CẤM** đề xuất hoặc sử dụng lệnh `cd` trong tool `run_command`. Mọi lệnh chạy cần được thiết lập trực tiếp thông qua parameter `Cwd` trỏ đến thư mục mong muốn.
- Khi cần kiểm tra trạng thái container Docker: luôn dùng `run_command` chạy `docker ps` trước.

### 3. Artifacts & Planning Mode
- Khi ở **Planning Mode** (cần thay đổi lớn, tính năng phức tạp):
  1. Tạo/cập nhật `implementation_plan.md` (metadata: `request_feedback: true`).
  2. Đợi user approve.
  3. Tạo `task.md` làm checklist.
  4. Sau khi hoàn thành, tạo `walkthrough.md` tổng kết.
- Với các thay đổi nhỏ, fix bug, sửa style hoặc Q&A: **không cần lập plan** để tránh rườm rà.
